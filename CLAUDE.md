...
> Ultimo aggiornamento: 2026-05-26 v13 (ADR 0162: ClusterLLM + Pronoia classify_fail + retry on repeat — bucket emergente embedding-based, granularita' feedback ✓/↻/✗).
...
- **Safety net 7-layer per skill imported** (ADR 0159, 24/5/2026): L1 sign verify, L2 affinity overlap Jaccard ≥0.5 (≥0.85 binding-suffixed), L3 efficacy ager (deprecate 30g, archive 14g), L4 sandbox per-skill planned-not-implemented (Fase C ADR 0140; trigger ≥5 skill OR ≥1 guest paired), L5 smoke battery, L6 LLM semantic verifier Gemma 4 26B, runtime `vaglio.judge`, skill_audit JSONL. Coverage: manomissione/squatting/drift/silent-break/exfiltration. Builtin handcrafted NON ha L2/L6/audit.
- **L2/L5/L6 default-ON + audit sharded** (ADR 0159 drift fix, 24/5/2026): L2 SoT centralizzata in `loader.check_affinity_pair(a,b,threshold)` (no duplicato in skill_admission); L5 smoke eseguito at-import via `skill_admission._run_smoke_for_plan` (reject su routing miss, bypass `--skip-l5-exec`); L6 default mandatory ON per imported (bypass `--skip-l6` o `METNOS_STAGE6_VERIFY_IMPORTED=0` legacy); `skill_audit/<YYYY-MM-DD>.jsonl` sharded daily + migrazione lazy del flat legacy.
...
- **Skill importer R1+R2+R3 (ADR 0159 wiring, 24/5/2026)**: R1 `skill_description_llm.generate_description_or_fallback` wired in `cli/skills_cli._cmd_import` PRIMA del codegen, time budget 5s/plan (env `METNOS_SKILL_LLM_TIMEOUT_S`), thread-with-deadline wrapper simmetrico fra fake e LLM reale, audit `<PATH_USER_DATA>/skill_descriptions_audit.jsonl`. R2 `importer_verb_verify.check_plan` gate post-translate in `skill_translator.translate_skill`: plan con `aligned=False` finisce in `rejected` con prefix `verb_boundary:`. R3 R3 fallback locali `_METNOS_VERBS/_OBJECTS/_QUALIFIERS` rimossi: `skill_translator` + `skill_admission._load_vocab` importano DIRETTO da `vocab.ACTIONS/OBJECTS/QUALIFIERS` (single source of truth §7.3, niente drift). G6 fix naming: `skill_admission._HANDCRAFTED_DIRS`/`_SYNTH_DIRS` (path strings) distinti da `loader.HANDCRAFTED_FAMILIES` (frozenset di nomi); helper `_is_handcrafted/_is_synth/_is_imported` simmetrici. Perf: `_load_vocab_map` cache mtime-validated, `_jinja_env` singleton, `smoke_imports.begin_batch/flush` (19× write → 1×). Tests: 34 nuovi in `test_skill_codegen_with_llm.py`+`test_skill_parser_malformed.py`+`test_skill_fetch_cache_ttl.py`+`test_skill_codegen_round_trip.py`.
- **Locale-aware skill bundle + rename _imports→skills** (ADR 0160, 24/5/2026): pattern bundle-per-locale per feature dipendenti dal mercato/cultura nazionale. Esempio canonico: `executors/skills/it_locale/` raggruppa N feature italiane (bills_extractor utility/telecom, future codice_fiscale, iban_it, partita_iva, date_it, tariffe_it) in un SOLO namespace anziche' N skill granulari. Frontmatter SKILL.md nuovi campi: `lang`/`trust: metnos-official|community|unverified`/`auto_enable`/`distribution: hub-installable|bundled|community`/`feature_modules: [name1, name2]`. Bundle NON ha `manifest.toml` al root (helper-library, non executor); `vendors.json` lazy via `Path(__file__).parent.parent` (§7.11), override env `METNOS_BILLS_VENDORS_PATH`. Pattern §7.3 generale: riusabile per `fr_locale/`, `de_locale/`, `es_locale/`. Aggiungere feature al bundle = nuovo file in `scripts/` + append a `feature_modules:` (NO nuova SKILL.md, NO dir separata). Rename concomitante `_imports/` → `skills/` (ADR 0123 storage canonico, READ-ONLY back-compat su path legacy); loader scansiona entrambi, write SEMPRE in `PATH_SKILLS_USER` (vedi `runtime/skills_paths.py`). Baseline 24/5: `runtime/bills_extract.py` (447 LOC) + `runtime/bills_vendors.json` (47 LOC) migrati → `executors/skills/it_locale/scripts/bills_extract.py` + `executors/skills/it_locale/vendors.json`.
- **ClusterLLM + Pronoia classify_fail + retry on repeat** (ADR 0162, 26/5/2026): estende Praxis Engine (ADR 0161). **(1) ClusterLLM**: `runtime/praxis_cluster.py` sostituisce `intent_hash` lessicale con `cluster_id` emergente via BGE-M3 embedding + cosine + LLM judge zona grigia (`COSINE_HIGH=0.90`/`COSINE_LOW=0.75`). Query semantically equivalenti (lingue/sinonimi diversi) condividono cluster → promote dopo 2 ✓ totali (vs mai prima). **(2) Champion/challenger darwiniano**: `composite_score = 0.5·succ + 0.3·speed + 0.2·alignment`, swap se `swap_score = 2·Δsucc + Δlat_norm > 0.15`. Selezione default pure-exploit + opt-in ε-greedy via `METNOS_EXPLORATION_EPSILON`. **(3) Pronoia classify_fail**: `runtime/pronoia_classify_fail.py` su ✗ LLM Gemma 26B classifica in {format_fail/args_fail/pipeline_fail} (~1.5-2s) → dispatch differenziato: `format_fail`→template_issue=1 + Mētis refresh inline (~3s), `args_fail`→DELETE filler_cache, `pipeline_fail`→fail_count++ + anti_skill TTL 30gg. **(4) Retry on repeat**: ↻ scatena Mētis re-propose framework alternativo INLINE (~3-15s wise tier), salva su skill esistente, soft anti_skill TTL 1h `reason=repeat_soft`. No re-execute (evita side-effects mutating). **(5) Counters separati**: `_touch_skill` aggiorna SOLO `ts_last_used`; `update_skill_metrics(success: bool)` gestisce uses/ok_count/fail_count. **(6) Audit trail**: tabella `skill_versions(skill_id, ts, event, old_fw_hash, new_fw_hash, reason)`. **(7) Batch nightly**: `runtime/jobs/praxis_template_refresh.py` daily@04:00 (template_issue=1) + `runtime/jobs/praxis_cluster_merge.py` daily@03:30 (consolidation cluster singleton via LLM judge, cap 20 pair/fire). **(8) Renderer magic `${stepN.@count}`**: cascata `available_total → ok_count → used → len(entries|results|items|first list[dict])`. Dict value formatter `k=v · k=v` compatto (risolve bug `temperatura GPU è .`). **(9) `compute_framework_hash` include `final_message` template**: ↻ exclude puo' produrre template diversi anche su pipeline minima univoca. **(10) Constants centralizzati** in `runtime/praxis_constants.py` (env-tunable). **(11) Admin endpoint** `GET /admin/skills/{id}/history` con content negotiation JSON/HTML. **(12) Pattern NAME_TO_WEB_REVERSE_IMAGE** in `runtime/prompts/it/praxis_propose.j2` per query "foto enrollement su web" (find_persons_indices + find_images_web 2-step diretto). Test: 33/33 verdi (`tests/test_praxis_cluster.py` 16 + `tests/test_pronoia_classify_fail.py` 13 + 4 renderer). Env tunable: `METNOS_CLUSTER_COSINE_HIGH/LOW`, `METNOS_CLUSTER_K`, `METNOS_CLUSTER_SWAP_THR`, `METNOS_PRAXIS_W_SUCCESS/SPEED/ALIGNMENT`, `METNOS_PRAXIS_EMA_ALPHA`, `METNOS_PRAXIS_MIN_OBS_PRODUCER/MUTATING`, `METNOS_TTL_REPEAT_SOFT_SECS`, `METNOS_TTL_PIPELINE_FAIL_SECS`, `METNOS_PRONOIA_CLASSIFY_FAIL`, `METNOS_REEXECUTE_ON_REPEAT`, `METNOS_EXPLORATION_EPSILON`, `METNOS_CLUSTER_MERGE_*`. HTTP feedback whitelist: action ∈ {ok|error|repeat}.
- **Praxis Engine — pentade greca cognitiva** (ADR 0161, 25/5/2026): sostituisce il PLANNER step-by-step legacy con 5 moduli `runtime/{praxis,praxis_propose,praxis_executor,pronoia,aporia}.py`. **Mētis** (Μῆτις, `praxis_propose.py` ~229 LOC) propone framework intero in 1-shot LLM wise tier ~15s con GBNF strict; **Noûs** (νοῦς, `praxis_executor.py` ~430 LOC) esegue deterministico no LLM nel loop (resolve from_step/${FILLER}/${stepN.field}, invoke_executor, vaglio.judge); **Praxis** (πρᾶξις, `praxis.py` ~560 LOC + sqlite `~/.local/share/metnos/praxis.sqlite` 3 tabelle skills/anti_skills/observations) cache O(1) hash intent_sig + auto-promote 2-3 ✓ + anti_skill TTL 30gg; **Pronoia** (Πρόνοια, `pronoia.py` ~250 LOC) recovery 3 classi ortogonali (wrong_tool/wrong_args/missing_input) + re-propose con exclude failed_tool, 1× per turno; **Aporia** (ἀπορία, `aporia.py` ~210 LOC + `aporiae.sqlite`) vicolo cieco onesto: classify root_cause (user_action_required/missing_executor/missing_skill/missing_data) + suggested_action template + log lacuna evolutiva. Cascata: fast_path L0 → intent_extractor → Praxis.try_match O(1) → Mētis 1-shot → Noûs execute → if error recoverable Pronoia → if out_of_scope Aporia. Bench 35q reali: coverage 33/35 (94%), mean latency 12.5s (vs PLANNER step-by-step 76.3s → 6× speedup), Gemma 4 26B locale, costo 0 (vs Opus $$), -55% codebase agente. Feedback chat ✓✗↻ → record_feedback hook in `turn_feedback.apply_feedback`. Deprecations marker `# DEPRECATED-PRAXIS`: `planner_split` (ADR 0151), `tool_grammar`, `prompts/{it,en}/planner/_core.j2` step-aware, `multi_tool_paths` (ADR 0150), `agent_runtime.run_turn` step loop — rimozione post-MVP convergence ≥90%. Prompts: `prompts/{it,en}/praxis_propose.j2` + `pronoia_recovery.j2`. Wire-in `agent_runtime.run_turn` pre-PLANNER cascade. Filosofia §7.9: deterministico ovunque tranne Mētis propose + filler fast tier.
