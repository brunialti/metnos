...
- **22 oggetti**: `files, dirs, packages, messages, events, calendars, contacts, places, processes, urls, numbers, images, signatures, texts, proposals, persons, tasks, inputs, credentials, issues, pulls, entries`. Eccezioni: `get_inputs`={values} UI (ADR 0090); `*_credentials` metadata-only (ADR 0123); `entries` meta-oggetto in-memory; `persons` vs `contacts` (ADR 0113/0137); `tasks` (scheduler v2) vs `events` (ADR 0112/0137); `calendars` vs `events` (provider google_workspace: create/delete_calendars ADR 0123/0136); `issues`/`pulls` provider github (ADR 0141).
