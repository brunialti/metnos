...
> Ultimo aggiornamento: 2026-05-15 sera (ADR 0138 filter_lists set ops bi-list + tassonomia operatori liste / ADR 0139 LLM query expansion language-sensitive per find_images_indices [supera corpus BGE-M3 expansion su query brevi mono-token]. filter_entries operatori stringa where_starts_with/contains/glob/regex + tolleranza wildcard where_in (§2.4 convention array-of-string aperto/chiuso). time_window_parser anno ISO `2026`, anno-mese `2026-05`, `next-Nm/Nh/Ny`. find_images_indices: text_score_min default 0.40 AND stretto, names plurale multi-persona AND validation, date extraction da SOLO basename (no organizzazione cartelle), top_k guard <50 → 100, `_index_dir` path LOGICAL no .resolve() per symlink NAS. detail_md autoritativo nel final_message (no wrap «{tool}: completato»). read_tasks_history arg time_window + summary aggregation. _ALT_TARGET_KEYS esteso (event_ids/event_id/entries/to/to_user). Gallery viewer caption :hover + title tooltip. Rule planner photos.persons_and_multi + calendar.events_overlap_intersect + scheduled_tasks.show_task_history. Caption VLM solo nel gallery viewer, no testo final).
> Ultimo aggiornamento precedente: 2026-05-15 mattina/pomeriggio (ADR 0134-0137: semantic affinity fallback BGE-M3 ibrido / grammar pool extensions [final_answer synthetic + parser truncated recovery + from_step gating al primo step] / skill dormancy detection + provider 4ª famiglia qualifier §2.2 / vocab persons+tasks OBJECTS 17→19. Battery e2e 22/22 PASS. Rename builtin scheduled canonical: schedule_recurring→create_tasks, list_scheduled_tasks→list_tasks, delete_tasks_scheduled→delete_tasks, show_scheduled_task→read_tasks, toggle_scheduled_task→set_tasks(fire_now), scheduled_task_history→read_tasks_history. Bonifica affinity 21 manifest. Mail delete pipeline (move_messages a Trash). Dedup fail early-exit. Cap_expand prepend skip post final_answer synthetic).
...
ADR registry canonico: `/opt/myclaw/decisions/` (`0001-0139`, `0055`/`0115`/`0116`/`0121` skipped — fonte unica per "perche' abbiamo scelto cosi'").
...
- **Qualifier opzionali, 4 famiglie**:
  ...
  - **provider** (15/5/2026): backend specifico non-default. `_google_workspace` (skill gmail/drive/calendar ADR 0123), `_metnos` (default locale implicito, di solito omesso). Estensibile a futuri `_outlook, _slack, _telegram` per skill esterne. Lookup `tool_grammar._PROVIDER_SUFFIX_MARKERS` filtra dal pool grammar se la query non contiene marker (`google/drive/gmail/workspace` per google_workspace). Loader marca `dormant=True` se `[provenance]` referenza una skill senza credenziali (vedi `skill_credentials.py`).
...
- **Semantic affinity fallback ibrido** (ADR 0134, 15/5/2026): `runtime/affinity_semantic.py` (~190 LOC) BGE-M3 ONNX int8 1024d cache su disco `~/.cache/metnos/affinity_emb/<sha16>.npz` (matrix + reverse index, invalidation hash su `(name, tag)` ordinati). Fast-path: hard match resta primario (1-2ms); semantic fallback solo se `top_score < METNOS_SEMANTIC_THRESHOLD` (default 8 bench-tunato su 301 query reali). Latency +25ms su 14% query. Re-rank `score = hard + alpha * max_cosine` (alpha default 4.0). Lazy singleton embedder, degrade silente. Env opt-out `METNOS_SEMANTIC_MATCH=0`. Bug fix collaterale: rimosso `"del": "delete"` da `_VERB_TO_CANONICAL` (preposizione articolata IT).
- **Grammar pool extensions** (ADR 0135, 15/5/2026): (a) `final_answer` synthetic tool nel pool GBNF — `pairFinalAnswer ::= "\"final_answer\"" sep "\"arguments\"" colon ("{" ws "\"message\"" colon jsonStr ws "}")`, abilitato da step>=2; runtime intercetta `chosen_name=="final_answer"` → close turn. Risolve regressione "final stupido post-grammar" (LLM forzato a tool_call non poteva emettere text). (b) `_parse_tool_call_tolerant` recovery regex per JSON truncated da llama.cpp EOS premature: `"name"\s*:\s*"([id])"` + best-effort args. General-purpose §7.3. (c) `_FROM_STEP_HELPERS = {filter/sort/compute/classify/group/describe_entries}` esclusi dal pool al primo step §4.2 (richiedono `from_step` su lista preesistente). `undo_last_turn` resta. Battery e2e 6/9→22/22 PASS.
- **Skill dormancy + provider qualifier 4ª famiglia** (ADR 0136, 15/5/2026): §2.2 esteso a 4 famiglie qualifier (+`provider`: `_google_workspace, _metnos, ...`). Lookup `tool_grammar._PROVIDER_SUFFIX_MARKERS` filtra dal pool grammar se query non contiene marker (`google/drive/gmail`). `runtime/skill_credentials.py` mappa skill→check function deterministico §7.9 (oggi: `google-workspace` verifica `google_token.json` + refresh_token). `Executor.dormant` calcolato al load. `prefilter._filter_dormant` skip dal pool top-K. Fail-open su exception. 14 unit test.
- **Vocab extension persons+tasks** (ADR 0137, 15/5/2026): OBJECTS 17→19. `persons` (registro nominale ADR 0113, distinto da `contacts`) + `tasks` (scheduler v2 ADR 0112, distinto da `events`). _OBJECT_TO_SECTIONS/_OBJECT_PRIMARY_TOOLS/OBJECT_DEFAULT_MUTATING_VERB/synonyms IT+EN aggiornati. _OPT_IN_SECTIONS svuotato (regressione grammar risolta). Rename builtin scheduler v2 §2.2: `schedule_recurring→create_tasks, list_scheduled_tasks→list_tasks, delete_tasks_scheduled→delete_tasks, show_scheduled_task→read_tasks, toggle_scheduled_task→set_tasks` (con `fire_now=true` per accorpare ex `run_scheduled_task_now`), `scheduled_task_history→read_tasks_history`.
- **Mail delete pipeline** (15/5/2026): nuovo rule `delete_mail_is_move_to_trash` in `mail.yaml` IT+EN — in Metnos `delete_messages` non esiste, cancellazione mail = pipeline `read_messages→classify_entries→filter_entries→move_messages(dst_folder="Trash")`. `_OBJECT_PRIMARY_TOOLS["messages"]` esteso a 4 entries (read/send/move/find).
- **Dedup fail early-exit** (15/5/2026): `agent_runtime` nuovo branch dopo duplicate_call detection: se `last_obs.ok=False` con error chiaro al primo step, chiudi turno con `auto_final_on_duplicate_fail` e error originale come `final_message`. Pattern §7.3: copre tutti i mutating tool con fail autorevole (target_not_found, missing_credentials). Risolve loop di `delete_tasks` su task inesistente.
- **Bonifica affinity catalogo** (15/5/2026): script `bonify_affinity.py` rimuove plurali IT regolari (-i→-o, -menti→-mento, -zioni→-zione) + plurali EN (-s/-es/-ies), jargon interno (`fs/blacklist/whitelist/graylist`), compound multi-root ridondanti (heads `stato/salute/situazione/carico`). 21 manifest bonificati e re-firmati. IT+EN paralleli mantenuti. Get/set_signatures arricchiti manualmente con voci user-facing.
- **filter_lists set ops bi-list + tassonomia operatori liste** (ADR 0138, 15/5/2026): nuovo executor `filter_lists` (~226 LOC, 13 test) per intersect/union/difference/symdiff/overlap fra 2 liste. Args canonici `op` + `on_keys` + `with_step` (parallelo a `from_step`). `op="overlap"` AND temporale auto-detect start/end/taken_at_iso/mtime. Wire `agent_runtime._resolve_from_step` Layer 5: `with_step=N` espande in `entries_b`. Tassonomia chiusa: `filter_entries` (1 lista, predicati) / `filter_lists` (2 liste, set ops) / `compute_entries` (1 lista, scalari aggregati) / `compute_lists` (2 liste, scalari — riservato futuro). `_FROM_STEP_HELPERS` esteso a 7 entries. Test 4 «HLT overlap MNM nei prossimi 3 mesi» PASS in 6 step.
- **filter_entries operatori stringa + tolleranza wildcard** (15/5/2026, commit `ef0c776`): 4 nuovi operatori case-insensitive su `where_field` arbitrario — `where_starts_with`, `where_contains`, `where_glob`, `where_regex`. `where_in`/`where_not_in` tollerano wildcard `*`/`?` via `fnmatch.fnmatchcase` (backward compatible: valori senza wildcard restano match esatto). §2.4 convention args `array of string` aperto (testo libero, fnmatch tolerance) vs chiuso (enum/slug/id, match esatto stretto). Risolve bug HLT (LLM passava `where_in=["HLT*"]` su valori opachi).
- **time_window_parser estensioni ISO + multi-unit** (15/5/2026, commit `ef0c776`+`a51e72d`): anno solo `2026`, anno-mese `2026-05`. Pattern `last-Nh/Nm/Ny` e `next-Nh/Nm/Ny` (hours/months ~30gg/years ~365gg). Risolve loop_break su query «appuntamenti 2026» / «prossimi 3 mesi».
- **LLM query expansion language-sensitive find_images_indices** (ADR 0139, 15/5/2026): `_expand_query_via_llm` Gemma 4 26B middle locale + cache disk indefinita `~/.cache/metnos/query_expansion_llm/<sha256(q)>.json`. Prompt in INGLESE + 6 few-shot IT/EN/FR per language fidelity. Output 8 sinonimi specifici stesso linguaggio. SUPERSEDES corpus-token BGE-M3 expansion su query brevi mono-token (prefix-bias `mare→amare/mappe/morte`). Pipeline `_filter_unified`: LLM expansion (1) → corpus BGE-M3 (2) → hybrid BM25+cosine (3). Live verificato: «mare» → 12 foto precision ~100% (vs 5 / vs 117 falsi positivi).
- **find_images_indices AND stretto + plural names + filename-only dates** (15/5/2026, commits `211dabb`+`a4302b6`+`33d2ecd`+`a3b04a8`): (a) `text_score_min` default 0.40 = AND stretto sul contributo testuale isolato, no piu' ranking-boost. (b) `names: list[str]` plurale validato come search criterion (era ignorato → "missing search criterion"). (c) Date extraction da SOLO `os.path.basename` (no organizzazione cartelle utente-specifica), regex `_PATH_DATE_FULL_COMPACT` per `YYYYMMDD` senza separatori. (d) Truncated flag corretto multi_dirs (era `len(all_entries) > top_k` post-truncation → sempre False). (e) `top_k` guard: valore <50 forza default 100 (protezione LLM-mistake). (f) Manifest description avverte di NON passare `top_k` salvo richiesta esplicita utente.
- **image-index path LOGICAL no .resolve()** (15/5/2026, commit `080d9c6`): tutti i 5 moduli image-index (find/create/get/delete + build_runner_unified + build_runner) usano path LOGICAL per il digest sha8 del corpus. `.resolve()` rimosso. Identita' di un corpus = posizione logica utente, NON storage fisico sottostante. Compatibilita' con symlink `~/.local/share/metnos/Immagini → /tmp/nas_public/media/Immagini` (NAS CIFS): indice resta valido anche se il symlink viene riallocato. Pattern §7.3 general-purpose per qualsiasi modulo che indicizza corpus per digest del path.
- **detail_md autoritativo nel final_message** (15/5/2026, commit `a51e72d`): `agent_runtime.auto_final_on_duplicate` controlla `last_obs.detail_md` PRIMA del wrap «{tool}: completato (N elementi)». Se presente non-vuoto, lo usa come `final_message` PURO (cap 1500 char). Pattern: executor producer ricchi (`read_tasks_history`, `lifecycle_summary`) decidono il proprio output display-ready senza prefisso ridondante.
- **read_tasks_history time_window + summary** (15/5/2026, commit `10ef198`): nuovo arg `time_window` (canonical §2.1: `last-7d`, `today`, ISO `2026-05`, ...). Summary aggregation: `_aggregate_summary()` calcola totale fire + esiti (success/fail/timeout) + per-task counts → `detail_md` markdown ricco compatibile con autoritativita' detail_md.
- **_ALT_TARGET_KEYS esteso** (15/5/2026, commit `b9b6477`): `event_ids, event_id, entries, to, to_user` aggiunti al set di alt-target keys che permettono di silenziare `from_step` superfluo quando un consumer riceve target espliciti inline. Pattern §7.3: copre `delete_events` ID-in-text, `send_messages` recipient esplicito, qualsiasi pipeline con entries inline.
- **Dedup fail early-exit** (15/5/2026, commit `d243738`): `agent_runtime` branch dopo duplicate_call detection — se `last_obs.ok=False` con error chiaro, chiudi turno con `auto_final_on_duplicate_fail` e error come `final_message`. Pattern §7.3 copre tutti i mutating tool con fail autorevole. Risolve loop di `delete_tasks` su target inesistente.
- **Cap_expand prepend skip post final_answer** (15/5/2026, commit `d243738`): `TurnLog.write` non prepende notice di truncation se `self.steps[-1].chosen_tool == "final_answer"` synthetic (ADR 0133+0135). Il LLM ha gia' visto `truncated/available_total` in observation e ha formulato un final consapevole.
- **Mail delete pipeline** (15/5/2026, commit `d243738`): nuovo rule `delete_mail_is_move_to_trash` in `mail.yaml` IT+EN — in Metnos `delete_messages` non esiste, cancellazione mail = pipeline `read_messages → classify_entries → filter_entries → move_messages(dst_folder="Trash")`. `_OBJECT_PRIMARY_TOOLS["messages"]` esteso a 4 entries (read/send/move/find).
- **Gallery viewer caption overlay** (15/5/2026, commit `33d2ecd`): `gallery.html` mostra description VLM al :hover sulla thumb (overlay span + `title` tooltip browser). Caption gia' propagata da `find_images_indices.attachments`. CSS only, niente JS, no restart. Caption rimossa dal final_message testuale (commit `a3b04a8`).
- **Rule planner nuove** (15/5/2026): `photos.yaml::persons_and_multi` (query multi-persona AND: `names=[a,b]` not `name="A, B"`); `calendar.yaml::events_overlap_intersect` (pipeline canonical 5-step filter+filter_lists overlap); `scheduled_tasks.yaml::show_task_history` (must_not list_tasks prima di read_tasks_history senza name esplicito).
...
- ADR registry: `/opt/myclaw/decisions/` (`0001-0139`, `0055`/`0115`/`0116`/`0121` skipped) — dettagli implementativi e razionale.
