...
- **4° livello descriptor** (ADR 0156, 21/5/2026 v2): schema POSIZIONALE `verb_object[_qualifier[_descriptor]]`, separatore unico `_`. Descriptor opzionale kebab-case interno `[a-z0-9]+(-[a-z0-9]+)*` max 30 char. **Regola d'oro: il 4° livello ESTENDE, non RIMPIAZZA il 3°** — il descriptor puo' apparire SOLO se il qualifier e' presente. Se serve estendere un nome a 2 livelli, le opzioni sono: (a) usare qualifier esistente, (b) proporre nuovo qualifier nel vocab §2.2, (c) lasciare nome 2-livello + contesto in `proposed_action`. Esempi validi: `compute_files_loc_per-language`, `find_dirs_empty_recursive`, `change_files_format_dry-run`. Enforce via `runtime/naming_grammar.py::validate_name` (deterministico §7.9) + GBNF generator. Riusato da: telos lenses, introvertiva, synt stage 1, skill importer mapping.
...
- **Telos engine — 9 lenti laterali** (ADR 0156, 21/5/2026): `runtime/telos_lenses/` pacchetto. Lenti: scamper, oulipo, inverse_rl, endgame_book, analogy_transfer, boden_transformational, compression, pattern_language, generative_design. Framework comune `_base.run_lens(lens_name, operators, build_prompt, ctx, llm_invoke, grammar)`. Env toggle per-lens `METNOS_TELOS_LENS_<NAME>=1`. Telemetria `~/.local/share/metnos/telos_proposals.jsonl`. LLM default Gemma 4 26B locale (vincolo §3 telos.html, background zero-cost).
