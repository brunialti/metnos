...
> Ultimo aggiornamento: 2026-05-11 (sessione 11 maggio sera: §6.1 nuovo (tipizzazione prompt) + §10.6.45 — roadmap prompt architecture A+B+C: hook anti-drift IT/EN, daemon `i18n_translate_pending` daily@02:00, consolidamento Z.N → 7 nomi semantici, frontmatter 8 campi su tutti i `.j2`, split `planner.j2` in `_core` + 6 sezioni + `_footer`, `prompt_loader.compose()` LRU 3-layer, linter `runtime/prompts_lint.py` 5 check + CLI `metnos-prompts lint --strict` + smoke integration. Legacy `planner.j2` eliminato §7.1.).
> Ultimo aggiornamento precedente: 2026-05-10 (§10.6.44 ADR 0123 skill importer agentskills.io; §10.6.43 ADR 0122 proposal auto-evaluator + ETA; 16° OBJECT `credentials` §2.2; 5° reverse_pattern `delete_<object>_by_id` §2.3).
...
- **16 oggetti** (plurale): `files, dirs, packages, messages, events, contacts, places, processes, urls, numbers, images, signatures, texts, proposals, inputs, credentials`. Eccezioni: `get_inputs` ritorna `{values:{var:value}}` (UI dichiarativa, ADR 0090); `find_credentials`/`set_credentials`/`delete_credentials` espongono SOLO metadata (binding, fingerprint, scopes, age, status) — i valori cleartext non tornano mai al PLANNER (ADR 0123).
