...
- **ClusterLLM + Pronoia classify_fail + retry on repeat** (ADR 0162, 26/5/2026): estende Praxis Engine (ADR 0161) con 3 capacita'. **(1) ClusterLLM**: `runtime/praxis_cluster.py` sostituisce `intent_hash` lessicale con `cluster_id` emergente via BGE-M3 embedding + cosine + LLM judge zona grigia. Soglie `COSINE_HIGH=0.90`/`COSINE_LOW=0.75` (centralizzate in `runtime/praxis_constants.py`). Query semantically equivalenti (lingue diverse / sinonimi) condividono cluster → promote dopo 2 ✓ totali (vs mai prima per bucket frammentati). Schema delta: `observations.embedding BLOB`, `observations.cluster_id`, `skills.cluster_id/framework_hash/latency_p50_ms/composite_score/champion/template_issue`. **(2) Champion/challenger darwiniano**: piu' framework coesistono per cluster, `composite_score = 0.5·succ + 0.3·speed + 0.2·alignment`, swap se `swap_score = 2·Δsucc + Δlat_norm > 0.15` (`SWAP_THR`). Selezione pure-exploit (no exploration random). **(3) Pronoia classify_fail**: `runtime/pronoia_classify_fail.py` su ✗ utente, LLM Gemma 26B classifica in {format_fail/args_fail/pipeline_fail} (~1.5-2s) → dispatch differenziato: `format_fail`→template_issue=1 + Mētis refresh inline (~3s), `args_fail`→DELETE filler_cache, `pipeline_fail`→fail_count++ + anti_skill TTL 30gg. **(4) Retry on repeat**: ↻ scatena Mētis re-propose framework alternativo INLINE (~3-15s wise tier), salva su skill esistente, soft anti_skill TTL 1h `reason=repeat_soft` sul vecchio fw_hash. No re-execute (evita side-effects mutating). **(5) Counters separati**: `_touch_skill` aggiorna SOLO `ts_last_used`; `update_skill_metrics(success: bool)` gestisce uses/ok_count/fail_count. **(6) Audit trail**: tabella `skill_versions(skill_id, ts, event, old_fw_hash, new_fw_hash, reason)`. **(7) Batch nightly safety net**: `runtime/jobs/praxis_template_refresh.py` registrato scheduler v2 daily@04:00, rigenera skill con `template_issue=1` non gestite inline. **(8) Renderer magic `${stepN.@count}`**: cascata deterministica `available_total → ok_count → used → len(first list[dict])`. Test: 29/29 verdi (`test_praxis_cluster.py` + `test_pronoia_classify_fail.py`). Env tunable: `METNOS_CLUSTER_COSINE_HIGH/LOW`, `METNOS_CLUSTER_SWAP_THR`, `METNOS_PRAXIS_W_SUCCESS/SPEED/ALIGNMENT`, `METNOS_PRONOIA_CLASSIFY_FAIL`. HTTP feedback whitelist: action ∈ {ok|error|repeat}.
