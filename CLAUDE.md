...
> **OBBLIGO a inizio sessione**: leggere integralmente QUESTO file e la parte mutabile @CLAUDE.mutabile.md (stato corrente del sistema + decisioni di runtime).
>
> **GOVERNANCE DEI DUE FILE** (fissata da Roberto, 7/7/2026)
> - **Questo file è INVARIANTE**: modalità definite da Roberto e regole generali di approccio al progetto. Lo modifica SOLO Roberto (o l'agente su sua istruzione esplicita e puntuale). MAI di iniziativa. Se una regola qui sembra obsoleta o contraddetta dal codice: NON modificarla — segnalarlo a Roberto con una proposta.
> - **`CLAUDE.mutabile.md` è MUTABILE**: lo manutiene l'agente secondo i criteri dichiarati in testa a quel file.
> - La numerazione `§` è CANONICA e STABILE (citata in codice/ADR/memorie): mai rinumerare. I §3-§5, §11-§12, §14 vivono nella parte mutabile. Storia: `git log CLAUDE.md` / `git log CLAUDE.mutabile.md`.
...
Struttura: `azione_oggetto[_qualifier]`. Vocabolario CHIUSO (escalation a Roberto per nuovi termini), centralizzato in `runtime/vocab.py` (SoT delle liste correnti di azioni/oggetti).
...
- **Qualifier opzionali, 4 famiglie**: **formato** (`_csv, _xlsx, _ocr, _zip, _pdf, _xml, _html, _json, _text, _gz, _tar, _video, _audio, _image, _hash`); **modalita'** (op `_size/_format/_similar/_loc/_empty`; granularita' `_lines/_paragraphs/_sentences/_pages/_segments`; persistente `_indices`; `_empty` ADR 0127 cross-domain con arg `size` unit-aware); **safety policy** (`signatures`, ADR 0071: `_blacklist, _whitelist, _graylist, _forbidden, _seed, _diff, _sanity, _command, _reversibility, _promotion, _candidates`); **provider** (ADR 0136: backend non-default, es. `_google_workspace`; `_metnos` default omesso; filtro pool via `tool_grammar._PROVIDER_SUFFIX_MARKERS`; dormant se no creds).
...
**Convention args `array of string`**: decidere a design-time il dominio. **Aperto** (testo libero) → tolleranza wildcard via `fnmatch`; manifest dichiara "valori con `*`/`?` = glob". **Chiuso** (enum/slug/ID/scope/email) → MATCH ESATTO stretto, dichiarato nel manifest. Razionale: l'LLM ha bias verso glob universali; fnmatch su dominio aperto previene fallimento silenzioso, match esatto su chiuso impedisce abuso.
...
```
`SCOPO`+`PATTERN` front-loaded (il proposer li estrae fino a `OUT:`). Vale per ogni manifest NUOVO e ogni VECCHIO toccato per un fix; **NON** rifattorizzare in massa. Boundary verbo §2.2 nel capitolo `NON:`. Esempio canonico: `executors/write_files/manifest.toml`.
