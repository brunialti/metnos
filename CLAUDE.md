...
- **4° livello descriptor** (ADR 0156, 21/5/2026): opzionale, separatore `#`, kebab-case `[a-z0-9]+(-[a-z0-9]+)*` max 30 char. Esempi: `compute_files_loc#per-language`, `read_events#calendar-conflicts`. Aperto (free text) ma vocab-aware: NON puo' coincidere con un verbo o oggetto §2.2; NIENTE underscore (riservato canonical); no leading/trailing hyphen; no doppi hyphen. Fuori grammar canonical: enforce via `runtime/naming_grammar.py::validate_name` (deterministico §7.9) + GBNF generator per LLM constrained. Riusato da: telos lenses, introvertiva, synt stage 1, skill importer mapping.
