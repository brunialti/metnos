...
ADR registry canonico: `decisions/` (relative alla repo root; `0001-0187`, `0055`/`0115`/`0116`/`0121` skipped — fonte unica per "perche' abbiamo scelto cosi'").
...
- **Preferenze utente esplicite** (6/7/2026, ADR 0187 W2-v1): tabella `user_prefs` (users.db) con vocabolario CHIUSO (lang/tone/reply_length/units), API `users.set/get/list/delete_pref`; gli executor leggono via `${RUNTIME:pref_<chiave>}` (assente ⇒ placeholder intatto §2.8); UI in `/admin/users/{id}`; `read_persons` le include nel profilo. Set-da-chat e consumo nel Finalizer = v2 (decisione vocab §2.2 aperta).
- **Manutenzione domini esterni = comandi schedulati NL** (5/7/2026, ADR 0186): la manutenzione di un dominio esterno (issue GitHub, mail, web-check) è una QUERY NL schedulata (`scheduler_v2` `run_user_query`) che compone executor via planner — mai job bespoke (`github_watcher` ritirato, formalizzato). Confine: dati/servizi utente→comando NL; organi interni (aging/reaper/i18n/index/promoter)→builtin registrato in `NIGHTLY_SEQUENCE`; osservatori esterni (audit Fable)→timer di sistema. Aging: esenzioni alla fonte (`executor_aging` PROTECTED_NAMES+handcrafted).
- **Learning-loop W1: crescere dall'esito dei turni reali** (5/7/2026, ADR 0185): (a) turno engine OK e COSTOSO (n_step≥4) ripetuto (≥2 osservazioni stesso intent+framework) → autopath **shadow** (`autopath.seed_from_run` dal dispatch; il primo ✓ umano lo conferma champion); (b) lacuna terminator ricorrente (n_seen≥3, classi capability) → change_intent `create_executor` PROPOSED (hook nel choke-point `_record_lacuna`; triage umano su /admin/changes, admission 6-layer al materialize; REJECTED non risorge per costruzione); (c) `learning_loop_review` notturno pota i seed mai confermati (TTL 21gg). Soglie via env; niente ML, niente estensione vocab autonoma.
- **Undo: scrittore al choke-point + reverse device-aware** (5/7/2026, ADR 0183): il log undo è scritto da `invoke_executor` (`_undo_pending` pre-exec + `_undo_done` post-ok, campo `device`) — copre engine/device/futuri per costruzione (la regressione af6c7b8 aveva lasciato l'undo senza scrittore). Op eseguita su un device → il reverse è tradotto in CHIAMATE EXECUTOR (`reverse_patterns.build_remote_reverse_calls`) e ACCODATO ALLO STESSO device da `undo_last_turn` (§2.9, attesa bounded, failed=ritentabile); `restore_blob_backup` non remotabile ⇒ `delete_files` senza device_ok (con target device gira locale). Mutanti device-abili: write/move (+read-only find/read/list); lazy-gw su TUTTI i dispatcher files/dirs. **Self-update automatico del client** (ADR 0184, ≥0.2.11): al poll version-mismatch → descrittore FIRMATO `/agent/client/update/{target}` verificato con la pubkey pinnata → idempotenza per sha dell'exe → swap atomico (.old=rollback) → respawn; client ≤0.2.10 richiedono UN ultimo upgrade manuale (join flow).
- **Cache-validity: firme del mondo sulle decisioni cachate** (5/7/2026, ADR 0182): ogni piano cachato (L0 `fastpaths`, L1 `autopaths`/`observations`, alternative-cache LRU del proposer) porta `tools_sig` (digest §7.10 dei tool referenziati) + `pool_sig` (famiglie di candidati dell'intent); VERIFICA A LETTURA in `dispatch` (`engine/cache_validity.py`): mismatch/sig-vuota → MISS (L0 morte+re-record dal successo; L1 fall-through, refresh alla ri-promozione; proposer `catalog_epoch` nella chiave LRU). Un executor ri-firmato o una capacità nuova nella famiglia INVALIDANO i piani interessati per costruzione — niente più purghe a mano (chiude il TODO CRUCIALE negative-path-cache). Firma SEMPRE col catalogo del chiamante (mai load_catalog implicito: side-effect aging).
...
- ADR registry: `decisions/` (`0001-0187`, `0055`/`0115`/`0116`/`0121` skipped) — dettagli implementativi e razionale.
