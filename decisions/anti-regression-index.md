...
- **Ricerca vuota: un solo recupero pertinente** (18/9/2026, bug live `7a8ca31b`): `search_no_results` passa al proposer un segnale strutturato senza snippet/URL; `SimpleRecovery` e `MetisRecovery` mantengono `find_urls` disponibile e chiedono una sola query materialmente diversa, conservando luogo, periodo e altri vincoli. `executor.compute_execution_fingerprint` confronta tool e valori effettivi dopo le riscritture: blocca il replay identico anche se cambia il testo finale, ma ammette una vera riformulazione con la stessa shape di cache. Dopo il secondo vuoto il terminatore chiude onestamente. Guard: `test_search_no_results_recovery.py`, `test_terminator_operational_blame.py`.
- **Fine tecnica distinta dall'esito funzionale** (18/9/2026, bug live `7a8ca31b`): `TurnLog.outcome` persiste `completed|partial|failed|awaiting_input` da `reliability.classify_turn`; `final_kind=answer` descrive soltanto il protocollo e non cancella un executor fallito senza effetti. HTTP, storico, contatori admin e chat espongono l'esito semantico, mentre `state=complete` resta lo stato di lifecycle. Guard: `test_reliability.py`, `test_turn_outcome.py`.
...
- **Annullamento Birth fedele al contratto e allo stato preesistente** (ADR 0224, 16/9/2026): `executor_birth_property_runner` seleziona risorse private da capacità, schema e inversa dichiarata; esegue l'inversa centrale dei percorsi e osserva le sessioni senza rete esterna. Mutanti su ricevute, directory, sessioni preesistenti, chiusura globale e inversa discordante devono fallire. Prove native delegate esplicite: `test_executor_birth_property_runner.py::test_native_created_paths_and_sessions`.
- **Pallino LRE distinto dalla salute del processo** (ADR 0213, 16/9/2026): `services.html` mostra grigio per disabilitazione confermata e verde solo per funzione abilitata e pronta; incertezza e discordanza non diventano verde. Salute canonica e supervisione restano invariate. Matrice IT/EN: `test_services_registry.py::test_lre_indicator_requires_verified_functional_readiness`.
- **Tempi LRE senza falsa precisione** (ADR 0213, 16/9/2026): `progress_many` usa l'avvio effettivo persistito, percentuale sulle unità note e stima prudente a fase unica; non anticipa il 100% e non usa la coda come esecuzione. UI spegne la stima su motore non pronto o dati vecchi e mostra `n.a.` via i18n. Prove: `test_progress.py`, aggregazione owner-scoped in `test_control.py`, browser eseguito e seed IT/EN in `test_durable_console_behavior.py`.
- **Prerequisito LRE sulla collocazione dei contratti** (ADR 0093, 16/9/2026): `admit_prerequisite` conserva il server solo se entrambi i contratti verificati sono server-only; dispositivi, proprietà e controlli restano invariati. Matrice generica in `test_executor_prerequisites.py`, ammissione reale isolata e deduplicazione in `test_image_indexing_boundaries.py`.
- **Contesa LRE distinta dal guasto** (ADR 0213, 16/9/2026): `DurableWorkerService` riconosce BUSY/LOCKED nativi, attende entro limiti senza interrompere subito corsie vive e conta cicli, non eccezioni simultanee. Prove: `test_service_contention.py`; stress isolato opzionale `test_contention_scale.py`.
- **Statistiche d'uso non sono disponibilità delle capacità** (ADR 0099/0213, 16/9/2026): `loader` firma e applica lo stesso stato semantico di ciclo di vita, senza invalidazione sui conteggi né lettura finale scollegata. WAL, modifica concorrente, rimozione override e fallimento chiuso: `test_loader_lifecycle_signature.py`; causa caricatore chiusa: `test_execution_bridge.py`.
- **Console LRE identifica il lavoro senza esporre il piano** (ADR 0213, 16/9/2026): descrizione, cartella e fase dalla revisione corrente ammessa del proprietario; blocchi in esecuzione distinti da limite/thread, scoperta e stima mancanti spiegate. Prove: `test_description.py`, `test_progress.py`, API e browser IT/EN in `test_durable_console_behavior.py`.
- **Stima della fase LRE distinta dalla fine del lavoro** (ADR 0213, 16/9/2026): almeno tre salvataggi della stessa unica fase attiva e interamente materializzata, inventario chiuso, niente retry/uso incerto; ancora persistita e freschezza legata alla cadenza entro timeout/30 minuti. Nessuna somma tra fasi o previsione spostata dal polling; `needs_attention` prioritario. ETA globale preservata separatamente. Prove: `test_progress.py`, Node e browser IT/EN in `test_durable_console_behavior.py`.
- **Descrizioni foto strutturate e diagnostica chiusa** (ADR 0093, 16/9/2026): schema di dominio vincola e verifica la risposta VLM; `length` fallisce dopo la contabilizzazione, senza maggiori limiti o ripetizioni. Errori executor conservati solo dall'enum dello schema approvato; inventario conserva cause/errno enumerati senza percorsi. Prove: `test_describe_images.py`, `test_image_index_build_phases.py`, `test_execution_bridge.py`, `test_manifest_inventory.py`.
- **ETA durante chiamate modello ancora vive** (ADR 0213, 16/9/2026): i consumi finali non ancora prodotti da una chiamata in corso non sono consumi sconosciuti. La stima di fase rifiuta comunque uso esplicitamente ignoto o tentativi terminali non contabilizzati; non modifica il gate contabile di completamento. Prova con `refresh_usage_complete=False` e chiamata attiva in `test_progress.py`.
- **Residui vuoti del catalogo: lettori condivisi, scrittori esclusi** (ADR 0223, 16/9/2026): la verifica conservativa di `writer.lock` usa `LOCK_SH`, senza ignorare contese con scrittori né alterazioni. Prove reali concorrenti, scrittore escluso durante lettura e rilasciato dopo, residui corrotti rifiutati: `test_contract_store.py`.
- **LRE, capacità congiunte e ripresa foto (2026-09-16)** — `storage.service_lane_demand` usa il collo di bottiglia, anche condiviso tra lavori, senza nascondere risorse indipendenti; `ImageIndexBuild.analysis_checkpoint` rivalida le foglie della stessa generazione. Decoder HEIC con prova reale e codici espliciti per file illeggibili. Test: `test_service_idle.py`, `test_image_index_build_phases.py`; ADR 0117.
- **LRE, letture coerenti e costo limitato (2026-09-16)** — `progress_many` riusa aggregazioni per fase senza scansioni tentativi per unità; materializzazione cerca stati nonterminali indicizzati; `control` legge una singola vista WAL. Prove VM senza statistiche e tutti gli stati: `test_progress.py`, `test_materialization_read_bounds.py`, `test_control.py`; ADR 0213.
- **LRE, scadenze e recupero equo (2026-09-16)** — clock commit dopo acquisizione writer; supervisore drena corsie scadute e riparte dopo riconciliazione; lavori invariati non monopolizzano il batch. Prove: `test_commit_deadline_after_contention.py`, `test_service_recovery_fairness.py`, `test_service_parallel_progress.py`; ADR 0213.
- **LRE, autorità e contratti senza prove implicite (2026-09-16)** — schemi annidati senza segreti, capacità complete congelate, autorità riletta dopo I/O, notifiche ritentate solo con mancata consegna esplicita, trigger canonici verificati. Prove: `test_security_boundaries.py`, `test_direct_invocation.py`, `test_admission_compiler.py`, `test_source_authority.py`, `test_outbox.py`, `test_schema.py`; ADR 0213.
- **LRE foto, integrità e letture senza blocchi (2026-09-16)** — cinque file sigillati e rivalidati in pubblicazione, file speciali negati prima di attese, hash sorgenti limitato e regex nomi senza backtracking esponenziale. Prove: `test_image_index_publication_integrity.py`, `test_image_index_filename_bounds.py`, `test_image_index_build_phases.py`; ADR 0117.
- **LRE, errore console non significa nessun lavoro (2026-09-16)** — primo caricamento fallito espone stato indisponibile; aggiornamento fallito conserva l'elenco ma lo segnala non aggiornato. Messaggi IT/EN dal catalogo; Node, API e Chromium isolato in `test_durable_console_behavior.py`, `test_durable_workloads_api.py`; ADR 0213.
- **LRE, esiti di dominio e storico finale (2026-09-16)** — ricevute approvate, confermate e legate al digest; `domain_errors.nitems` aggrega tutti i codici prima del limite visivo e resta dopo reopen, senza contare retry o fasi successive. `attempt_errors` conserva errori tecnici poi recuperati senza alterare il successo; letture circoscritte alla revisione e soli codici stabili. Copertura/consumi invariati. Prove: `test_domain_outcome.py`, `test_image_indexing_e2e.py`, Node/Chromium IT/EN in `test_durable_console_behavior.py`; ADR 0213.
- **Foto non indicizzabili senza falsi contenuti (2026-09-16)** — prefisso fisso fuori i18n, due soli errori di decodifica convertiti in record negativi senza modelli/vettori; pubblicazione e ricerche separano successi/errori, nuovo aggiornamento riprova i negativi, vecchi scrittori negati prima di modifiche. Prove: `test_image_index_negative_outcomes.py`, `test_image_negative_records.py`, `test_image_backfill_guard.py`; ADR 0117.
