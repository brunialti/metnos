...
- **Fastpath L0 lifecycle AUTO** (11/6, decisione Roberto — NIENTE bottone approvazione): auto-produzione `engine/fastpath.record_success` ← `dispatch._maybe_record_fastpath` (successo piano pieno engine/recovery, MAI hit L0/L1; esclusi undo_last_turn/get_inputs); pertinenza 0b solo `query_specific=0` (`engine/executor.is_query_specific`, condiviso L1); aging+morte `fastpath.prune` ← `task_state_reaper` (grazia 14gg / stale 30gg / cap LRU 500, env `METNOS_FASTPATH_*`; C1 tool∉catalog, C2 famiglia `{intent_verb}_{intent_object}[_*]` §2.2 non nel piano); C1 hit-time guard in `dispatch.run_turn` (delete + fall-through, self-healing); valvola `POST /admin/praxis/fastpaths/{id}/delete`.
