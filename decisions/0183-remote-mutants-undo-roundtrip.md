...
### D3 — delete_files resta SERVER-only (CP4)
`reverse_pattern=restore_blob_backup` non è remotabile ⇒ niente `device_ok`: con un target device, delete gira LOCALE (gate placement). Asimmetria voluta: la chat non cancella sul PC; l'undo sì, ma solo ciò che ha creato lui (reverse via enqueue diretto, che NON passa dal gate). Rimuovere il vincolo = progettare il blob round-trip (upload capped nei results o restore device-side) — follow-up esplicito, non implicito.
