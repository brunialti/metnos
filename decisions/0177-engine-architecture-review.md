...
✓ 11 guard (righe 257–1336) ri-derivano deterministicamente OGGETTO, VERBO, ORDINE e ARGS che il proposer LLM *dovrebbe* emettere. ⚠ Lettura: lo strato deterministico **compensa un proposer debole sotto MTP** invece di vincolarne la generazione. ~~Tre guard «produttore mancante» si sovrappongono~~ → **CHIARITO CP3 6/7 (S2)**: sono DUE guard di pipeline COMPLEMENTARI — `_enforce_missing_clauses` (verb-level, con `_align_foreign_producers_v3` come helper INTERNO) + `_enforce_missing_objects` (object-level, drop per-object che il verb-level non vede). NON ridondanti; interazione pinnata da `test_two_enforce_guards_compose_no_double_producer` (ogni oggetto = 1 produttore). Merge NON fatto (rischio sui compound a errore=0): il valore CP3 è il layering esplicito+testato. ⚠ La pipeline è **ordine-dipendente** (load-bearing: `_ensure_extract_clause` PRIMA di `_conform_to_intent_order`; `_fill_clause_args` DOPO ordine stabile) ma l'ordine non è documentato come contratto.

### S3 — Idempotenza dei guard sugli hit cache ✓ MISURATA E BLOCCATA (CP1·M0, 5/7)
~~asserita, non provata ⚠~~ → **CHIUSO**: (1) sweep su corpus REALE (L0 fastpaths + L1 autopaths + flagship compound, 33 piani) = **33/33 idempotenti** full-chain E per-guard; (2) test durevole `tests/runtime/engine/test_guard_pipeline_contract.py` (T4: corpus incorporato, double-apply full-chain + per-guard). Il T4 ha SCOVATO e fatto fixare **2 non-idempotenze reali di seconda passata** che lo sweep non copriva: (a) flag booleano spurio `recursive=True` al 2° giro (falso-positivo prefisso-4 «delle»~«della» in `_bool_flag_triggered` — parole-funzione IT aggiunte al noise); (b) la clausola delete-mail, riscritta da `_route_mail_delete_to_trash` in `move_messages`, veniva ri-appesa da enforce al giro dopo — e `derive_tool_name` la risolveva a `delete_messages_github` (fallback suffissi che pescava una variante PROVIDER senza marker: ora escluse). L'ordine della catena è il **contratto dichiarativo `GUARD_PIPELINE`** (T3, vedi §4).
...
### S5 — Cinque percorsi divergenti producono il messaggio finale ✓ CHIUSO (CP2·M2, 5/7)
✓ Il `final_message` può nascere da: (1) terminator zero-result deterministico → `MSG_NO_RESULTS` i18n; (2) template del proposer (`_render_final_message`); (3) synth LLM (`_synthesize_final_from_steps`); (4) `describe_entries` builtin; (5) `output_policy` ranked/gallery. ⚠ Nessuna fonte unica → mutazione del messaggio sparsa, i18n non garantito (il template letterale del proposer NON passa da `MSG_*`), e il bug «final_message=1» (decomposer write-injection → result terso → synth rende il conteggio) è un sintomo di questa frammentazione. ~~⚠ Il blocco render-degenere→synth è duplicato in due punti~~ → **CHIUSO 5/7 (T5)**: `_finalize_answer_text` = fonte UNICA (render→bullets §2.7→zero-i18n→synth), i due gemelli eliminati (il fallback post-loop guadagna i bullets); test anti-gemelli. Precisazioni misurate: (4) `describe_entries` è uno STEP che alimenta il render, (5) `output_policy` è PRE-esecuzione (normalize_terminal) — non fonti parallele del testo. Il template del proposer resta testo generato nella lingua utente (non stringa runtime): l'i18n-garanzia copre le stringhe RUNTIME (tutte via MSG_*).
...
### S7.bis — AGGIORNAMENTO 4-5/7: legacy RIMOSSO; S1 e S7 CHIUSI ✓
Il 4/7 (Roberto «procedi senza approvazione») il blocco legacy è stato **cancellato fisicamente** (−3407 LOC, `af6c7b8`; upload e resume già migrati su engine v3 via `seed_state`, §5.bis/5.ter). `agent_runtime.py` 9615→6289 righe. Gate G3 incondizionato. Nessuna capacità di produzione persa (audit 4 agenti). → S7 chiuso; M1 completo.

### S2.bis — CONFERMA 4-5/7: la batteria guard È il planner compound ✓ (inventario aggiornato)
La sessione compound-spreadsheet (4-5/7, errore=0) ha AGGIUNTO 5 guard clause-scoped: `_decontaminate_reader_qualifier`, `_scope_sink_provider_to_clause`, extract STRUTTURALE in `_ensure_extract_clause`, FIX-5b writer-coverage in `_dropped_required_verbs`, `_clause_scoped_drive_term`. La caccia ai turni 8b675402→b5a53690 (5/7) ne ha aggiunti altri 2: `_overwrite_phantom_install_args` (arg install-root fantasma, ripara i piani cachati on-hit) e `_degenerate_find_to_list` (§2.2 find-senza-selettore=list). **Catena attuale: 14 guard**, ordine = contratto `GUARD_PIPELINE` (T3). Scoperte strutturali della caccia: (a) l'align DEMOLIVA il piano corretto cachato per l'asimmetria files↔dirs → introdotta l'equivalenza `_fs_equivalent` (align pass-1, foreign-producers v3, enforce_missing_objects, chunk-matching di fill); (b) `derive_tool_name` iterava un SET (fratello dipendente dall'hash-seed del processo: find_files vs get_files fra due restart — violazione §11) → ordine fisso; (c) esiste un **ulteriore layer di cache**: la alternative-cache del proposer Metis («retry serving cached alternative without LLM call») serve framework fra turni vicini — da includere nello scope cache-discipline di M4/D2; (d) i default appresi (`args_defaults`) sono un canale di avvelenamento arg (path install-root auto-rinforzante, 46 usi) → filtro cattura+iniezione in `args_resolver`.
...
1. **M0 — Sicurezza prima (cheap, alto valore)**: ✅ **FATTO 5/7 (CP1 mandato Fable)** — T4 (`test_guard_pipeline_contract.py`: idempotenza full-chain+per-guard su corpus incorporato; sweep live 33/33) + T3 (`GUARD_PIPELINE` dichiarativa: ordine=contratto con vincoli di posizione documentati + test di sequenza). Il T4 ha scovato 2 non-idempotenze reali (vedi S3) — il rischio silenzioso ERA reale, ora chiuso e bloccato nel tempo.
...
3. **M2 — Unifica sintesi finale**: ✅ **FATTO 5/7 (CP2 mandato Fable)** — T5: `executor._finalize_answer_text` = UNICA fonte del testo `answer` (render→bullets §2.7→zero-i18n→synth), i due blocchi gemelli divergenti eliminati (il fallback post-loop guadagna i bullets); `test_finalizer_unico.py` blocca strategia e anti-gemelli. `output_policy`=pre-esecuzione e `describe_entries`=step: NON fonti parallele (chiarito). Sana «final_message=1».
...
- **T3 estensione — ESEGUITO 2026-07-07 → registro `ArgTransform`.** La famiglia resolver deterministica pre-esecuzione (mail_account, from_contains, junk_mail, time_window, photo_fields, backend_arg, self_recipient, calendar) era una **catena cablata a mano** in `Executor.run` + `resolve_query_canonical_args`, PARALLELA e duplicante il pattern di `GUARD_PIPELINE` (rilevata durante il fix i18n di get_files, 7/7: il planner emetteva sinonimi NL non-enum e mancava un punto di registrazione ovvio → tentazione di hardcodare una mappa nell'executor). Unificata in `executor.ARG_TRANSFORM_PIPELINE` (registro tipizzato gemello di `Guard`: `name/module/func/scope/reads/writes/rationale/adr`) + driver unico `apply_arg_transforms(tool, args, query, *, scope, args_schema)`. Il campo `scope` (`query-det` | `exec-only`) rende STRUTTURALE il confine che prima era un commento in prosa: i query-det sono riapplicati al record L0 (`_maybe_record_fastpath`), gli exec-only no → un edit che sposta un resolver ctx-dipendente nel path riapplicabile non può più avvelenare L0 per costruzione. 8 entry migrate; 5 irregolari (from_step/fillers/placeholders/scope_args post-invoke/matrix — firme e cicli-di-vita non uniformi) restano cablate e documentate al call-site (§7.2: non forzare un contratto universale). Behavior-invariant (pytest full + 606 test mirati + E2E query-det/exec-only). Test `test_argtransform_pipeline.py`; spec `internal/design/spec_argtransform_registry.md`.
