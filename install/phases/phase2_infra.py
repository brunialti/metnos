...
def _write_tiers_toml(*, local_endpoint: str | None, local_model: str | None,
    ...
    ``local_endpoint`` set → every local role routes to that llama-server.
    ``local_endpoint`` None → every local role uses the configured cloud binding.
    """
    p = _tiers_toml_path()
    if p.exists():
    ...
    for tier in llm_manager.LOCAL_TIER_NAMES:
        ...
        if local_endpoint:
            ...
            lines.append('model    = "claude-opus-4-8"')
        lines.append("")
    lines += [
        "[frontier]",
...
def _configure_frontier(args: Any) -> dict[str, Any]:
    ...
    if ui.confirm("Use Anthropic for the frontier tier?", default=True):
        return {"frontier_provider": "anthropic", "frontier_model": "claude-opus-4-8"}
...
def _offer_optionals(args: Any) -> dict[str, Any]:
    ...
    for key, entry in sidecar.SIDECARS.items():
        ...
        explicitly_enabled = key in getattr(args, "enable", [])
        if not (args.yes and explicitly_enabled) and not ui.confirm(
                f"Install {label}?", default=False):
            out[key] = "skipped"
            continue
        if not entry["ready"]:
