...
genera dai template in `install/units/*.tmpl`, sostituendo i percorsi. Un solo
`metnos.target` possiede lo stack installato e ne governa prontezza, quarantena
e controllo periodico limitato. Le unit installate: `metnos-http.service`, `metnos-side-display.service`,
`metnos-playwright.service` (con `--enable playwright`),
`metnos-telegram-daemon.service` (se pairato) e
`metnos-i18n-translator.timer` (riempimento traduzioni i18n a ciclo). Il
display virtuale usa Xvfb su `:99`; se Xvfb manca, l'installer segnala il
prerequisito e non attiva il Side browser. Per rifarle a mano:
...
systemctl --user status metnos-side-display metnos-playwright
systemctl --user list-timers metnos-i18n-translator.timer
journalctl --user -u metnos-http -f

# Pannello amministrativo di stato; i componenti integrati sono in sola lettura
# http://127.0.0.1:8770/admin/services

# Per sopravvivere al logout
sudo loginctl enable-linger $USER
```

Su un nodo nuovo la fase 5 abilita il target integrato. Se rileva invece un
`metnos-http.service` system gia' attivo, installa le user unit ma non avvia il
target e non disabilita il servizio funzionante: la migrazione richiede
inventario, due cicli E2E e rollback verificato secondo
`systemd/README.md`. Questo evita sia il doppio listener sia un cutover privo di
evidenza.

Il pannello usa il catalogo unico in `runtime/services_registry.py`; i timer
restano nella pagina Timer. I componenti integrati non espongono azioni
parziali: il loro ciclo di vita passa da `runtime/stack_reconcile.py`. Le
installazioni standard hanno unit user-level e non richiedono privilegi. Se
un'installazione esistente mantiene unit autonome sul system bus, installare la
policy minima generata dallo stesso catalogo:

```bash
sudo python3 -m install.service_control_policy --user "$USER"
```

La policy consente solo `start`, `stop` e `restart` sulle unit registrate; non
consente wildcard, modifica delle unit o comandi systemd generici e non rende
azionabili separatamente i componenti posseduti da `metnos.target`.
