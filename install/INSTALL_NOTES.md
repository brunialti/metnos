...
- **Lazy** — the **Playwright JS-render sidecar** only: install it on first use
  ...
  When installed, it is paired with `metnos-side-display.service`, which runs
  Xvfb on `:99`; `xvfb` is therefore a mandatory system package for the Side
  browser surface. Missing Xvfb is an explicit unavailable state, never an
...
- Non-interactive contract: `--yes --enable <name>` installs that sidecar
  without prompting; sidecars not listed by `--enable` remain skipped.
- `playwright` uses the same registry and flag contract as the other sidecars;
  its installer also enables the graphical Side display when Xvfb is present.
- An explicitly enabled VLM acquires a pinned local `llama-server` binary when
  the text tiers point to an existing endpoint and no managed binary exists.
- VLM readiness requires a successful HTTP status from `/health`; a temporary
  `503 Loading model` is not ready and remains inside the bounded wait loop.
...
- Relocatable sandbox: bubblewrap must mount the daemon's canonical `runtime/`
  directory read-only. Relying on the historical `/opt` system bind makes a
  fresh install under `$HOME` load executor code but fail its shared imports
  (`messages`, `executor_helpers`). The bind is derived from `sandbox.__file__`,
  never from a fixed installation path.

## Integrated stack lifecycle (contract added 18/7/2026)

- Fresh installs enable one user-level `metnos.target`; component units declare
  `PartOf=metnos.target` and order before `metnos-stack-ready.service`.
- Readiness is composite: HTTP source contract, local/live catalog parity,
  Playwright health and exact fingerprint when installed, plus broker state. An
  open port alone is never ready.
- Existing optional units receive a narrow ownership drop-in. Phase 5 must not
  replace their locally tuned service bodies.
- The closed owned set includes `metnos-issues-sidecar.service`. Quarantine also
  stops both `metnos-i18n-translator.service` and its timer; the first watchdog
  check starts only after the readiness timeout has elapsed.
- If a system-level `metnos-http.service` is active, phase 5 renders the new
  units but MUST NOT enable the target, start a second listener or disable the
  rollback baseline. It records `migration_required=true` and probes the live
  HTTP service honestly. A failed or incomplete system-manager probe is treated
  as migration-required, never as permission to start a second listener.
- `runtime/stack_migration.py pilot --cycles 2` is the mandatory upgrade gate.
  Every cycle starts the user target, waits for composite readiness, completes
  a natural turn, proves quiescence and restores the system baseline.
- `cutover` runs as root with an explicit `--service-user` and accepts only
  readable, successful two-cycle evidence whose HTTP contract, effective units,
  control-plane sources, host and service-user fingerprints still match. Root
  user-manager actions run as that service user, and pilot/cutover share the
  same service-user lock. Any failure rolls back automatically.
- Reconcile holds an owned, no-follow process lock; systemd actions use a closed
  unit catalog; deploy signs/verifies only named canonical executors. Automatic
  recovery uses a persistent 3-failures/10-minutes circuit breaker opened for
  15 minutes.
- The Playwright sidecar is lazy, but when installed its server contract and
  fingerprint are inseparable from HTTP readiness. Never restart or publish
  only one side of that pair.
- The service panel is status-only for integrated components; partial component
  control is rejected before invoking systemd.
