...
- **Fase 6** voce — STANDBY. **Fase 7** (client Rust executor remoti) — QUASI CHIUSA (7/7): topic 1 MVP + C7 mutanti FATTI (write/move/delete remoti validati sul PC reale, undo device-aware ADR 0183); topic 2 multi-user FATTO (device→users.id, owner-filter), multi-OS = Windows+Linux (macOS ESCLUSO da Roberto); robustezza FATTA (task#6 blob-TTL, co-location data-locality consumer↔producer, device-i18n bundleato). **W4 AppContainer ABILITATO in prod** (7/7 sera): `appcontainer::gate_on()` default ON su Windows (opt-OUT `METNOS_SANDBOX_APPCONTAINER=0`), client 0.2.18 — validato sul PC reale (`sandbox=appcontainer ok=True` su turno di scrittura). **RESIDUO (0.2.19 MANUALE, handoff `project_w4_prod_enable_followup`)**: 2 bug scoperti dal vivo — (A) respawn self-update già corretto (`bef7e4f`, `Stdio::null()`), (B) W4 rompe gli executor `code:exec` (get_processes/tasklist bloccati da AppContainer → skip AppContainer per `code:exec`). Deploy manuale (self-update rotto sul PC). **Fase 8** stress logico — DOPO il 0.2.19.
