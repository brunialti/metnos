...
def test_g6_production_inventory(
    slug: str, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    ...
    bundled_alias = REPO_ROOT / "executors/list_dirs/path_alias.py"
    canonical_alias = REPO_ROOT / "runtime/path_alias.py"
    assert bundled_alias.is_file() and not bundled_alias.is_symlink()
    assert canonical_alias.is_file() and not canonical_alias.is_symlink()
    assert bundled_alias.read_bytes() == canonical_alias.read_bytes()
    certification_module = importlib.import_module(
        ".certification_v1", __package__
    )
    missing_root = tmp_path / "missing-worktree"
    with monkeypatch.context() as missing:
        missing.setattr(certification_module, "REPO_ROOT", missing_root)
        with pytest.raises(CertificationError, match="absent"):
            validate_production_inventory()
    inventory = validate_production_inventory(enforce_filesystem=False)
    first_path = inventory["files"][0]["path"]
    non_regular_root = tmp_path / "non-regular-worktree"
    non_regular_root.joinpath(first_path).mkdir(parents=True)
    with monkeypatch.context() as non_regular:
        non_regular.setattr(certification_module, "REPO_ROOT", non_regular_root)
        with pytest.raises(CertificationError, match="not a regular file"):
            validate_production_inventory()
    ...
    clean_results = iter((GitResult(0), GitResult(0), GitResult(0)))
    with monkeypatch.context() as clean:
        clean.setattr(
            certification_module.subprocess,
            "run",
            lambda *args, **kwargs: next(clean_results),
        )
        validate_clean_tracked_worktree()
    dirty_results = iter((GitResult(0), GitResult(1)))
    with monkeypatch.context() as dirty:
        ...
        with pytest.raises(CertificationError, match="differs"):
            validate_clean_tracked_worktree()
...
def test_g6_activity_selection(slug: str) -> None:
    ...
    assert sum(len(cells) for cells in selected.values()) == 250
...
def test_g6_pre_fix_snapshot(
    ...
    def seed(repository: Path) -> tuple[str, dict[str, str]]:
        repository.mkdir()
        git(repository, "init", "-q")
        git(repository, "config", "user.name", "RM-0008 test")
        git(repository, "config", "user.email", "rm0008@example.invalid")
        files = {
            "portable/a.py": b"assert True\n",
            "portable/inventory.json": b'{"files":[]}\n',
            "windows/a.py": b"assert True\n",
            "manifest.json": b'{"cells":[]}\n',
            "workflow.yml": b"jobs: {}\n",
            "requirements.txt": b"pytest==8.3.5\n",
            "pytest.ini": b"[pytest]\n",
            "portable-conftest.py": b"# portable\n",
            "windows-conftest.py": b"# windows\n",
        }
        ...
        git(repository, "add", ".")
        git(repository, "commit", "-q", "-m", "frozen baseline")
        source = git(repository, "rev-parse", "HEAD")
        evidence = {
            "manifest_sha256": certification_module.digest_bytes(files["manifest.json"]),
            "production_inventory_sha256": certification_module.digest_bytes(
                files["portable/inventory.json"]
            ),
        }
        return source, evidence

    def configure(repository: Path, patcher: pytest.MonkeyPatch) -> None:
        patcher.setattr(certification_module, "REPO_ROOT", repository)
        patcher.setattr(certification_module, "MANIFEST_PATH", repository / "manifest.json")
        patcher.setattr(
            certification_module,
            "INVENTORY_PATH",
            repository / "portable/inventory.json",
        )
        patcher.setattr(
            certification_module,
        ...
        patcher.setattr(
            ...
            {"portable/inventory.json"},
        )

    repository = tmp_path / "history"
    source, historical = seed(repository)
    with monkeypatch.context() as frozen:
        configure(repository, frozen)
        certification_module._validate_frozen_acceptance_baseline(
            source, historical
        )
        with pytest.raises(CertificationError, match="resolve"):
            certification_module._validate_frozen_acceptance_baseline(
                "0" * 40, historical
            )
        detached = git(
            repository,
            "commit-tree",
            git(repository, "rev-parse", f"{source}^{{tree}}"),
            "-m",
            "detached source",
        )
        with pytest.raises(CertificationError, match="not an ancestor"):
            certification_module._validate_frozen_acceptance_baseline(
                detached, historical
            )
        (repository / "portable/inventory.json").write_bytes(b'{"files":[1]}\n')
        git(repository, "add", "portable/inventory.json")
        git(repository, "commit", "-q", "-m", "mechanical inventory refresh")
        certification_module._validate_frozen_acceptance_baseline(
            source, historical
        )
        stale_inventory = dict(historical)
        stale_inventory["production_inventory_sha256"] = (
            certification_module.digest_bytes(b'{"files":[1]}\n')
        )
        with pytest.raises(CertificationError, match="source blob"):
            certification_module._validate_frozen_acceptance_baseline(
                source, stale_inventory
            )
        (repository / "portable/a.py").write_bytes(b"assert False\n")
        git(repository, "add", "portable/a.py")
        git(repository, "commit", "-q", "-m", "weaken acceptance")
        with pytest.raises(CertificationError, match="baseline differs"):
            certification_module._validate_frozen_acceptance_baseline(
                source, historical
            )

    mode_repository = tmp_path / "mode-history"
    mode_source, mode_evidence = seed(mode_repository)
    git(mode_repository, "update-index", "--chmod=+x", "portable/a.py")
    git(mode_repository, "commit", "-q", "-m", "change acceptance mode")
    with monkeypatch.context() as frozen_mode:
        configure(mode_repository, frozen_mode)
        with pytest.raises(CertificationError, match="baseline differs"):
            certification_module._validate_frozen_acceptance_baseline(
                mode_source, mode_evidence
            )


@pytest.mark.parametrize("slug", ["workflow-dependency"], ids=["workflow-dependency"])
def test_g6_workflow_dependency(slug: str, tmp_path: Path) -> None:
    validate_workflow_structure()
    workflow = WORKFLOW_PATH.read_text(encoding="utf-8")
    injected_step = workflow.replace(
        "      - name: Run the owned manifest cells\n",
        "      - name: Rewrite tracked product\n"
        "        run: python tools/rewrite_tracked_product.py\n"
        "      - name: Run the owned manifest cells\n",
        1,
    )
    injected_path = tmp_path / "injected-step.yml"
    injected_path.write_text(injected_step, encoding="utf-8")
    with pytest.raises(CertificationError, match="closed five-step"):
        validate_workflow_structure(injected_path)
    appended_command = workflow.replace(
        "          --evidence rm0008-2a-evidence-manifest.json\n",
        "          --evidence rm0008-2a-evidence-manifest.json\n"
        "          python tools/rewrite_tracked_product.py\n",
        1,
    )
    appended_path = tmp_path / "appended-command.yml"
    appended_path.write_text(appended_command, encoding="utf-8")
    with pytest.raises(CertificationError, match="unexpected folded command"):
        validate_workflow_structure(appended_path)
    action_mutants = {
        "checkout": workflow.replace(
            "actions/checkout@3d3c42e5aac5ba805825da76410c181273ba90b1 # v7.0.1",
            "example/arbitrary-action@0123456789012345678901234567890123456789",
        ),
        "setup": workflow.replace(
            "actions/setup-python@a309ff8b426b58ec0e2a45f0f869d46889d02405 # v6.2.0",
            "example/arbitrary-action@0123456789012345678901234567890123456789",
        ),
    }
    for name, mutant in action_mutants.items():
        mutant_path = tmp_path / f"arbitrary-{name}-action.yml"
        mutant_path.write_text(mutant, encoding="utf-8")
        with pytest.raises(
            CertificationError,
            match="pinned full-history checkout|frozen certification form",
        ):
            validate_workflow_structure(mutant_path)
    ignored_results = workflow.replace(
        '          for result in "$HISTORICAL" "$MANIFEST" "$PORTABLE_UBUNTU" '
        '"$PORTABLE_WINDOWS" "$CONCURRENCY_UBUNTU" "$CONCURRENCY_WINDOWS" '
        '"$WINDOWS_ACL"; do\n'
        '            test "$result" = success\n'
        "          done",
        "          true",
    )
    assert ignored_results != workflow
    ignored_results_path = tmp_path / "ignored-summary-results.yml"
    ignored_results_path.write_text(ignored_results, encoding="utf-8")
    with pytest.raises(CertificationError, match="frozen certification form"):
        validate_workflow_structure(ignored_results_path)
