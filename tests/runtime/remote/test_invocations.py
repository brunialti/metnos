...
class PurgeInvocationsTests(_InvocationsFixture):
    ...
    def test_epoch_telemetry_is_recorded_across_lifecycle(self):
        inv_id = self._mk()
        queued = invocations.get_invocation(inv_id, db_path=self.db)
        self.assertIsInstance(queued["created_epoch"], float)
        wire = invocations.next_invocation(self.device_id, db_path=self.db)
        self.assertEqual(wire["invocation_id"], inv_id)
        delivered = invocations.get_invocation(inv_id, db_path=self.db)
        self.assertIsInstance(delivered["delivered_epoch"], float)
        result = {"invocation_id": inv_id, "device_id": self.device_id,
                  "ok": True, "entries": [], "elapsed_ms": 1,
                  "sandbox": "test"}
        invocations.complete_invocation(result, db_path=self.db)
        done = invocations.get_invocation(inv_id, db_path=self.db)
        self.assertIsInstance(done["completed_epoch"], float)
        self.assertLessEqual(done["created_epoch"], done["delivered_epoch"])
        self.assertLessEqual(done["delivered_epoch"], done["completed_epoch"])
...
class ExpireStaleInvocationsTests(_InvocationsFixture):
    ...
    def test_abandoned_expiry_notifies_origin(self):
        ...
        try:
            ...
            self.assertEqual(n, 1)
            out = _un.drain(
                "http", "host", owner_user_id=self.key.device.owner_user_id)
