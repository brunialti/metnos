...
class TestFindImagesUnified(unittest.TestCase):
    ...
    def test_index_missing_returns_builder_handoff_without_side_effect(self):
        import find_images_indices as fii
        out = fii.invoke({"base_path": str(self.corpus), "query_text": "mare"})
        self.assertFalse(out["ok"])
        self.assertEqual(out["error_class"], "index_missing")
        self.assertEqual(out["error_code"], "image_index_missing")
        self.assertEqual(out["recommended_action"], {
            "executor": "create_images_indices",
            "args": {"base_path": str(self.corpus)},
        })
...
class TestFindImagesResolve(unittest.TestCase):
    ...
    def test_omitted_no_indexed_dirs(self):
        ...
        self.assertEqual(out["error_class"], "index_missing")
        self.assertEqual(
            out["recommended_action"]["executor"], "create_images_indices",
        )
    ...
    def test_discovery_uses_materialized_dir_when_workspace_link_is_hidden(self):
        """Index metadata is usable without re-hashing its offline NAS path."""
        import find_images_indices as fii

        target = self.tmp / "nas" / "photos"
        target.mkdir(parents=True)
        logical = self.tmp / "data" / "Immagini"
        logical.symlink_to(target, target_is_directory=True)
        idx_dir = fii._index_dir(logical)
        _build_unified_index(target, idx_dir, n_photos=1)

        # Model the conforming bwrap: only index:read is visible.  The logical
        # workspace symlink and source corpus are absent from that namespace.
        logical.unlink()
        out = fii.invoke({"match_all": True})

        self.assertTrue(out["ok"], out)
        self.assertEqual(len(out["entries"]), 1)
        self.assertEqual(out["_resolved_dirs"], [str(target)])

    def test_explicit_offline_target_resolves_from_persisted_metadata(self):
        import find_images_indices as fii

        target = self.tmp / "nas" / "photos-explicit"
        target.mkdir(parents=True)
        logical = self.tmp / "data" / "Immagini"
        logical.symlink_to(target, target_is_directory=True)
        idx_dir = fii._index_dir(logical)
        ...
        out = fii.invoke({"base_path": str(target), "match_all": True})

        self.assertTrue(out["ok"], out)
        self.assertEqual(len(out["entries"]), 1)
        self.assertEqual(out["base_path"], str(target))
