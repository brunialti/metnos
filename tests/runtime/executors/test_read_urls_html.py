...
class TestReadUrlsHtml(unittest.TestCase):
    ...
    def test_simple_html_page(self):
        ...
        self.assertEqual(e["origin"], self.url("/p1"))
        self.assertEqual(e["final_url"], self.url("/p1"))
        self.assertEqual(e["domain"], "127.0.0.1")
        self.assertEqual(e["language"], "it")
        self.assertEqual(e["text_length"], len(e["body_text"]))
        self.assertEqual(e["status"], "ok")
        self.assertFalse(e["redirected"])
    ...
    def test_redirect_followed(self):
        ...
        self.assertEqual(e["origin"], self.url("/redir"))
        self.assertEqual(e["final_url"], target_url)
        self.assertTrue(e["redirected"])

    def test_labeled_page_date_is_deterministic(self):
        import read_urls_html
        body = ("<html><body><article>Release 3.14.6. "
                "Last updated on Jul 21, 2026 (18:32 UTC)."
                "</article></body></html>")
        _set_pages({"/dated": (200, "text/html", body)})
    ...
    def test_audit_mode_preserves_success_and_failure_in_input_order(self):
        import read_urls_html
        _set_pages({
            "/good": (200, "text/html",
                      "<html><title>Good</title><body>readable</body></html>"),
            "/bad": (404, "text/plain", b"missing"),
        })
        out = read_urls_html.invoke({
            "urls": [self.url("/bad"), self.url("/good")],
            "include_failures_as_entries": True,
            "cache_ttl_s": 0,
        })
        self.assertTrue(out["ok"], out)
        self.assertEqual(out["ok_count"], 1)
        ...
        self.assertEqual(out["entries"][0]["status"], "not_found")
        self.assertEqual(out["entries"][0]["text_length"], 0)
        self.assertEqual(out["entries"][1]["status"], "ok")
        self.assertTrue(out["audit_mode"])

    def test_audit_mode_requires_boolean(self):
        import read_urls_html
        out = read_urls_html.invoke({
            "urls": [self.url("/p1")],
            "include_failures_as_entries": "yes",
        })
        self.assertFalse(out["ok"])
        self.assertEqual(
            out["error_code"], "include_failures_as_entries_not_boolean")
    ...
    def test_pool_is_bounded_by_distinct_host_capacity(self):
        """Thread oltre gli slot HostThrottle non possono fare lavoro utile."""
        import read_urls_html
        for i in range(8):
            _pages[f"/bounded-{i}"] = (
                200, "text/html",
                f"<html><body><article>page {i}</article></body></html>".encode(),
                {},
            )
        real_pool = read_urls_html.ThreadPoolExecutor
        seen_workers = []
        ...
        with mock.patch.object(
                read_urls_html, "ThreadPoolExecutor", side_effect=recording_pool):
            out = read_urls_html.invoke({
                "urls": [self.url(f"/bounded-{i}") for i in range(8)],
                "cache_ttl_s": 0,
                "follow_iframes": False,
            })
        self.assertEqual(out["ok_count"], 8, out)
        self.assertEqual(seen_workers, [read_urls_html._PER_HOST_MAX])

    def test_worker_cap_scales_with_distinct_hosts(self):
        import read_urls_html
        urls = [f"http://host-{i}.test/page" for i in range(5) for _ in range(4)]
        expected = min(
            read_urls_html._GLOBAL_MAX,
            len(urls),
            5 * read_urls_html._PER_HOST_MAX,
        )
        self.assertEqual(read_urls_html._worker_count_for_urls(urls), expected)
