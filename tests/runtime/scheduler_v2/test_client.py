...
def test_add_job_persists_finite_run_budget():
    out = sched_client.add_job(
        name="finite", trigger="every_60s", callback_key="noop",
        remaining_runs=3,
    )
    assert out["remaining_runs"] == 3
...
def test_run_now_advances_next_fire_at_to_now():
    # A fixed wall-clock time becomes flaky when the suite happens to run near
    # that time.  A relative recurrence preserves the actual invariant under
    # test: run_now moves a genuinely future firing time to now.
    sched_client.add_job(name="rn", trigger="every_30m", callback_key="noop")
