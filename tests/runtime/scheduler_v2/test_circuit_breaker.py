...
def test_storage_partial_is_observable_but_resets_hard_failure_streak(db_path):
    s = SchedulerStorage(db_path)
    try:
        eid = s.upsert(_rec_entry())
        s.record_outcome(eid, status="error", duration_ms=1, error="x",
                         next_fire_at=time.time() + 60,
                         decrement_remaining=False, disable=False)
        s.record_outcome(eid, status="partial", duration_ms=1, error="1/20",
                         next_fire_at=time.time() + 60,
                         decrement_remaining=False, disable=False)
        got = s.get_by_name("cb_task")
        assert got.consecutive_failures == 0
        assert got.total_failures == 2
        assert got.last_status == "partial"
    finally:
        s.close()
...
def test_daemon_partial_never_trips_hard_failure_breaker(db_path):
    async def run():
        hooks = []
        d = SchedulerDaemon(db_path)
        d.on_circuit_break = lambda entry, error: hooks.append(entry.name)
        d.callbacks.register(
            "boom", lambda payload: CallbackOutcome(
                status="partial", output="19/20", error="one failed"))
        d.storage.upsert(_rec_entry(consecutive_failures=2))
        await d.start()
        await asyncio.sleep(1.3)
        await d.stop()
        return d.storage.get_by_name("cb_task"), hooks

    got, hooks = asyncio.run(run())
...
def test_daemon_jit_aligns_legacy_finite_task_before_fire(db_path):
    async def run():
        d = SchedulerDaemon(db_path)
        d.callbacks.register("boom", lambda payload: "ok")
        d.storage.upsert(_rec_entry(payload={"times": 1}, remaining_runs=0))
        await d.start()
        await asyncio.sleep(1.3)
        await d.stop()
        return d.storage.get_by_name("cb_task")

    got = asyncio.run(run())
    assert got.enabled is False
    assert got.remaining_runs == 0
