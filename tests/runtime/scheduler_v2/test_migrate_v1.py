...
def _make_v1_recurring(path: Path) -> None:
    ...
    conn.executemany(
        "INSERT INTO recurring_tasks (name, owner_user_id, scheduler_name, "
        "schedule, query, actor, channel, chat_id, label, callback_key, "
        "times, fired_count, grace_window_minutes, enabled) "
        "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
        [
            ("morning_mail", "host-id", "user_host_morning_mail",
             "daily@08:00", "leggi mail importanti",
             "host", "telegram", "12345", "check posta mattutina",
             "run_user_query", None, 0, 240, 1),
            ("ten_pings", "guest-id", "user_guest_ten_pings",
             "every_30m", "ping",
             "guest_abc123", "telegram", "67890", "ping ogni 30m",
             "run_user_query", 10, 3, None, 1),
            ("disabled_task", "host-id", "user_host_disabled_task",
             "daily@22:00", "scrivi log",
...
def _live_fixture_owners(monkeypatch):
    import users
...
def test_migrate_inserts_user_and_builtin(tmp_path, v1_dbs):
    ...
    try:
        ...
        assert "user_host_morning_mail" in names
        assert "user_guest_ten_pings" in names
        assert "user_host_disabled_task" in names
        for _bn in _FIXTURE_BUILTINS:
            assert _bn in names, f"builtin {_bn} non migrato"
        assert "legacy_dead_task" not in names

        morning = s.get_by_name("user_host_morning_mail")
        ...
        assert morning.payload["owner_user_id"] == "host-id"
        assert morning.payload["chat_id"] == "12345"
        assert morning.grace_window_s == 240 * 60
        assert morning.enabled is True

        disabled = s.get_by_name("user_host_disabled_task")
        assert disabled is not None
        assert disabled.enabled is False

        ten = s.get_by_name("user_guest_ten_pings")
