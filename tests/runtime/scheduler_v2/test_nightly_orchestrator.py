...
def test_run_nightly_counts_declared_callback_failure(
        require_asyncio_thread_wakeup):
    cb = CallbackRegistry()

    def _reported_failure(payload=None):
        return {"ok": False, "error_class": "upstream_unavailable"}

    for key in NIGHTLY_SEQUENCE:
        cb.register(key, _reported_failure)

    rep = asyncio.run(run_nightly(cb))

    assert rep["ok_count"] == 0
    assert rep["fail_count"] == len(NIGHTLY_SEQUENCE)
    assert all(value == "error: upstream_unavailable"
               for value in rep["ran"].values())


def test_run_nightly_counts_nonzero_failure_counters(
        require_asyncio_thread_wakeup):
    cb = CallbackRegistry()

    def _degraded(payload=None):
        return {"ok": True, "ok_count": 8, "error_count": 2}

    for key in NIGHTLY_SEQUENCE:
        cb.register(key, _degraded)

    rep = asyncio.run(run_nightly(cb))

    assert rep["ok_count"] == 0
    assert rep["fail_count"] == len(NIGHTLY_SEQUENCE)
    assert all(value == "error: error_count=2"
               for value in rep["ran"].values())
