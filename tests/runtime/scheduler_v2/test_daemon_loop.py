...
def test_runtime_status_observes_loop_not_job_recency(db_path):
    async def run():
        daemon = SchedulerDaemon(db_path)
        before = daemon.runtime_status()
        await daemon.start()
        await asyncio.sleep(0)
        active = daemon.runtime_status()
        await daemon.stop()
        stopped = daemon.runtime_status()
        return before, active, stopped

    before, active, stopped = asyncio.run(run())
    assert before["state"] == "stopped"
    assert before["reason_code"] == "not_started"
    assert active["state"] == "running"
    assert active["healthy"] is True
    assert active["reason_code"] == "loop_active"
    assert active["last_run_at"] == ""
    assert stopped["state"] == "stopped"
    assert stopped["reason_code"] == "graceful_stop"


def test_runtime_status_preserves_loop_failure_reason(db_path, monkeypatch):
    async def run():
        daemon = SchedulerDaemon(db_path)
        monkeypatch.setattr(
            daemon.storage, "mark_crashed_runs",
            lambda: (_ for _ in ()).throw(RuntimeError("db unavailable")),
        )
        await daemon.start()
        await asyncio.sleep(0.05)
        status = daemon.runtime_status()
        await daemon.stop()
        return status
    ...
    assert status["reason_code"] == "loop_failed"
    assert status["error_class"] == "RuntimeError"
    assert status["error_summary"] == "db unavailable"
