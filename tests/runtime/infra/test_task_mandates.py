...
def test_sites_mandate_uses_only_verified_exact_hosts(tmp_path, monkeypatch):
    ...
    events = [
        {"event": "session_open", "owner_user_id": "host-id", "session_id": "s1",
         "domain": "telepass.com",
         "allowlist": ["telepass.com", "www.telepass.com"]},
        {"event": "allowlist_change", "owner_user_id": "host-id", "session_id": "s1",
         "domain": "telepass.com", "added_host": "login.telepass.com",
         "source": "approved_blocked_resource"},
        {"event": "credential_origin_approval", "owner_user_id": "host-id",
    ...
    mandate = task_mandates.build_for_task(
        ...
        owner_user_id="host-id", audit_path=audit)
...
def test_credential_mandate_is_the_interactive_default(tmp_path, monkeypatch):
    ...
    binding = credential_mandates.resolve_sites_binding(
        "host-id", "www.x.test", audit_path=audit)
    ...
    assert credential_mandates.resolve_sites_binding(
        "host-id", "www.x.test", audit_path=audit) is None
...
def test_unscoped_exact_topology_does_not_shadow_scoped_binding(
    ...
    audit = tmp_path / "sites.jsonl"
    events = [
        {"event": "session_open", "owner_user_id": "host-id", "session_id": "root",
         "domain": "x.test", "allowlist": ["x.test", "www.x.test"]},
        {"event": "session_open", "owner_user_id": "host-id", "session_id": "exact",
...
def test_www_only_topology_uses_scoped_base_binding(tmp_path, monkeypatch):
    ...
    audit = tmp_path / "sites.jsonl"
    audit.write_text(json.dumps({
        "event": "session_open", "owner_user_id": "host-id", "session_id": "www",
        "domain": "www.x.test", "allowlist": ["www.x.test"],
    }) + "\n")

    for requested in ("x.test", "www.x.test"):
        binding = credential_mandates.resolve_sites_binding(
            "host-id", requested, audit_path=audit)
        assert binding is not None
        assert binding["root_host"] == "x.test"
        assert binding["allowed_hosts"] == ["www.x.test", "x.test"]

    mandate = task_mandates.build_for_task(
        "accedi a x.test e leggi i documenti", "host",
        owner_user_id="host-id", audit_path=audit)
...
def test_www_alias_does_not_authorize_arbitrary_subdomain(
    ...
    audit = tmp_path / "sites.jsonl"
    audit.write_text(json.dumps({
        "event": "session_open", "owner_user_id": "host-id", "session_id": "shop",
        "domain": "shop.x.test", "allowlist": ["shop.x.test"],
    }) + "\n")

    assert credential_mandates.resolve_sites_binding(
        "host-id", "shop.x.test", audit_path=audit) is None
...
def test_recurring_task_persists_integrity_checked_mandate(tmp_path,
    ...
    row = recurring_tasks.register_user_task(
        ...
        actor="host", owner_user_id="host-id", channel="telegram",
        mandates=mandate)
    assert json.loads(row["mandates"]) == mandate
    assert task_mandates.load_for_task(
        row["scheduler_name"], "host-id", db_path=db)["query_hash"] == mandate["query_hash"]
    ...
    conn.close()
    assert task_mandates.load_for_task(
        row["scheduler_name"], "host-id", db_path=db) is None


def test_recurring_task_init_retires_ownerless_legacy_db(tmp_path,
                                                         monkeypatch):
    import recurring_tasks
    ...
    # Actor names are not immutable identity evidence: legacy ownerless tasks
    # are retired instead of being silently assigned to a future user.
    assert row is None
...
def test_scheduled_redirect_uses_task_hosts_without_dialog(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        if (task, owner, host) == ("read_x", "alice-id", "x.test") else None)
    sb._sessions.clear()
    sb._pending_opens.clear()

    out = asyncio.run(sb.op_open(
        owner="alice", owner_user_id="alice-id", url="https://x.test",
        task_name="read_x", task_owner_user_id="alice-id"))
