...
_GOLDEN = Path(__file__).resolve().parent.parent / "data" / "guard_equivalence_golden.json"
...
_WRITES_SEMANTIC_ALLOWED = {
    ...
    # and it writes a reference to `get_location`, never a coordinate of its
    # own.  A centre the planner did state is left untouched.
    ("ensure_proximity_center", "near"),
    # Il nome della famiglia output è letterale nella query e la negazione
    # impone un predicato strutturale sul path, non semantica inventata.
    ("normalize_result_folder_exclusion", "name_regex"),
    ("normalize_result_folder_exclusion", "where_field"),
    ("normalize_result_folder_exclusion", "where_regex"),
    ("normalize_result_folder_exclusion", "from_step"),
    # La route sostituisce una pipeline image-corpus impropria con la query
    # utente letterale quando lo scope richiesto e' esplicitamente web. Non
    # inventa semantica: preserva esattamente il testo sorgente.
    ("route_text_web_image_search", "queries"),
    # Dataflow type-checking: sink columns are not invented semantic values;
    # they are an additive required-schema union on extract.fields.
    ("propagate_sink_schema_to_extract", "fields"),
    # Placeholder canonicalization is a representation repair, not a change
    # to the planner's report text/path semantics: ``group_key`` is replaced
    # only when the closed group clause and declared extract schema prove the
    # exact field; compact wildcard spelling is normalized losslessly.
    ("normalize_grouped_artifact_templates", "path_template"),
    ("normalize_grouped_artifact_templates", "content_template"),
    # Explicit user constraint (new folder / do not overwrite) owns the
    # collision policy.  ``exist_ok=False`` is clause-derived safety, not a
    # planner preference.
    ("enforce_create_only_artifact_policy", "exist_ok"),
}
