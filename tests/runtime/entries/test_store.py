...
class _CrudContract:
    ...
    def test_null_order_matches_sqlite(self):
        self.store.write([
            {"id": "none-a", "n": None}, {"id": "two", "n": 2},
            {"id": "none-b", "n": None}, {"id": "one", "n": 1},
        ])
        asc = [r["id"] for r in self.store.find(
            order=[("n", "asc"), ("id", "asc")])]
        desc = [r["id"] for r in self.store.find(
            order=[("n", "desc"), ("id", "asc")])]
        self.assertEqual(asc, ["none-a", "none-b", "one", "two"])
        self.assertEqual(desc, ["two", "one", "none-a", "none-b"])
    ...
    def test_json_encoded_text_has_same_roundtrip(self):
        self.store.write({"id": "json-text", "meta": '{"k": [1, 2]}'})
        self.assertEqual(
            self.store.get(where={"id": "json-text"})["meta"],
            {"k": [1, 2]},
        )
