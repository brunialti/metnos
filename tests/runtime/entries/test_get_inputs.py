...
Run con `python3 -m pytest tests/runtime/entries/test_get_inputs.py -v`.
"""
...
def isolate_dialog_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("METNOS_OWNER_USER_ID", _OWNER)
    yield
...
class TestConsumeRetrieval:
    def test_consume_advances_then_completes(self, gi):
        ...
        did = r["dialog_id"]
        # consume step 1
        s1 = dialog_pending.consume_pending_step(
            "host", did, "a", "first", owner_user_id=_OWNER)
        assert s1["ok"] is True
        assert s1["step_index"] == 1
        assert s1["completed"] is False
        # consume step 2 → completed
        s2 = dialog_pending.consume_pending_step(
            "host", did, "b", "second", owner_user_id=_OWNER)
    ...
    def test_cancel_then_lookup(self, gi):
        ...
        did = r["dialog_id"]
        assert dialog_pending.cancel_pending(
            "host", did, owner_user_id=_OWNER) is True
        r2 = gi.invoke({"dialog_id": did})
    ...
    def test_var_mismatch_rejected(self, gi):
        ...
        res = dialog_pending.consume_pending_step(
            "host", did, "second", "x", owner_user_id=_OWNER)
