...
_ROOT = (Path(__file__).resolve().parents[3] / "runtime").parent
...
def isolated_dialog_dir(tmp_path, monkeypatch):
    ...
    # HOME copre le installazioni legacy; METNOS_USER_DATA rende esplicita la
    # stessa destinazione quando la suite globale ha gia' impostato un root.
    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv(
        "METNOS_USER_DATA", str(tmp_path / ".local" / "share" / "metnos"))
    yield tmp_path
...
def test_invoke_executor_overwrites_spoofed_runtime_identity(
        tmp_path, monkeypatch):
    import agent_runtime
    from loader import Executor

    ex = Executor(
        name="get_inputs", version="1.0", description="", affinity=[],
        args_schema={}, capabilities=[], tests=[], code_path=_CODE,
        manifest_path=_CODE.parent / "manifest.toml", signed_by="test",
    )
    captured = {}
    ...
    agent_runtime.invoke_executor(
        ex,
        {"title": "x", "dialog": [], "_actor": "foreign",
         "_channel": "foreign", "_turn_id": "foreign"},
        timeout_s=10, actor="alice", channel="telegram", turn_id="trusted",
    )

    assert captured["_actor"] == "alice"
    assert captured["_channel"] == "telegram"
    assert captured["_turn_id"] == "trusted"
