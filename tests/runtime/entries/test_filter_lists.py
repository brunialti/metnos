...
def test_set_ops_preserve_source_order():
    a = [{"id": "z"}, {"id": "a"}, {"id": "m"}]
    b = [{"id": "m"}, {"id": "b"}, {"id": "z"}]
    intersect = cl.invoke({"op": "intersect", "entries": a,
                           "entries_b": b, "on_keys": ["id"]})
    union = cl.invoke({"op": "union", "entries": a,
                       "entries_b": b, "on_keys": ["id"]})
    symdiff = cl.invoke({"op": "symdiff", "entries": a,
                         "entries_b": b, "on_keys": ["id"]})
    assert [e["id"] for e in intersect["entries"]] == ["z", "m"]
    assert [e["id"] for e in union["entries"]] == ["z", "a", "m", "b"]
    assert [e["id"] for e in symdiff["entries"]] == ["a", "b"]


def test_subprocess_consumes_one_request_and_exits():
    script = Path(cl.__file__)
    proc = subprocess.run(
        [sys.executable, str(script)],
        input=json.dumps({
            "op": "intersect", "entries": [{"id": 1}],
            "entries_b": [{"id": 1}], "on_keys": ["id"],
        }) + "\n",
