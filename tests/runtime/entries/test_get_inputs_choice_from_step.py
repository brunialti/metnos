...
Run con `python3 -m pytest tests/runtime/entries/test_get_inputs_choice_from_step.py -v`.
"""
...
def test_derivation_preserves_label_format(gi):
    ...
    state = dialog_pending.load_pending(
        sender, r["dialog_id"], owner_user_id=_OWNER)
...
def test_value_field_extraction(gi):
    ...
    import dialog_pending
    state = dialog_pending.load_pending(
        "host", r["dialog_id"], owner_user_id=_OWNER)
