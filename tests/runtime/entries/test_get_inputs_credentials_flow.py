...
Run con `python3 -m pytest tests/runtime/entries/test_get_inputs_credentials_flow.py -v`.
"""
...
def test_runtime_orchestrates_get_inputs_on_needs_inputs(isolated_dirs):
    ...
    res = orchestrate_needs_inputs(
        obs, sender_id=sender_id, actor="host", owner_user_id=_OWNER,
        channel=None)
    ...
    state = dialog_pending.load_pending(
        sender_id, dialog_id, owner_user_id=_OWNER)
...
def test_dialog_completion_triggers_credentials_store_and_resume(isolated_dirs):
    ...
    res = orchestrate_needs_inputs(
        obs, sender_id=sender_id, actor=actor, owner_user_id=_OWNER,
        channel=None)
    dialog_id = res["dialog_id"]

    # 2. Utente compila i 2 step (consume_pending_step idempotente)
    dialog_pending.consume_pending_step(
        sender_id, dialog_id, "username", "alice", owner_user_id=_OWNER)
    dialog_pending.consume_pending_step(
        sender_id, dialog_id, "password", "hunter2", owner_user_id=_OWNER)
    ...
    with mock.patch("loader.invoke_verb_unique") as mocked:
        ...
        msg_back = process_completion_callback(
            sender_id, dialog_id, actor=actor, owner_user_id=_OWNER,
            channel=None,
        ).text
