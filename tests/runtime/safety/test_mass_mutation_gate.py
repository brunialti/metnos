...
Run: `python3 -m pytest tests/runtime/safety/test_mass_mutation_gate.py -xvs`.
"""
...
class InsertMassMutationGateTests(unittest.TestCase):
    ...
    def test_without_device_does_not_invent_a_computer_location(self):
        fw = _fw(("find_files", {"base_path": "/d"}),
                 ("delete_files", {"from_step": 1}))
        out = D._insert_mass_mutation_gate(fw, "q", {})
        prompt = out.steps[1].args["prompt"].casefold()
...
class GetApprovalWebFormTests(unittest.TestCase):
    ...
    def _invoke(self, **args):
        ...
        with mock.patch.dict(
                "os.environ", {"METNOS_OWNER_USER_ID": "test-owner"}):
