...
Run: `python3 -m pytest tests/runtime/safety/test_mutating_args_grounded_placeholders.py -xvs`.
"""
...
from engine.dispatch import (  # noqa: E402
    _mutating_args_grounded, _ungrounded_mutating_args,
)
...
class MutatingArgsGroundedTests(unittest.TestCase):
    ...
    def test_create_only_turn_directory_checks_base_not_dynamic_segment(self):
        fw = _fw("create_dirs", {
            "paths": ["Documenti/Verifica Metnos/Run_${RUNTIME:turn_id}"],
            "exist_ok": False,
        })
        query = "salva in Documenti/Verifica Metnos senza sovrascrivere"
        self.assertTrue(_mutating_args_grounded(fw, query))
        self.assertEqual(_ungrounded_mutating_args(fw, query), [])

    def test_create_only_turn_directory_wrong_base_still_rejected(self):
        fw = _fw("create_dirs", {
            "paths": ["Documenti/Altro/Run_${RUNTIME:turn_id}"],
            "exist_ok": False,
        })
        query = "salva in Documenti/Verifica Metnos senza sovrascrivere"
        self.assertFalse(_mutating_args_grounded(fw, query))
        self.assertIn(
            ("create_dirs", "paths", "documenti/altro"),
            _ungrounded_mutating_args(fw, query),
        )

    def test_destructive_runtime_path_gets_no_create_only_exemption(self):
        fw = _fw("delete_files", {
            "paths": ["Documenti/Altro/Run_${RUNTIME:turn_id}"],
        })
        self.assertFalse(_mutating_args_grounded(
            fw, "cancella la cartella del turno"))
