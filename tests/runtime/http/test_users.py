...
Run: `python3 -m pytest tests/runtime/http/test_users.py -v`.
...
class UsersTests(unittest.TestCase):
    ...
    def test_pairing_token_is_atomic_under_concurrent_consumers(self):
        self.users.init_db()
        host = self.users.list_users(role="host")[0]
        user = self.users.create_user("race_user", owner_user_id=host["id"])
        token = self.users.issue_pairing_token(
            user["id"], "telegram", ttl_s=3600)
        barrier = threading.Barrier(2)
        results = []

        def _consume(recipient):
            barrier.wait()
            try:
                self.users.consume_pairing_token(
                    "telegram", recipient, token)
                results.append((recipient, "ok"))
            except ValueError:
                results.append((recipient, "rejected"))

        threads = [
            threading.Thread(target=_consume, args=("100",)),
            threading.Thread(target=_consume, args=("200",)),
        ]
        ...
        self.assertEqual(
            sorted(status for _recipient, status in results),
            ["ok", "rejected"],
        )
        channel = self.users.get_channel(user["id"], "telegram")
        winner = next(recipient for recipient, status in results
                      if status == "ok")
        self.assertEqual(channel["recipient_id"], winner)
