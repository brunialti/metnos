...
Mock urllib (getFile + binary GET) per validare:
  ...
  - foto + caption → InboundMessage con file_id non materializzato
  - media_group_id propagato
  - il download avviene soltanto dopo il gate del daemon

Run: `python3 -m pytest tests/runtime/channels/test_telegram_attached_photos.py -xvs`.
"""
...
class TelegramAttachedPhotosTests(unittest.TestCase):
    ...
    def test_poll_emits_inbound_with_deferred_photo_id(self):
        """update Telegram con `photo` + `caption` produce un InboundMessage
        con file_id opaco e nessun download pre-autenticazione."""
        ch = self._make_channel()
        blob = b"\x89PNGFAKE"
        ...
        with mock.patch(
                ...
                return_value=_FakeHTTPResponse(getupdates_body)) as request:
            msgs = ch.poll(timeout_s=0)
        self.assertEqual(len(msgs), 1)
        m = msgs[0]
        self.assertEqual(m.text, "trova foto simili a questa")
        self.assertEqual(m.extra.get("photo_file_id"), "big")
        self.assertNotIn("attached_images", m.extra)
        self.assertNotIn("attached_failed", m.extra)
        self.assertEqual(request.call_count, 1)
        self.assertFalse(self._tg_mod.UPLOAD_DIR.exists())
    ...
    def test_poll_propagates_media_group_id(self):
        ...
        with mock.patch(
                "urllib.request.urlopen",
                return_value=_FakeHTTPResponse(getupdates_body)):
            msgs = ch.poll(timeout_s=0)
        self.assertEqual(len(msgs), 1)
        self.assertEqual(msgs[0].extra.get("media_group_id"), "GRP123")

    # --- 5. poll non tenta neppure getFile -------------------------------

    def test_poll_never_downloads_before_admission(self):
        ch = self._make_channel()
        getupdates_body = json.dumps({
            "ok": True,
            "result": [{
        ...
        self.assertEqual(len(msgs), 1)
        m = msgs[0]
        self.assertEqual(m.extra.get("photo_file_id"), "BADX")
        self.assertNotIn("attached_images", m.extra)
        self.assertEqual(request.call_count, 1)
