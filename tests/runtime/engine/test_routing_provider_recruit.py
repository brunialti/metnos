...
def test_hidden_canonical_recruits_unique_provider_without_marker(monkeypatch):
    monkeypatch.setenv("METNOS_HIDE_EXECUTORS", "read_events")
    cat = _cat("read_events_google_workspace")
    out = _run(["find_events_empty"], "appuntamenti di domani",
               _intent("find", "events"), cat)
    assert "read_events_google_workspace" in out


def test_hidden_canonical_recruits_exact_provider_action(monkeypatch):
    monkeypatch.setenv("METNOS_HIDE_EXECUTORS", "find_messages")
    cat = _cat("find_messages_google_workspace", "find_credentials")
    out = _run(["find_credentials"], "cerca mail da bookings",
               _intent("find", "messages"), cat)
    assert "find_messages_google_workspace" in out


def test_hidden_canonical_does_not_guess_between_providers(monkeypatch):
    monkeypatch.setenv("METNOS_HIDE_EXECUTORS", "read_events")
    cat = _cat("read_events_google_workspace", "read_events_github")
    out = _run(["find_events_empty"], "appuntamenti di domani",
               _intent("find", "events"), cat)
    assert "read_events_google_workspace" not in out
    assert "read_events_github" not in out
