...
def test_core_provider_backends_declare_invocation_authority():
    """L'uso dei segnali legacy qui e' solo inventario di migrazione.

    L'autorita' runtime resta derivata esclusivamente da ``provider:access``
    per gli executor conformi; questo guard impedisce che il catalogo core
    aggiunga nuovo debito mentre i legacy completano la migrazione.
    """
    problems = []
    executors_root = _RT.parent / "executors"
    for manifest_path in sorted(executors_root.glob("*/manifest.toml")):
        with manifest_path.open("rb") as handle:
            manifest = tomllib.load(handle)
        name = str(manifest.get("name") or manifest_path.parent.name)
        schema = manifest.get("args") or {}
        capabilities = manifest.get("capabilities") or []
        properties = schema.get("properties") or {}
        client_schema = properties.get("client") or {}
        client_enum = client_schema.get("enum") or []
        ...
        for client in client_enum:
            ...
            actual = {
                ...
                for capability in effective_capabilities(
                    capabilities, schema, {"client": client},
                )
                if capability.get("name") == "provider:access"
                for hint in capability.get("hint", [])
            }
            expected = (
                {vocab.PROVIDER_SKILLS[client]}
                if client in vocab.PROVIDER_SKILLS else set()
            )
            if (any(value in vocab.PROVIDER_SKILLS for value in client_enum)
                    and actual != expected):
                problems.append(
                    f"{name}: client={client!r}, expected={sorted(expected)}, "
                    f"actual={sorted(actual)}"
                )

        suffix = next(
            (provider for provider in vocab.PROVIDER_SUFFIXES
             if name.endswith("_" + provider)),
            None,
        )
        if suffix is not None and not client_enum:
            actual = {
                hint
                for capability in effective_capabilities(capabilities, schema, {})
                if capability.get("name") == "provider:access"
                for hint in capability.get("hint", [])
            }
            expected = {vocab.PROVIDER_SKILLS[suffix]}
            if actual != expected:
                problems.append(
...
def test_modality_and_provider_are_two_orthogonal_axes():
    for nm, modality in (
        ("read_files_xlsx_google_workspace", "xlsx"),
        ("send_messages_thread_google_workspace", "thread"),
        ("list_messages_labels_google_workspace", "labels"),
    ):
        nc = ng.parse_name(nm)
        assert nc.qualifier == modality and nc.provider == "google_workspace"
        assert nc.full == nm
        result = ng.validate_name(nm)
        assert result.ok, f"{nm} dovrebbe validare: {result.reason}"
