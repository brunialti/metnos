...
class TestApplyToFramework(unittest.TestCase):
    ...
    def test_injects_before_artifact_boundary_and_rewires_sheet(self):
        """A sort after report/spreadsheet sinks is semantically dead."""
        fw = Framework.from_dict({
            "steps": [
                {"tool": "read_urls_html", "args": {"urls": ["https://x"]}},
                {"tool": "extract_entries", "args": {
                    "from_step": 1, "fields": ["domain", "title"]}},
                {"tool": "group_entries", "args": {
                    "from_step": 2, "dedup_key": "url"}},
                {"tool": "create_dirs", "args": {"paths": ["Output"]}},
                {"tool": "write_files", "args": {
                    "from_step": 3, "path": "Output/report.txt"}},
                {"tool": "create_files_spreadsheet", "args": {
                    "from_step": 3, "columns": ["domain", "title"]}},
                {"tool": "final_answer", "args": {}},
            ],
            "final_message": "saved",
        })
        out = apply_to_framework(
            fw, "crea un foglio ordinato per dominio",
            catalog_names=_CAT | {"read_urls_html", "extract_entries",
                                  "group_entries", "create_dirs",
                                  "write_files",
                                  "create_files_spreadsheet"})
        tools = [step.tool for step in out.steps]
        assert tools == [
            "read_urls_html", "extract_entries", "group_entries",
            "sort_entries", "create_dirs", "write_files",
            "create_files_spreadsheet", "final_answer",
        ]
        assert out.steps[3].args["from_step"] == 3
        assert out.steps[5].args["from_step"] == 4
...
class TestDescribeGroupDirective(unittest.TestCase):
    ...
    def test_group_by_directive_sections_in_order(self):
        ...
        self.assertIn("campo `account`", prompt)
    ...
    def test_unresolvable_key_conceptual_directive(self):
        ...
        self.assertTrue(res["ok"])
        self.assertIn("criterio `tema`", self._prompts[-1])
    ...
    def test_high_cardinality_key_ordered_directive(self):
        ...
        self.assertTrue(res["ok"])
        self.assertIn("già ordinato per `subject`", self._prompts[-1])
