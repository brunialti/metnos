...
def test_execution_policy_cap_is_not_offered_as_a_tool_argument(
        isolated_dialog_dir):
    log = _make_turnlog_with_truncated_step(
        executor="describe_images",
        cap_field="max_images_per_request",
        cap_value=8,
        available_total=1000,
        used=8,
        truncated_what="file immagine",
    )
    log.steps[0].result["cap_expandable"] = False

    assert log._collect_expandable_caps() == []
