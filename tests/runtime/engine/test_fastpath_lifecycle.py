...
class _FastpathDbCase(unittest.TestCase):
    """DB fastpath isolato per-test (stesso pattern di test_engine_core)."""
...
class TestShouldCachePlan(unittest.TestCase):
    ...
    def test_query_context_number_is_cacheable_but_query_specific(self):
        # Il contesto copiato dalla query non è un ID baked: su L0 exact-match
        # è sicuro, mentre is_query_specific impedisce L0-cosine/L1.
        import json
        from engine.executor import is_query_specific
        fw = _fw("describe_entries", args_map={
            "describe_entries": {
                "from_step": 1,
                "context": "analizza gli ultimi 60 giorni",
            },
        })
        self.assertTrue(eng_dispatch._should_cache_plan(
            fw, "analizza gli ultimi 60 giorni"))
...
class TestTemporalCacheability(_FastpathDbCase):
    ...
    def test_absolute_iso_literal_not_recorded(self):
        ...
        self.assertEqual(
            eng_fastpath._absolute_temporal_literals(fw),
            [("read_messages", "args.since_iso", "2026-06-11")],
        )
        self.assertEqual(
