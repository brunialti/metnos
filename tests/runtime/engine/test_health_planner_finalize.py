...
- Level 2: handle_describe_entries usa `health_context` per pre-pendere
  l'indicazione che lo stato del sistema è già stato riassunto.
...
class TestLevel2DescribeEntriesHealthContext(unittest.TestCase):
    ...
    al prompt LLM il blocco dello stato già riassunto e l'istruzione di non
    ripeterlo. Il test verifica il significato, non una vecchia grafia."""
    ...
    def test_health_context_pre_prepends_directive(self):
        ...
        self.assertIn("STATO DEL SISTEMA GIÀ RIASSUNTO", prompt)
        self.assertIn("NON DEVI: ripetere", prompt)
        self.assertIn("non dichiararli indisponibili", prompt)
    ...
    def test_no_health_context_no_directive(self):
        ...
        self.assertTrue(res.get("ok"))
        self.assertNotIn("STATO DEL SISTEMA GIÀ RIASSUNTO", captured["prompt"])
...
class TestLevel3PrependHealthEmptyMessage(unittest.TestCase):
    ...
    def test_prepend_health_preserves_multidomain_summary(self):
        """Health is additive in a file+system pipeline, never a replacement."""
        from agent_runtime import TurnLog, StepLog
        log = TurnLog(ts_start=0.0, turn_id="test_multidomain_health",
                      user_query="mostra i file e indica RAM, spazio libero e CPU")
        files = StepLog(step_num=1, chosen_tool="sort_entries", raw_args={})
        files.result = {"ok": True, "entries": [{"name": "big.xlsx", "size": 42}]}
        health = StepLog(step_num=2, chosen_tool="get_processes",
                         raw_args={"include_health": True})
        health.result = {
            "ok": True, "entries": [],
            "health": {
                "load": {"available": True, "1m": 0.2, "5m": 0.2,
                          "15m": 0.2, "uptime_s": 100},
                "memory": {"available": True, "used_mb": 100,
                           "total_mb": 1000, "swap_pct": 0, "pct": 10},
                "disk": [], "services": [],
            },
        }
        log.steps = [files, health]
        log.final_kind = "answer"
        log.final_message = "4 file sorgente; totale 8.242 byte; top: big.xlsx"
        log._prepend_health_block_if_any()
        self.assertIn("4 file sorgente", log.final_message)
        self.assertIn("Stato", log.final_message)
