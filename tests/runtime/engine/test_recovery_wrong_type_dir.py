...
Run: `python3 -m pytest tests/runtime/engine/test_recovery_wrong_type_dir.py -xvs`.
"""
...
class MutatingHonestyMessageTests(unittest.TestCase):
    ...
    def _turn_with_delete_result(self, result, final="Fatto."):
        ...
        # Isola la coda user_notices dalla produzione e da altri test: write()
        # drena intenzionalmente gli avvisi per (channel, actor).
        log = agent_runtime.TurnLog(
            ts_start=0.0, user_query="q",
            channel="test", actor="mutating-honesty-test")
