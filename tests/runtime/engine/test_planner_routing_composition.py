...
def _load_real_catalog():
    ...
    cat = Catalog()
    _load_dir_into_catalog(Path(__file__).resolve().parents[3] / "executors", cat,
...
# ── 3. Regression (ADR 0117) — «X al mare» usa l'indice unificato ────

class TestPipelineSceneRegression:
    def test_X_al_mare_keeps_find_images_indices_in_pool(self):
        """«carol al mare» (persona+scena) → find_images_indices DEVE
        restare nel top-K per la chiamata unificata name+query_text.
        """
        ...
        assert "find_images_indices" in top, (
            ...
            f"(indice unificato). Top8={top}"
        )

    def test_X_al_mare_routes_to_unified_images_without_name_bias(self):
        """«carol al mare» usa l'indice immagini unificato ADR 0117.

        I nomi propri non appartengono alle affinity statiche: introdurrebbero
        un bias verso poche persone registrate e riaprirebbero la vecchia
        pipeline a due passi. La scena rende invece find_images_indices un
        candidato positivo; il planner puo' passargli direttamente nome+testo.
        """
        cat = {e.name: e for e in _load_real_catalog()}
        if "find_persons_indices" not in cat or "find_images_indices" not in cat:
            pytest.skip("executor non in catalog (env minimale)")
        ...
        s_images = _score_for(q, cat["find_images_indices"])
        assert s_persons == 0, (
            f"un nome hardcoded contamina find_persons_indices: {s_persons}")
        assert s_images > 0, (
            f"find_images_indices score atteso > 0, ottenuto {s_images}")
