...
class _BaseDigestTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self._env = mock.patch.dict("os.environ", {
            ...
            "METNOS_USERS_DB": str(self._tmpdir / "users.sqlite"),
            "METNOS_LANG": "it",
    ...
    def _seed_promoted_grace(self, proposal_id: str, name: str,
        ...
        conn.execute(
            ...
            " evaluator_verdict, practical_example, notified_at, created_at) "
            "VALUES (?, ?, 'promoted_grace', '2026-05-11T00:00:00Z', "
            " '2026-05-14T00:00:00Z', ?, ?, ?, '2026-05-11T00:00:00Z')",
            (proposal_id, name, verdict, example, notified_at),
        )
        conn.commit()
        conn.close()
...
class TestInlineKeyboard(_BaseDigestTest):
    ...
    def test_keyboard_format_ok_rollback(self):
        ...
        self.assertEqual(kb[0][0]["text"], "Conferma")
        self.assertEqual(kb[0][0]["data"], "promoter:foo_bar_123:ok")
        self.assertEqual(kb[0][1]["text"], "Ripristina")
...
class TestAggregatedMode(_BaseDigestTest):
    ...
    def test_threshold_triggers_aggregated_message(self):
        ...
        self.assertIn("Apri la revisione", body)
    ...
    def test_host_preference_controls_digest_language(self):
        ...
        self.assertTrue(users.set_pref(host["id"], "lang", "en")["ok"])
        users.add_channel(
            host["id"], "telegram", "123456", verified=True,
        )
        sends = []
    ...
    def test_persisted_example_is_rebuilt_in_recipient_language(self):
        ...
        import json
        proposal_id = "lang_example_001"
        proposals = self._tmpdir / "proposals"
        proposals.mkdir()
        os.environ["METNOS_SYNT_PROPOSALS_DIR"] = str(proposals)
        (proposals / f"{proposal_id}.json").write_text(json.dumps({
            "id": proposal_id,
            "name": "find_packages",
            "user_query": "find JSON packages",
            "path_steps": ["find_files", "filter_entries"],
            "stages": [],
        }), encoding="utf-8")
        self._seed_promoted_grace(
            proposal_id, "find_packages",
            example="**Pipeline corrente**: contenuto persistito in italiano",
            verdict=json.dumps({
                "verdict": "accept",
                "signals": {"call_freq_60d": 2},
            }),
        )
        import i18n
        from jobs.promoter_state import pending_notification
