...
Run con `python3 -m pytest tests/runtime/learning/test_propose_and_fire_pipeline.py -v`.
"""
...
_EXEC_FF_DIR = Path(__file__).resolve().parents[3] / "executors/find_events_empty"
_EXEC_GI_DIR = Path(__file__).resolve().parents[3] / "executors/get_inputs"
...
def _run_pipeline(fef, gi, query_label):
    ...
    state = dialog_pending.load_pending(
        "host", out_gi["dialog_id"], owner_user_id="pytest-runtime-owner")
