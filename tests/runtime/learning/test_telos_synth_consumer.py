...
class TestConsumer(unittest.TestCase):
    ...
    def test_candidate_is_preserved_and_counts_as_success(self):
        self._write_marker("candidate1", "find_new", "trova elementi nuovi")
        with mock.patch("synth_request.handle_synth_request") as m:
            m.return_value = {
                "ok": True,
                "synthesized": True,
            ...
            result = tsc.run_once(max_jobs=10)
        self.assertEqual(result["success"], 1)
        self.assertEqual(result["failed"], 0)
        self.assertTrue(
            (self.processed / "candidate1.candidate.json").exists(),
        )
