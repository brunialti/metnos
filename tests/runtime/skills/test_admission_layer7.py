...
class TestL7Admission:
    ...
    def test_case2_synth_disjoint_no_block(self, isolated_imports, monkeypatch):
        ...
        def fake_multistage(intent, llm_m, llm_w, progress=None, **_kwargs):
...
class TestBindingShortCircuit:
    ...
    da tool builtin nativi (cifs/ssh → admin; web → login_urls).
    ...
    def test_web_query_redirects_to_login_urls(self, monkeypatch):
        self._patch_empty_catalog(monkeypatch)
        from synth_request import handle_synth_request
        res = handle_synth_request(
        ...
        assert res.get("name") == "login_urls"
