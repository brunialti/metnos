...
class TestRealCatalogCapabilitiesSchema(unittest.TestCase):
    ...
    def test_all_real_manifests_have_array_of_tables_capabilities(self):
        import tomllib
        repo_executors = Path(__file__).resolve().parents[3] / "executors"
...
class TestElasticPoolBudget(unittest.TestCase):
    """Le teste corte cedono budget senza rendere il prompt illimitato."""

    def test_short_head_donates_unused_budget_to_long_head(self):
        from manifest_rules import (
            RENDER_BUDGET, RENDER_HARD_MAX, RENDER_POOL_SLACK,
            render_heads_budgeted,
        )

        short = "SCOPO: breve. PATTERN: a(). NON: b. OUT: x."
        long_head = (
            "SCOPO: " + "analisi completa " * 11
            + "PATTERN: b(entries=[...]). "
            + "NON: conserva integralmente questo confine deterministico. "
        )
        long = long_head + "OUT: entries."
        rendered = render_heads_budgeted([short, long])

        self.assertEqual(rendered[0], short.split("OUT:", 1)[0].strip())
        self.assertEqual(rendered[1], long_head.strip())
        self.assertGreater(len(rendered[1]), RENDER_BUDGET)
        self.assertLessEqual(len(rendered[1]), RENDER_HARD_MAX)
        self.assertLessEqual(
            sum(map(len, rendered)),
            2 * RENDER_BUDGET + RENDER_POOL_SLACK,
        )

    def test_pool_budget_is_deterministic_and_bounded(self):
        from manifest_rules import (
            RENDER_BUDGET, RENDER_HARD_MAX, RENDER_POOL_SLACK,
            render_heads_budgeted,
        )
        ...
        descriptions = [
            f"SCOPO: {str(i) + ' parola' * 90}. PATTERN: x(). NON: y. OUT: z."
            for i in range(5)
        ]
        first = render_heads_budgeted(descriptions)
        second = render_heads_budgeted(descriptions)
        self.assertEqual(first, second)
        self.assertTrue(all(len(item) <= RENDER_HARD_MAX for item in first))
        self.assertLessEqual(
            sum(map(len, first)),
            len(first) * RENDER_BUDGET + RENDER_POOL_SLACK,
        )

    def test_proposer_pool_uses_the_shared_budget(self):
        from types import SimpleNamespace
        from engine.proposer import _render_tool_pool

        short = SimpleNamespace(
            name="short_tool",
            description="SCOPO: breve. PATTERN: short_tool(). NON: x. OUT: y.",
            args_schema={"type": "object", "properties": {}},
        )
        boundary = "BOUNDARY_ELASTICO_VISIBILE"
        long = SimpleNamespace(
            name="long_tool",
            description=(
                "SCOPO: " + "contesto utile " * 13
                + "PATTERN: long_tool(). NON: " + boundary + ". OUT: y."
            ),
            args_schema={"type": "object", "properties": {}},
        )

        rendered = _render_tool_pool(
