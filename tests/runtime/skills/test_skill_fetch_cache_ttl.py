...
class TestCacheValidityCheck:
    ...
    def test_fresh_repository_root_is_valid(self, isolated_cache,
                                             fake_skill_md_content):
        """Il clone GitHub ha SKILL.md sotto <cache>/<repo>/, non alla root."""
        from skill_fetch import _cached_skill_path, _is_cache_valid
        cache_dir = isolated_cache / "key"
        skill = cache_dir / "example-repo" / "SKILL.md"
        skill.parent.mkdir(parents=True)
        skill.write_bytes(fake_skill_md_content)
        assert _is_cache_valid(cache_dir, ttl_s=3600) is True
        assert _cached_skill_path(cache_dir, ttl_s=3600) == skill
...
class TestGitHubRepositoryCacheHit:
    def test_cached_clone_returns_nested_skill_without_recloning(
        self, isolated_cache, fake_skill_md_content, monkeypatch,
    ):
        import skill_fetch

        url = "https://github.com/example/example-skill"
        cache_dir = skill_fetch._cache_dir_for(url)
        skill = cache_dir / "example-skill" / "SKILL.md"
        skill.parent.mkdir(parents=True)
        skill.write_bytes(fake_skill_md_content)

        def _must_not_clone(*_args, **_kwargs):
            raise AssertionError("fresh repository cache was ignored")

        monkeypatch.setattr(skill_fetch, "_git_clone_github", _must_not_clone)
        assert skill_fetch.fetch_skill_source(url, cache_ttl_s=3600) == skill
