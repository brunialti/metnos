...
class TestAlignPromptsPattern(unittest.TestCase):
    ...
    def test_nested_planner_role_is_aligned(self):
        role = "planner/sections/web/search"
        self._setup_role(role, {"it": "Cerca sul web."})
        with mock.patch.object(self.i18n_translator, "translate_prompt_file") as mtp:
            mtp.return_value = {
                "ok": True,
                "role": role,
                "candidate_path": str(
                    self.prompts_root / "en" / "_pending" / f"{role}.j2.candidate"
                ),
                "validation": [],
            }
            with mock.patch.object(
        ...
        self.assertIn(role, {r.get("role") for r in results})
        mtp.assert_called_once_with(
            role, target_lang="en", source_lang="it", tier="wise",
        )
