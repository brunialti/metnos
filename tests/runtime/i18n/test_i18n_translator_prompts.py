...
class TestCanonicalVocabularyInTranslationPrompts(unittest.TestCase):
    def test_templates_render_the_complete_current_vocabulary(self):
        rendered = (
            itx._PROMPT_TEMPLATE_LLM_TARGETED.format(
                source_name="Italian",
                target_name="English",
                json_strings="{}",
                **itx._VOCAB_PROMPT_ARGS,
            )
            + itx._PROMPT_FILE_TEMPLATE.format(
                source_name="Italian",
                target_name="English",
                source_text="test",
                **itx._VOCAB_PROMPT_ARGS,
        ...
        for token in ACTIONS + OBJECTS + QUALIFIERS:
...
class TestTranslatePromptFile(unittest.TestCase):
    ...
    def test_nested_role_creates_candidate_parent_directories(self):
        role = "planner/sections/web/search"
        source = self.tmp / "prompts" / "it" / f"{role}.j2"
        source.parent.mkdir(parents=True, exist_ok=True)
        source.write_text(
            (self.tmp / "it" / f"{self.role}.j2").read_text(encoding="utf-8"),
            encoding="utf-8",
        )
        fake_file = self.tmp / "i18n_translator.py"
        fake_file.write_text("# stub", encoding="utf-8")
        orig_file = itx.__file__
        try:
            itx.__file__ = str(fake_file)
            res = itx.translate_prompt_file(
                role, target_lang="en", source_lang="it",
            )
        finally:
            itx.__file__ = orig_file

        self.assertTrue(res.get("ok"), res)
        candidate = Path(res["candidate_path"])
        self.assertTrue(candidate.is_file())
        self.assertEqual(
            candidate.relative_to(self.tmp).as_posix(),
            "prompts/en/_pending/planner/sections/web/search.j2.candidate",
        )
