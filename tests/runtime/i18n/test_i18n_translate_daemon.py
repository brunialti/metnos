...
def _fake_call_llm_factory(translations: dict[str, str]):
    ...
    dal campo "Testo sorgente: ..." del prompt e ritorna il JSON corrispondente.
    """
    def _fake(query, sys_prompt, *, tier="middle", max_tokens=600,
              temperature=0.0, think=False):
        # Il prompt e' la `query`; estraiamo il source text dal campo stabile.
        source_text = ""
        if isinstance(query, str) and "Testo sorgente:" in query:
            source_text = query.split("Testo sorgente:", 1)[1].strip()
...
class TestI18nTranslateTask(unittest.TestCase):
    ...
    def test_idempotent_skip_when_source_unchanged(self):
        ...
        pending = conn.execute(
            "SELECT needs_translation FROM i18n "
            "WHERE key='MSG_HI' AND lang='en'"
        ).fetchone()[0]
        conn.close()
        self.assertEqual(pending, 0, "lo skip idempotente deve drenare la coda")
    ...
    def test_invalid_legacy_pending_does_not_starve_valid_row(self):
        """Il LIMIT si applica dopo il filtro delle pending azionabili."""
        _seed_i18n_db(self._db, [
            ("AAA_STALE", "it", "Gia italiano", 1, None),
            ("MSG_VALID", "it", "Traduci questo", 0, None),
            ("MSG_VALID", "en", None, 1, "it"),
        ])
        task = self._import_task()
        with mock.patch("llm_helpers.call_llm",
                        _fake_call_llm_factory({"Traduci questo": "Translate this"})):
            result = task(payload={})
        self.assertEqual(result["ok_count"], 1)
        self.assertEqual(result["metadata"]["pending_seen"], 1)
        conn = sqlite3.connect(str(self._db))
        try:
            stale = conn.execute(
                "SELECT needs_translation FROM i18n "
                "WHERE key='AAA_STALE' AND lang='it'"
            ).fetchone()[0]
            translated = conn.execute(
                "SELECT text, needs_translation FROM i18n "
                "WHERE key='MSG_VALID' AND lang='en'"
            ).fetchone()
        finally:
            conn.close()
        self.assertEqual(stale, 1)
        self.assertEqual(translated, ("Translate this", 0))

    def test_auto_synth_review_is_separate_from_translation_queue(self):
        """Materializzazione bilingue: auto_translated=1, pending=0."""
        _seed_i18n_db(self._db, [
            ("ERR_SYNTH_X", "it", "<auto-synth: ERR_SYNTH_X>", 0, None),
            ("ERR_SYNTH_X", "en", "<auto-synth: ERR_SYNTH_X>", 0, None),
        ])
        task = self._import_task()

        def _stub_fake(*_args, **_kwargs):
            return (json.dumps({"it": "Errore synth.", "en": "Synth error."}),
                    {"model": "mock"})

        with mock.patch("llm_helpers.call_llm", _stub_fake):
            result = task(payload={})
        self.assertEqual(result["metadata"]["reason"], "no_pending")
        conn = sqlite3.connect(str(self._db))
        try:
            rows = conn.execute(
                "SELECT lang, text, needs_translation, auto_translated, "
                "source_lang, source_text_hash, version_hash "
                "FROM i18n WHERE key='ERR_SYNTH_X' ORDER BY lang"
            ).fetchall()
        finally:
            conn.close()
        ...
        en, it = rows
        self.assertEqual(en[4], "it")
        self.assertEqual(en[5], it[6])

    def test_placeholder_loss_is_retried_before_persisting(self):
        _seed_i18n_db(self._db, [
            ("MSG_PLACE", "it", "Ciao {name}, hai {n} elementi.", 0, None),
            ("MSG_PLACE", "en", None, 1, "it"),
        ])
        task = self._import_task()
        calls = {"n": 0}

        def _candidate_fake(*_args, **_kwargs):
            calls["n"] += 1
            text = ("Hello." if calls["n"] == 1
                    else "Hello {name}, you have {n} items.")
            return json.dumps({"translation": text}), {"model": "mock"}

        with mock.patch("llm_helpers.call_llm", _candidate_fake):
            result = task(payload={})
        ...
        try:
            row = conn.execute(
                "SELECT text, needs_translation, source_text_hash, version_hash "
                "FROM i18n WHERE key='MSG_PLACE' AND lang='en'"
            ).fetchone()
        finally:
            conn.close()
        self.assertEqual(row[0], "Hello {name}, you have {n} items.")
        self.assertEqual(row[1], 0)
        self.assertTrue(row[2].startswith("sha256:"))
        self.assertTrue(row[3].startswith("sha256:"))

    def test_interprocess_lock_makes_second_runner_a_noop(self):
        _seed_i18n_db(self._db, [
            ("MSG_LOCK", "it", "Sorgente", 0, None),
            ("MSG_LOCK", "en", None, 1, "it"),
        ])
        task = self._import_task()
        mod = sys.modules["jobs.i18n_translate_pending"]
        with mod._exclusive_translation_lock() as acquired:
            self.assertTrue(acquired)
            result = task(payload={})
        self.assertTrue(result["ok"])
        self.assertEqual(result["metadata"]["reason"], "already_running")
        conn = sqlite3.connect(str(self._db))
        pending = conn.execute(
            "SELECT needs_translation FROM i18n "
            "WHERE key='MSG_LOCK' AND lang='en'"
        ).fetchone()[0]
        conn.close()
        self.assertEqual(pending, 1)

    def test_source_change_during_llm_call_defers_stale_translation(self):
        _seed_i18n_db(self._db, [
            ("MSG_RACE", "it", "Versione uno", 0, None),
            ("MSG_RACE", "en", None, 1, "it"),
        ])
        task = self._import_task()

        def _racing_fake(*_args, **_kwargs):
            conn = sqlite3.connect(str(self._db))
            conn.execute(
                "UPDATE i18n SET text='Versione due' "
                "WHERE key='MSG_RACE' AND lang='it'"
            )
            conn.commit()
            conn.close()
            return json.dumps({"translation": "Version one"}), {"model": "mock"}

        with mock.patch("llm_helpers.call_llm", _racing_fake):
            result = task(payload={})
        ...
        try:
            row = conn.execute(
                "SELECT text, needs_translation FROM i18n "
                "WHERE key='MSG_RACE' AND lang='en'"
            ).fetchone()
        finally:
            conn.close()
        self.assertEqual(row, (None, 1))
        audit = Path(result["metadata"]["audit_path"])
        events = [json.loads(line) for line in audit.read_text().splitlines()]
        self.assertEqual(events[-1]["reason"], "source_changed_during_translation")
