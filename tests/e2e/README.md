...
- **Zero import** da `tests/e2e/` verso `runtime/` (o viceversa). Il simulatore parla SOLO via HTTP/CLI.
- **Server in subprocess**: `python3 -m runtime.metnos_http_server` su porta random.
- **Storage isolato**: `tests/e2e/tmp/<run_id>/` (mai tocca `~/.local/share/metnos/` reale).
- **Credenziali**: copiate runtime da sistema in dir isolata, mai committed in repo.

## Quick start

```bash
cd /opt/metnos/tests/e2e
...
Le soglie canoniche sono in `quality_targets.json`. L'obiettivo è raggiunto
solo con due esecuzioni consecutive sulla stessa matrice che abbiano: zero
fallimenti, zero errori, successo del 100% sui test eseguiti, copertura di
esecuzione almeno del 95% e almeno 280 test raccolti. Ogni passaggio conserva
un JUnit e un log distinti; `quality-summary.json` registra metriche, impronte
della matrice e stato finale `objective_achieved`.
