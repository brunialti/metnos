...
Usa un calendario effimero creato e cancellato dal test per evitare pollution
del calendario reale e dipendenza da identificatori esterni persistenti.
...
def cal_id() -> str:
    ...
    created = cal.create_calendar({
        "summary": f"Metnos E2E {time.time_ns()}",
        "description": "Isolated automated lifecycle test",
        "timezone": "Europe/Rome",
    })
    assert created.get("ok"), f"calendar setup failed: {created}"
    cid = (created.get("results") or [{}])[0].get("calendar_id")
    assert cid, f"calendar setup returned no id: {created}"
    try:
        yield cid
    finally:
        deleted = cal.delete_calendar({"calendar_id": cid})
        assert deleted.get("ok"), f"calendar cleanup failed: {deleted}"
