...
Il server usa una copia isolata e realistica dello storage live: le query
read-only storiche vedono indici e configurazioni, mentre ogni scrittura resta
confinata nella directory temporanea E2E.
"""
...
async def driver(quality_server: E2EServer):
    pending = quality_server.user_data / "get_inputs"
    if pending.exists():
        import shutil
        shutil.rmtree(pending, ignore_errors=True)
    async with E2EClient(quality_server.url, quality_server.admin_key,
