...
def test_provisioned_birth_bootstrap_builds_with_separated_authorities(
    ...
    stores = [
        load_birth_keystore(config_path.parent / "author-keystore"),
        load_birth_keystore(config_path.parent / value["admission"]["keystore"]),
    ]
    ...
    assert len(stores) == 2 + len(_producer_capabilities_for_bootstrap())
    ...
    assert bundle.verification.producer_db.exists()
    assert not hasattr(bundle, "core")
