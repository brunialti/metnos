...
def _infer_success(turn: dict, feedback_by_turn: dict) -> int | None:
    """Feedback esplicito > evidenza operativa. Ritorna 1/0/None.

    ``final_kind=answer`` indica soltanto che il protocollo ha prodotto testo:
    non dimostra che l'intento sia stato completato. L'inferenza positiva
    richiede quindi almeno un executor produttivo e risultati esplicitamente
    riusciti; i turni solo conversazionali restano non classificati.
    """
    ...
    if tid and tid in feedback_by_turn:
        ...
        if action == "error":
            return 0
    # Inferenza conservativa dall'esito registrato.
    final_kind = turn.get("final_kind", "")
    if final_kind in ("error", "timeout", "loop_break"):
        return 0
    steps = [
        step for step in (turn.get("steps") or [])
        if step.get("chosen_tool") and step.get("chosen_tool") != "final_answer"
    ]
    if final_kind != "answer" or not steps:
        return None
    outcomes = []
    for step in steps:
        ...
        if not isinstance(result, dict):
            return None
        if result.get("decision") in {"needs_inputs", "needs_approval"}:
            return None
        if result.get("ok") is False or result.get("error_class"):
            return 0
