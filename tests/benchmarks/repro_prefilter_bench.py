...
(``tests/benchmarks/corpus_snapshot.jsonl``) and a frozen snapshot of the real 96-tool
catalog (``tests/benchmarks/catalog_snapshot.json``), then measures, per strategy:
...
Usage:
    ...
    python3 tests/benchmarks/repro_prefilter_bench.py --mode scaling
    python3 tests/benchmarks/repro_prefilter_bench.py --mode both --output /tmp/report.md
"""
...
# PII-free typing snapshot ships in tests/benchmarks/typing so the prod number reproduces
...
def dense_available() -> bool:
    ...
    (see tests/benchmarks/requirements-dense.txt). Absent it, we SKIP them honestly rather
...
def main() -> None:
    ...
    if not dense_available():
        note = ("dense baseline SKIPPED — BGE-M3 embedder not installed "
                "(see tests/benchmarks/README.md 'Reproducing the dense baseline')")
    ...
    blocks: list[str] = [
        ...
        "",
        f"Generated by `tests/benchmarks/repro_prefilter_bench.py` · seed={SEED} · "
        "deterministic (no LLM).",
        f"Corpus: `tests/benchmarks/corpus_snapshot.jsonl` "
        f"({len(corpus)} organic queries, PII-scrubbed). "
        f"Catalog: `tests/benchmarks/catalog_snapshot.json` ({len(real_execs)} tools, frozen).",
