...
Uso:
  METNOS_ENGINE=v3   python3 tests/benchmarks/compound_dryrun.py [--query "..."]
  METNOS_ENGINE=metis python3 tests/benchmarks/compound_dryrun.py     # confronto v2
"""
...
def build_calls():
    ...
    def fast(system, user, max_tokens=320, think=False):
        return getattr(r.provider("fast").chat(system, user, max_tokens=max_tokens), "text", "")
    def wise(system, user, *, max_tokens=2048, think=True, **kw):
        ...
        if grammar is not None: ck["grammar"] = grammar
        return (getattr(r.provider(tier).chat(system, user, **ck), "text", "") or "").strip()
    return fast, wise
