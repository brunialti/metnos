...
private turn logs: it reads a frozen, PII-scrubbed query snapshot that ships
in the repo (``data/prefilter_corpus_snapshot.jsonl``) and the real executor
catalog (``executors/``), then measures, per strategy:
...
Usage:
    python3 scripts/repro_prefilter_bench.py --mode comparison
    python3 scripts/repro_prefilter_bench.py --mode scaling
    python3 scripts/repro_prefilter_bench.py --mode both --output /tmp/report.md
"""
...
SNAPSHOT = _ROOT / "data" / "prefilter_corpus_snapshot.jsonl"
...
def main() -> None:
    ...
    print("loading frozen snapshot + real catalog…", flush=True)
    corpus = load_snapshot()
    from loader import load_catalog
    cat = load_catalog(verify=True, include_synth=True)
    real_execs = list(cat.executors.values())
    print(f"  {len(corpus)} queries · {len(real_execs)} real executors",
          flush=True)

    blocks: list[str] = [
        "# Prefilter strategies — reproducible benchmark",
        "",
        f"Generated by `scripts/repro_prefilter_bench.py` · seed={SEED} · "
        "deterministic (no LLM).",
        f"Corpus: `data/prefilter_corpus_snapshot.jsonl` "
