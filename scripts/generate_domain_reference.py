...
DOMAIN_COPY = {
    ...
    "tasks": _copy("communication", ("Attività programmate", "Scheduled tasks"),
        ("Crea, consulta, modifica ed elimina promemoria o richieste ricorrenti; conserva anche la cronologia delle esecuzioni. Riguarda soltanto lo scheduler interno di Metnos.",
         "Creates, reads, changes, and deletes reminders or recurring requests and retains execution history. It covers only the internal Metnos scheduler."),
        ("Un'attività programmata esegue una richiesta a un'ora stabilita o al verificarsi di un evento; non è un appuntamento di calendario né un'automazione remota, che appartiene ad Automazioni remote.",
         "A task executes a request at a time or trigger; it is neither a calendar appointment nor a remote automation, which belongs to Remote automations."),
    ...
    "issues": _copy("collaboration", ("Issue", "Issues"),
        ...
        ("Nel repository metnos trova le issue aperte con etichetta bug create nell'ultima settimana.", "Crea una issue nel repository acme/app con questo titolo e assegnala a chi segue il rilascio."),
        ("In the metnos repository, find open issues labelled bug created in the last week.", "Create an issue in acme/app with this title and assign it to the release owner.")),
    ...
    "comments": _copy("collaboration", ("Commenti", "Comments"),
        ("Scrive e cancella commenti sugli elementi di discussione di un fornitore: segnalazioni e proposte di modifica.",
         "Writes and deletes comments on a provider's discussion items: issues and pull requests."),
        ("Un commento non è posta: non ha destinatario, cartella né consegna. Approvare o chiedere modifiche a una proposta è invece un verdetto e appartiene a Pull request.",
         "A comment is not mail: it has no recipient, folder, or delivery. Approving or requesting changes on a pull request is a verdict instead, and belongs to Pull requests."),
        ("Scrivi sulla issue 42 di acme/app che il problema si presenta anche sulla versione nuova.", "Cancella il commento che ho lasciato per sbaglio sulla proposta 7."),
        ("On issue 42 of acme/app, write that the problem also shows on the new version.", "Delete the comment I left by mistake on pull request 7.")),
    "workflows": _copy("collaboration", ("Automazioni remote", "Remote automations"),
        ("Avvia le automazioni già presenti in un repository ed elenca le loro esecuzioni con esito, ramo e durata.",
         "Starts automations already present in a repository and lists their runs with outcome, branch, and duration."),
        ("È distinto dalle attività programmate, che sono lo scheduler interno di Metnos, e dai processi, che sono software in esecuzione su una macchina.",
         "It is distinct from scheduled tasks, which are the internal Metnos scheduler, and from processes, which are software running on a machine."),
        ("Avvia l'automazione ci.yml di acme/app sul ramo principale.", "Mostrami le ultime esecuzioni fallite delle automazioni di acme/app."),
        ("Start the ci.yml automation of acme/app on the main branch.", "Show the latest failed automation runs of acme/app.")),
...
<script defer src="/assets/wiki-shell.js?v=20260822-2"></script>
<style>
