...
#
# Il certificato del server di register.it è rilasciato per *.securemail.pro:
...
SMTP_HOST="${MYKLEOS_SMTP_HOST:-authsmtp.securemail.pro}"
SMTP_PORT="${MYKLEOS_SMTP_PORT:-465}"
...
curl -sS -k --max-time 30 \
    ...
    --user "${MYKLEOS_MAIL_USER}:${MYKLEOS_MAIL_PASS}" \
    --mail-from "$MYKLEOS_MAIL_USER" \
    --mail-rcpt "$TO" \
    --upload-file "$TMP_MSG"
