#!/usr/bin/env bash
# Controlla la posta in arrivo della casella user@example.com (example.com).
# Credenziali lette da ~/.config/account_personal/mail.env (chmod 600), non da ambiente
...
#
# Il certificato di imap.example.com ? rilasciato per *.example.com (alias
...
if [ ! -f "$ENV_FILE" ]; then
    ...
    echo "Crealo con account_personal_MAIL_USER, account_personal_MAIL_PASS, account_personal_MAIL_HOST, account_personal_MAIL_PORT (chmod 600)." >&2
    exit 1
fi
...
account_personal_MAIL_HOST="${account_personal_MAIL_HOST:-imap.example.com}"
account_personal_MAIL_PORT="${account_personal_MAIL_PORT:-993}"

URL="imaps://${account_personal_MAIL_HOST}:${account_personal_MAIL_PORT}/INBOX"
...
status_of() {
    ...
    curl -sS -k --max-time 10 --url "$URL" \
        -u "${account_personal_MAIL_USER}:${account_personal_MAIL_PASS}" \
...
# --- esecuzione --------------------------------------------------------
echo "Casella: ${account_personal_MAIL_USER}  (host ${account_personal_MAIL_HOST})"
echo
