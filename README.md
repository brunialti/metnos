...
### Tutor: product knowledge in the product

Ask Metnos, for example: “Where can I see the configured models, and how do I
change them?” Tutor answers product questions in natural language, gives an
actionable UI path, and links to the official documentation when available. It
explains capabilities and procedures but does not perform an operation by
itself. In a mixed request, it can answer the explanatory part and, only after
confirmation, hand the user's literal action clause to the ordinary engine.

Tutor is compiled locally from the public documentation, signed manifests, and
current runtime registries into a verified semantic catalog. Internal
development notes are never knowledge sources. Documentation changes invalidate
the catalog; the installer rebuilds it during its third phase, before service
startup, so its guidance stays aligned with the installed product.

An exact published filename, relative path, or canonical URL identifies a
documentation source before semantic retrieval; a read or summary is then bound
to that document, while a modification remains an operational request. Signed
sources may also request bounded read-only observations of admitted executors,
service health, the current user's devices, or that actor's scheduled tasks.
Feedback can refine source retrieval for the same user without storing the
clear-text question, affecting the planner, or changing another user's answers.
...
The reference inventory is generated from the currently signed manifests
rather than copied into this README as a count that can go stale. Every listed
executor declares the same Executor Standard; transport is not a Composer
...
After installation, verify the managed target and the HTTP endpoint:
...
The health response proves reachability, not a complete turn. Finish the check
in the web chat with a harmless request such as “What time is it, and which time
zone are you using?”
