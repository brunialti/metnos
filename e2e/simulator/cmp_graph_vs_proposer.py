...
def _llm_fast(sys_msg, user_msg, *, max_tokens=80, think=False, **_):
    try:
        r = LLMRouter().provider("fast").chat(sys_msg, user_msg, max_tokens=max_tokens, think=think)
...
def _llm_wise(sys_msg, user_msg, *, max_tokens=2048, think=True, **kw):
    try:
        ...
        r = LLMRouter().provider(tier).chat(sys_msg, user_msg, max_tokens=max_tokens, think=think)
        return (getattr(r, "text", r) or "").strip()
    except Exception:
        return ""
