...
differenza unica = presenza skill importata:
  ...
  B) imported_server: builtin + 21 executor `*_google_workspace` importati
...
_QUERIES = [
    ...
    ("mail_recent",     "leggi le 3 mail piu' recenti",          "it", "mail"),
    ("mail_search",     "cerca mail con oggetto fattura",         "it", "mail"),
...
def _import_google_skill(env: dict, tmp_root: Path) -> None:
    ...
    if r.returncode != 0:
        raise RuntimeError(f"skill import failed in pre-spawn: {r.stdout[-1000:]}")
...
async def test_compare_query(builtin_client, imported_client,
    ...
    issues = []
    if not lr_b.ok:
        issues.append(f"BUILTIN lint fail: {lr_b.fail_message()}")
    if not lr_i.ok:
        issues.append(f"IMPORTED lint fail: {lr_i.fail_message()}")
    if issues:
        pytest.fail("\n".join(issues))
    ...
    out_dir = _REPO_ROOT / "e2e" / "reports"
