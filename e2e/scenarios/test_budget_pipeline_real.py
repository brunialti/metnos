...
Skip se:
  ...
  - bollette vere assenti (/opt/metnos/UTENZE E SPESE/)
...
_BILLS_DIR = Path("/opt/metnos/UTENZE E SPESE")
...
def _extract_pdf_text(pdf_path: str, *, min_native_chars: int = 100
    ...
    ocr_executor = Path("/opt/metnos/executors/read_files_ocr/read_files_ocr.py")
    r = subprocess.run(
...
def _llm_extract(focused_text: str) -> dict:
    ...
    §7.9 limitato (LLM è probabilistico, ma temperature=0)."""
    ...
    out, _ = call_llm(focused_text, prompt,
                      tier="middle", max_tokens=200, temperature=0.0)
    cleaned = out.strip()
