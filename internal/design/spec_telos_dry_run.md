...
- **Orchestratore** `runtime/telos_introspect.py`: `run_for_telos(telos, *, catalog, llm_invoke, lenses, operators, persist=True)` `:255`; `run_all_telos(*, persist=True)` `:409`; `_persist(record)` `:78` (**qui vivono i filtri**: `rejected_targets()` `:96-99`, dedup `(target,lens)` `:103-106`); `_llm_invoke_local` `:219` (bypassa LLMRouter, va sul Qwen locale `:8080`). CLI `:434-477` (`--persist`, `--mock-llm`).
