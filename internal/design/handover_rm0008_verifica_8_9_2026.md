...
> **Aggiornamento incidente dopo reboot, sera dell'8 settembre:** il successo
> F4 descritto sotto è storico, non lo stato attuale. HTTP è inattivo. Il
> ripristino della directory volatile è stato applicato, ma la certificazione
> rifiuta il Python aggiornato e, separatamente, il digest della configurazione
> systemd. Il tentativo conservativo con Python precedente non ha riattivato
> Metnos: tutti gli otto pacchetti sono stati riportati alla .16 iniziale.
> Autenticazione amministrativa disponibile; nessun reset o aggiramento delle
> firme eseguito. Prove e proposta di correzione del confine di manutenzione:
> `internal/reports/reboot-failure-20260908.md`. Non rilanciare il vecchio cutover.
> Il ramo pubblico di correzione test `codex/rm0008-ci-repair`, HEAD `41c6774`,
> ha superato tutte le verifiche del run `34284018003` alle 00:14 del 9 settembre:
> Ubuntu 2.103 test e 7 prove Linux delegate; Windows 1.430; sei attività 2A
> e riepilogo verdi. Pubblicato su main dopo nuovo GII senza rilievi.
> Questo NON ripristina il server. Alle 00:05 del 9 settembre HTTP è inattivo,
> `metnos-telegram-daemon.service` è fallito e il modello risponde correttamente.
...
`internal/reports/publication-gii-20260908.md`. Le correzioni del publisher
sono nel worktree `/tmp/metnos-rm0008-legacy-catalog-fix`; non usare le
vecchie copie in `/opt/metnos` o il wrapper temporaneo `publish-final.sh`.
