...
#### 6.9.3 Il caso scoperto: dove si affronta, e che cosa serve a RM-0001

Se la via per identità di sorgente lascia una lacuna documentata, la risposta **non**
è l'immersione del testo in un vettore, ma una **firma composita tipizzata**
dell'oggetto: un insieme dichiarato di tratti — tipologia, struttura, sorgente,
argomento, richiesta, tono — in larga maggioranza calcolati e non inferiti, con
vocabolari chiusi dichiarati dai domini. Con tratti così, «simile» smette di essere
una distanza e diventa un'**uguaglianza su una proiezione dichiarata**: niente
vettori, niente soglie, niente indice denso, e le classi si derivano dal corpus per
conteggio invece che per clustering.

La direzione per esteso — le famiglie di tratti, il reticolo delle proiezioni, la
normalità relativa al contesto, i rischi e le due regole di sicurezza (il canale
entra sempre, il tono non basta mai da solo) — sta in
`internal/design/spec_object_signature_28_7.md`, perché **descrive gli oggetti e
non l'utente**: è una capacità dei domini che quegli oggetti li producono, e la sua
ratifica è una ADR di dominio, non questa roadmap.

**Che cosa RM-0001 le chiede, e nient'altro:**

- una firma **stabile** per lo stesso oggetto, **versionata**, così che un cambio
  di estrattore invalidi le firme precedenti invece di mescolarle;
- **ispezionabile e cancellabile**: una riga di valori tipizzati che può comparire
  nell'inventario dell'utente e sparire con un oblio;
- una **proiezione dichiarata** a cui agganciare un giudizio, così che l'ampiezza
  della generalizzazione sia una scelta esplicita e mostrabile.

Ciò che resta di RM-0001 è solo il **giudizio**: il tuo segno su una firma, con
principale, provenienza, evidenza, contesto e cancellazione. Se la firma diventasse
un modo per descrivere *te* invece delle cose che guardi, saremmo tornati al
profilo narrativo di §6.7.1 per un'altra strada.
