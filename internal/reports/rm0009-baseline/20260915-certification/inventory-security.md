# RM-0009 — inventario sicurezza ricontrollato

Fonte immutabile: `09a58c7cf33e2610ce1506b129db3b3d4a37b735`. L’inventario è stato calcolato due volte sugli stessi 2411 blob tracciati selezionati; path, hash e strutture parse sono identici (`21a34b7ea58055e4f2268fb69e34498a5119a758c74532b05192d85904531014` in entrambe le passate). Il working tree, le copie installate e ogni dato runtime reale sono esclusi.
...
## Delta esatto rispetto a `1c308922839f7659a3cf54d988f995bba0f215d6`

- Input selezionati: 2407 → 2411 (`+4`); aggiunti 4, rimossi 0, modificati 127, invariati 2280.
- Aggiunti: `tests/internal/test_rm0009_inventory_check.py`, `tests/internal/test_rm0009_plan_check.py`, `tests/internal/test_rm0009_test_lab.py`, `tests/runtime/infra/test_project_license.py`.
- Modificati: 118 cambiano soltanto l’identificatore SPDX; 9 sono ancora legati alla migrazione di licenza: `install/bootstrap.sh`, `install/disclaimer.py`, `install/i18n.py`, i tre manifest built-in `describe_images`/`list_skills`/`set_skills`, `runtime/contract_boundary_guard.py`, `runtime/executor_birth_admin_preflight.py`, `scripts/publish-public.sh`. L’elenco esatto dei 127 path è nel JSON.
- I 25 nuovi file preparatori sotto `internal/reports/rm0009-baseline/` sono classificati tutti come blob tracciati esclusi: stanno fuori dalle sei radici selezionate, nessun path contiene `fixture` e nessuno parte da `tests/data`. I JSON della baseline non entrano per la sola estensione. L’elenco esatto è nel JSON.
- Risultati strutturali invariati: 109 manifest, 400 casi, 33 manifest legacy, 113 casi legacy, 24 fixture strutturati, 13 gruppi FS-B e 10 percorsi F6. Errori di parse/scansione: 0.
- Determinismo: due passate indipendenti via `git ls-tree` + `git cat-file`; entrambi i digest sono `21a34b7ea58055e4f2268fb69e34498a5119a758c74532b05192d85904531014`.
...
## Ricontrollo semantico sul nuovo commit

- FS-A: nessun manifest legacy o fixture strutturato è cambiato. I tre manifest built-in modificati aggiornano soltanto il digest del codice dopo SPDX; `executor_birth_functional.py` cambia soltanto SPDX. Restano gli stessi 33 ID e la stessa regola: casi e asserzioni dentro l’isolamento Birth; pytest/runner host non sostituiscono quella prova.
- FS-B: i nove file consumer modificati cambiano soltanto SPDX. Nelle radici protette, l’unica modifica ulteriore è il pin di revisione sorgente in `executor_birth_admin_preflight.py`, conseguente alla licenza. Restano gli stessi 13 ID e il confine proposto: il segreto resta nel core, i consumer ricevono handle opachi per invocazione.
- F6: tutti i 13 file unici dei dieci percorsi sono byte-identici alla fonte precedente. Non esiste ancora l’autorità RM-0009; rischi di bypass e doppio conteggio restano aperti.
- Deriva semantica RM-0009 effettiva: nessuna. Le variazioni semantiche del sorgente riguardano presentazione MIT, digest conseguenti e pin di revisione, non implementano FS-A, FS-B o F6.

## Limiti

Nessun runner legacy o Birth è stato eseguito; nessun servizio, vault, chiave, database/log runtime, provider o copia installata è stato letto. Gli hash coprono soltanto lo scope dichiarato nel JSON. Questo è un inventario, non una certificazione, prova di completezza o approvazione: FS-A e FS-B restano non implementati; G0.5 richiede work manifest per tutti gli stessi ID, prove isolate e verifica release/installed dove prevista.
