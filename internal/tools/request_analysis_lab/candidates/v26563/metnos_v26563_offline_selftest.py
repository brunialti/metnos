#!/usr/bin/env python3
"""Offline author tests for V26.5.6.3; every HTTP exchange is in memory."""
...
EVALUATOR_PATH = HERE / "metnos_v26563_offline_evaluator.py"
BASELINE_SELFTEST = (
    REPOSITORY / "internal/tools/request_analysis_lab/candidates/v26562/"
    "metnos_v26562_offline_selftest.py"
)
...
def main() -> None:
    ...
    runner = load_module(RUNNER_PATH, "metnos_v26563_runner_selftest")
    evaluator = load_module(EVALUATOR_PATH, "metnos_v26563_evaluator_selftest")
    tests: list[str] = []

    baseline = run_cli(BASELINE_SELFTEST)
    check(baseline.returncode == 0 and not baseline.stderr, "V26562 baseline failed")
    baseline_result = json.loads(baseline.stdout)
    check(
        baseline_result.get("status") == "PASS"
        and baseline_result.get("total_asserted_tests") == 50
        and baseline_result.get("network_calls") == 0
        and baseline_result.get("model_calls") == 0,
        "V26562 inherited 50/50 proof failed",
    )
    tests.append("inherited_v26562_50_of_50")
    ...
    with tempfile.TemporaryDirectory(
        prefix=".v26563-gate-reachability-", dir=gate_test_parent,
    ) as temporary:
        root = Path(temporary)
    ...
    with tempfile.TemporaryDirectory(prefix="v26563-anchor-") as temporary:
        root = Path(temporary)
    ...
    with tempfile.TemporaryDirectory(prefix="v26563-preflight-") as temporary:
    ...
    with tempfile.TemporaryDirectory(prefix="v26563-cli-") as temporary:
    ...
    nonisolated_probe = (
        ...
        "s=importlib.util.spec_from_file_location('v26563_nonisolated',p);"
    ...
    tests.append("pre_gold_8_bypasses_and_boundaries_zero_gold_reads")

    with tempfile.TemporaryDirectory(prefix="v26563-cap-") as temporary:
        root = Path(temporary)
        small = {"unicode": "é\\n\"", "keys": ["a", "b"]}
    ...
    result = {
        "version": "metnos.v26.5.6.3-offline-author-selftest/1.0",
        "status": "PASS",
        "inherited_v26562_passed": 50,
        "new_tests": len(tests),
        "new_passed": len(tests),
        "total_asserted_tests": 50 + len(tests),
        "test_names": tests,
