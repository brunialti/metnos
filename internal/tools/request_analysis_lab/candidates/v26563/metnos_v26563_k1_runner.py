#!/usr/bin/env python3
"""Compact V26.5.6.3 K1/34 runner, frozen with inference disabled.
...
VERSION = "metnos.v26.5.6.3-compact-k1-runner/1.0"
HERE = Path(__file__).resolve(strict=True).parent
REPOSITORY = HERE.parents[4]
RUNNER_PATH = HERE / "metnos_v26563_k1_runner.py"
OFFLINE_EVALUATOR_PATH = HERE / "metnos_v26563_offline_evaluator.py"
SELFTEST_PATH = HERE / "metnos_v26563_offline_selftest.py"
...
PREFLIGHT_GATE_PATH = HERE / "metnos_v26563_preflight_gate.lock.json"
...
def _facade() -> Any:
    ...
    if _FACADE_MODULE is None:
        _FACADE_MODULE = _load_runtime_module(
            "facade_v2655", "metnos_v26563_pinned_facade",
        )
...
def _transport_smoke(endpoint: str, opener: Any | None = None) -> dict[str, Any]:
    ...
    return {
        "version": "metnos.v26.5.6.3-transport-preflight/1.0",
...
def _validate_external_preflight(
    ...
    fixed = {
        ...
        "preflight_gate_authority": "independent_v26563_preflight_gate_verifier",
    }
...
def _verify_preflight_gate_snapshot(
    ...
    try:
        ...
        raise RuntimeError(
            "V26.5.6.3 preflight gate is absent; author transport=false",
        ) from error
    ...
    if type(gate) is not dict or set(gate) != required:
        raise RuntimeError("V26.5.6.3 preflight gate envelope is not closed")
    expected = {
        "version": "metnos.v26.5.6.3-preflight-gate/1.0",
        "status": "authorized_transport_preflight_only",
        "authority": "independent_v26563_preflight_gate_verifier",
    ...
    for key, value in expected.items():
        if gate.get(key) != value:
            raise RuntimeError(f"V26.5.6.3 preflight gate mismatch: {key}")
...
def _verify_external_gate_snapshot(
    ...
    try:
        ...
        raise RuntimeError(
            "V26.5.6.3 external gate is absent; author inference=false",
        ) from error
    ...
    if type(gate) is not dict or set(gate) != required:
        raise RuntimeError("V26.5.6.3 external gate envelope is not closed")
    expected = {
        "version": "metnos.v26.5.6.3-external-gate/1.0",
    ...
    for key, expected_value in expected.items():
        if gate.get(key) != expected_value:
            raise RuntimeError(f"V26.5.6.3 external gate field mismatch: {key}")
    if gate.get("authority") != "independent_v26563_infra_review_and_gate_verifier":
        raise RuntimeError("V26.5.6.3 external gate authority mismatch")
...
def _verify_author_bytes() -> tuple[dict[str, Any], dict[str, str]]:
    ...
    if (
        ...
        raise RuntimeError("V26.5.6.3 requires /usr/bin/python3 -I -B and Unicode 15.0.0")
    ...
    expected_gate = {
        ...
        "lock_version": "metnos.v26.5.6.3-author-pre-gate/1.0",
        ...
        "required_external_gate_path": (
            "internal/tools/request_analysis_lab/candidates/v26563/"
            "metnos_v26563_external_gate.lock.json"
        ),
        "required_preflight_gate_path": (
            "internal/tools/request_analysis_lab/candidates/v26563/"
            "metnos_v26563_preflight_gate.lock.json"
        ),
    ...
    if (
        ...
        raise RuntimeError("V26.5.6.3 author gate/freeze unexpectedly authorizes work")
    actual_pins = {
        identity: {
    ...
    for key, value in expected_fields.items():
        if freeze.get(key) != value:
            raise RuntimeError(f"V26.5.6.3 freeze pin mismatch: {key}")
    return freeze, {
...
def _verify_author_gates_absent() -> tuple[dict[str, Any], dict[str, str]]:
    ...
    for absent_path, label in (
        ...
        try:
            ...
            raise RuntimeError(f"V26.5.6.3 {label} must be absent at author checkpoint")
