#!/usr/bin/env python3
"""Compact V26.5.6.2 K1/34 runner, frozen with inference disabled.
...
VERSION = "metnos.v26.5.6.2-compact-k1-runner/1.0"
HERE = Path(__file__).resolve(strict=True).parent
REPOSITORY = HERE.parents[4]
RUNNER_PATH = HERE / "metnos_v26562_k1_runner.py"
OFFLINE_EVALUATOR_PATH = HERE / "metnos_v26562_offline_evaluator.py"
SELFTEST_PATH = HERE / "metnos_v26562_offline_selftest.py"
FREEZE_PATH = HERE / "metnos_v26562_author.freeze.json"
AUTHOR_GATE_PATH = HERE / "metnos_v26562_author_pre_gate.json"
EXTERNAL_GATE_PATH = HERE / "metnos_v26562_external_gate.lock.json"
PREFLIGHT_GATE_PATH = HERE / "metnos_v26562_preflight_gate.lock.json"
...
def _facade() -> Any:
    ...
    if _FACADE_MODULE is None:
        _FACADE_MODULE = _load_runtime_module(
            "facade_v2655", "metnos_v26562_pinned_facade",
        )
...
def _transport_smoke(endpoint: str, opener: Any | None = None) -> dict[str, Any]:
    ...
    return {
        "version": "metnos.v26.5.6.2-transport-preflight/1.0",
...
def _validate_external_preflight(
    ...
    fixed = {
        ...
        "preflight_gate_authority": "independent_v26562_preflight_gate_verifier",
    }
...
def _verify_preflight_gate_snapshot(
    ...
    try:
        ...
        raise RuntimeError(
            "V26.5.6.2 preflight gate is absent; author transport=false",
        ) from error
    ...
    if type(gate) is not dict or set(gate) != required:
        raise RuntimeError("V26.5.6.2 preflight gate envelope is not closed")
    expected = {
        "version": "metnos.v26.5.6.2-preflight-gate/1.0",
        "status": "authorized_transport_preflight_only",
        "authority": "independent_v26562_preflight_gate_verifier",
    ...
    for key, value in expected.items():
        if gate.get(key) != value:
            raise RuntimeError(f"V26.5.6.2 preflight gate mismatch: {key}")
...
def _verify_external_gate_snapshot(
    ...
    try:
        ...
        raise RuntimeError(
            "V26.5.6.2 external gate is absent; author inference=false",
        ) from error
    ...
    if type(gate) is not dict or set(gate) != required:
        raise RuntimeError("V26.5.6.2 external gate envelope is not closed")
    expected = {
        "version": "metnos.v26.5.6.2-external-gate/1.0",
    ...
    for key, expected_value in expected.items():
        if gate.get(key) != expected_value:
            raise RuntimeError(f"V26.5.6.2 external gate field mismatch: {key}")
    if gate.get("authority") != "independent_v26562_infra_review_and_gate_verifier":
        raise RuntimeError("V26.5.6.2 external gate authority mismatch")
...
def _verify_author_bytes() -> tuple[dict[str, Any], dict[str, str]]:
    ...
    if (
        ...
        raise RuntimeError("V26.5.6.2 requires /usr/bin/python3 -I -B and Unicode 15.0.0")
    ...
    evaluator_bytes = _fixed_repo_snapshot(
        OFFLINE_EVALUATOR_PATH, 2 * 1024 * 1024,
    )
    ...
    expected_gate = {
        ...
        "lock_version": "metnos.v26.5.6.2-author-pre-gate/1.0",
        "model_calls_before_checkpoint": 0,
        "network_calls_before_checkpoint": 0,
        "offline_evaluator_allowed": True,
        "preflight_gate_required": True,
        "preflight_gate_status": "absent",
        "required_external_gate_path": (
            "internal/tools/request_analysis_lab/candidates/v26562/"
            "metnos_v26562_external_gate.lock.json"
        ),
        "required_preflight_gate_path": (
            "internal/tools/request_analysis_lab/candidates/v26562/"
            "metnos_v26562_preflight_gate.lock.json"
        ),
    ...
    if (
        ...
        raise RuntimeError("V26.5.6.2 author gate/freeze unexpectedly authorizes work")
    actual_pins = {
        identity: {
    ...
    for key, value in expected_fields.items():
        if freeze.get(key) != value:
            raise RuntimeError(f"V26.5.6.2 freeze pin mismatch: {key}")
    return freeze, {
...
def _verify_author_gates_absent() -> tuple[dict[str, Any], dict[str, str]]:
    ...
    for absent_path, label in (
        ...
        try:
            ...
            raise RuntimeError(f"V26.5.6.2 {label} must be absent at author checkpoint")
