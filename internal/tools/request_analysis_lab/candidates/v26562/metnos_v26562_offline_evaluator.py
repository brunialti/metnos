#!/usr/bin/env python3
"""Offline, post-batch Phase-1 evaluator for a V26.5.6.2 model artifact.
...
VERSION = "metnos.v26.5.6.2-offline-phase1-evaluator/1.0"
HERE = Path(__file__).resolve(strict=True).parent
REPOSITORY = HERE.parents[4]
SELF_PATH = HERE / "metnos_v26562_offline_evaluator.py"
...
def _verify_execution_context() -> None:
    ...
    if (
        ...
        raise RuntimeError(
            "V26.5.6.2 evaluator requires /usr/bin/python3 -I -B, "
...
def _load_pre_gold_validator(pins: dict[str, Any]) -> tuple[Any, Any]:
    ...
    validator = types.ModuleType("metnos_v26562_pre_gold_validator")
...
def _checkpoint_before_gold(batch: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if native:
        ...
        if (
            ...
            != "independent_v26562_infra_review_and_gate_verifier"
        ):
...
def _validate_body_diagnostic(value: Any) -> None:
    keys = {
        "captured_bytes", "captured_sha256", "body_complete", "declared_bytes",
        "body_bytes", "body_sha256",
    }
    if type(value) is not dict or set(value) != keys:
        raise RuntimeError("batch response-body diagnostic is not closed")
    declared = value["declared_bytes"]
    if (
        not _nonnegative_integer(value["captured_bytes"])
        or not _is_sha256(value["captured_sha256"])
        or value["body_complete"] is not True
        or (declared is not None and not _nonnegative_integer(declared))
        or not _nonnegative_integer(value["body_bytes"])
        or not _is_sha256(value["body_sha256"])
        or value["captured_bytes"] != value["body_bytes"]
        or value["captured_sha256"] != value["body_sha256"]
        or (declared is not None and declared != value["body_bytes"])
    ):
        raise RuntimeError("batch response-body values are inconsistent")
...
def _validate_closed_diagnostic(diagnostic: Any) -> dict[str, int]:
    ...
    _validate_body_diagnostic(diagnostic["response_body"])
...
def _validate_inline_preflight(preflight: Any) -> dict[str, int]:
    ...
    if (
        preflight.get("version") != "metnos.v26.5.6.2-transport-preflight/1.0"
...
def _validate_batch_before_gold(
    ...
    if (
        batch.get("version") != "metnos.v26.5.6.2-compact-k1-runner/1.0"
    ...
    if (
        ...
        or summary.get("inline_preflight_counters") != preflight_counters
        or any(
...
def evaluate_batch_bytes(batch_bytes: bytes, batch_label: str = "<memory>") -> dict[str, Any]:
    ...
    evaluator = types.ModuleType("metnos_v26562_pinned_phase1_evaluator_offline")
