#!/usr/bin/env python3
"""Clause-owned V26.5.7.1 K1/34 runner, frozen with inference disabled.
...
VERSION = "metnos.v26.5.7.1-clause-owned-k1-runner/1.0"
HERE = Path(__file__).resolve(strict=True).parent
REPOSITORY = HERE.parents[4]
RUNNER_PATH = HERE / "metnos_v26571_k1_runner.py"
OFFLINE_EVALUATOR_PATH = HERE / "metnos_v26571_offline_evaluator.py"
SELFTEST_PATH = HERE / "metnos_v26571_offline_selftest.py"
...
PREFLIGHT_GATE_PATH = HERE / "metnos_v26571_preflight_gate.lock.json"
...
ARTIFACT_PINS: dict[str, tuple[Path, str, int]] = {
    "registry_projection_v26570": (
        _rp("internal/tools/request_analysis_lab/candidates/v26570/metnos_v26570_registry_projection.py"),
        "3f18906b2e3fd56584de0d582680742171670892ecdc569cf6881fee55f42b95", 26363,
    ),
    "expander_v26566": (
    ...
    "clause_owned_schema": (
        _rp("internal/tools/request_analysis_lab/candidates/v26570/metnos_v26570_relation_typed.schema.json"),
        "8a9845fa3ca6805f2fa1db182184b3b1c81bce45c906e70778f0548331bd06f2", 111165,
    ),
    "clause_owned_prompt": (
        _rp("internal/tools/request_analysis_lab/candidates/v26570/metnos_v26570_relation_typed.prompt.txt"),
        "19c7a5e811319b39cff3c2c8cef6e6eb0ffe3f2cc2e67daf68b83f1eeb4cd3e4", 8125,
    ),
    "v26570_freeze": (
        _rp("internal/tools/request_analysis_lab/candidates/v26570/metnos_v26570_author.freeze.json"),
        "28a10af072bbcea811c327e028fc85f03d5bc07a6ccafbab8c2c850580501bfc", 2068,
    ),
    "v26570_selftest_result": (
        _rp("internal/tools/request_analysis_lab/candidates/v26570/metnos_v26570_author_selftest_result.json"),
        "e8f936ebf3ea6f25deaa0e3d2d3bd202c4a89e765ed0f3b553bfdfab4b4c2c73", 5118,
    ),
    "runtime_controls34": (
...
def _contract() -> dict[str, Any]:
    ...
    projection = _load_runtime_module(
        "registry_projection_v26570", "metnos_v26571_pinned_projection",
    )
    expander = _load_runtime_module(
        "expander_v26566", "metnos_v26571_pinned_expander",
    )
    pipeline = _load_runtime_module(
        "pipeline_v26566", "metnos_v26571_pinned_pipeline",
    )
...
def request_body_bytes(query: str) -> bytes:
    # NOT sort_keys. The server builds its grammar from the schema in the order
    # the properties appear, so sorting them puts `arguments` before
    # `relation` and forces the model to commit to a relation branch blind.
    # That single detail cost the V26.5.6.9 live: 31 cases out of 34 collapsed
    # onto one wrong relation. Insertion order is deterministic here, so the
    # request stays byte-reproducible without sorting.
    return json.dumps(
...
def _transport_smoke(endpoint: str, opener: Any | None = None) -> dict[str, Any]:
    ...
    return {
        "version": "metnos.v26.5.7.1-transport-preflight/1.0",
...
def _validate_external_preflight(
    ...
    fixed = {
        ...
        "preflight_gate_authority": "user_authorized_v26571_preflight_gate_verifier",
    }
...
def _verify_preflight_gate_snapshot(
    ...
    try:
        ...
        raise RuntimeError(
            "V26.5.7.1 preflight gate is absent; author transport=false",
        ) from error
    ...
    if type(gate) is not dict or set(gate) != required:
        raise RuntimeError("V26.5.7.1 preflight gate envelope is not closed")
    expected = {
        "version": "metnos.v26.5.7.1-preflight-gate/1.0",
        "status": "authorized_transport_preflight_only",
        "authority": "user_authorized_v26571_preflight_gate_verifier",
    ...
    for key, value in expected.items():
        if not _exact_json_equal(gate.get(key), value):
            raise RuntimeError(f"V26.5.7.1 preflight gate mismatch: {key}")
...
def _verify_external_gate_snapshot(
    ...
    try:
        ...
        raise RuntimeError(
            "V26.5.7.1 external gate is absent; author inference=false",
        ) from error
    ...
    if type(gate) is not dict or set(gate) != required:
        raise RuntimeError("V26.5.7.1 external gate envelope is not closed")
    expected = {
    ...
    for key, expected_value in expected.items():
        if not _exact_json_equal(gate.get(key), expected_value):
            raise RuntimeError(f"V26.5.7.1 external gate field mismatch: {key}")
    if gate.get("authority") != "user_authorized_v26571_infra_review_and_gate_verifier":
        raise RuntimeError("V26.5.7.1 external gate authority mismatch")
...
def _verify_author_bytes() -> tuple[dict[str, Any], dict[str, str]]:
    ...
    if (
        ...
        raise RuntimeError("V26.5.7.1 requires /usr/bin/python3 -I -B and Unicode 15.0.0")
    ...
    expected_gate = {
        ...
        "lock_version": "metnos.v26.5.7.1-author-pre-gate/1.0",
        ...
        "required_external_gate_path": (
            "internal/tools/request_analysis_lab/candidates/v26571/"
            "metnos_v26571_external_gate.lock.json"
        ),
        "required_preflight_gate_path": (
            "internal/tools/request_analysis_lab/candidates/v26571/"
            "metnos_v26571_preflight_gate.lock.json"
        ),
    ...
    if (
        ...
        raise RuntimeError("V26.5.7.1 author gate/freeze unexpectedly authorizes work")
    actual_pins = {
        identity: {
    ...
    for key, value in expected_fields.items():
        if freeze.get(key) != value:
            raise RuntimeError(f"V26.5.7.1 freeze pin mismatch: {key}")
    return freeze, {
...
def _verify_author_gates_absent() -> tuple[dict[str, Any], dict[str, str]]:
    ...
    for absent_path, label in (
        ...
        try:
            ...
            raise RuntimeError(f"V26.5.7.1 {label} must be absent at author checkpoint")
