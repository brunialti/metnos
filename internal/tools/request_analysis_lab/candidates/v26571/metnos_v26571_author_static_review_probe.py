#!/usr/bin/env python3
"""V26.5.7.1 author static review probe: attempts to falsify the bundle.
...
metnos_v26571_author_static_review_probe.py [--json <path>]
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    runner = load(RUNNER_PATH, "v26571_runner_review")
    evaluator = load(EVALUATOR_PATH, "v26571_evaluator_review")
    selftest = load(SELFTEST_PATH, "v26571_selftest_review")
    ...
    for label, action in (
        ("preflight", lambda: runner.run_preflight(
            ENDPOINT, Path("/tmp/metnos_v26571_review_should_not_exist.json"),
    ...
    if Path("/tmp/metnos_v26571_review_should_not_exist.json").exists():
        finding("G2", "a blocked action still wrote its output",
                "/tmp/metnos_v26571_review_should_not_exist.json exists")
    ...
    freeze = json.loads((HERE / "metnos_v26571_author.freeze.json").read_text("utf-8"))
    integrity = {
    ...
    payload = {
        "version": "metnos.v26.5.7.1-author-static-review-probe/1.0",
        "reviewer": "author",
        "independent": False,
