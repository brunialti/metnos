#!/usr/bin/env python3
"""Offline author tests for the V26.5.7.1 clause-owned runner.
...
EVALUATOR_PATH = HERE / "metnos_v26571_offline_evaluator.py"
CONTRACT_DIR = HERE.parent / "v26570"
BASELINE_SELFTEST = (
...
def main() -> None:
    ...
    runner = load_module(RUNNER_PATH, "metnos_v26571_runner_selftest")
    tests: list[str] = []
    ...
    with tempfile.TemporaryDirectory(
        prefix=".v26571-gate-reachability-", dir=gate_test_parent,
    ) as temporary:
        root = Path(temporary)
    ...
    materialised_schema = json.loads(
        (CONTRACT_DIR / "metnos_v26570_relation_typed.schema.json").read_text("utf-8"))
    materialised_prompt = (
        CONTRACT_DIR / "metnos_v26570_relation_typed.prompt.txt").read_text("utf-8")
    ...
    evaluator = load_module(EVALUATOR_PATH, "metnos_v26571_evaluator_selftest")
    ...
    with tempfile.TemporaryDirectory(prefix="v26571-anchor-") as temporary:
        root = Path(temporary)
    ...
    with tempfile.TemporaryDirectory(prefix="v26571-preflight-") as temporary:
    ...
    with tempfile.TemporaryDirectory(prefix="v26571-live-") as temporary:
    ...
    with tempfile.TemporaryDirectory(prefix="v26571-cli-") as temporary:
    ...
    for identity in ("expander_v26566", "pipeline_v26566", "clause_owned_schema",
                     "clause_owned_prompt", "registry_projection_v26570"):
    ...
    with tempfile.TemporaryDirectory(prefix="v26571-cap-") as temporary:
        root = Path(temporary)
        small = {"unicode": "é\\n\"", "keys": ["a", "b"]}
    ...
    result = {
        "version": "metnos.v26.5.7.1-offline-author-selftest/1.0",
        "status": "PASS",
