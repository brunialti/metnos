#!/usr/bin/env python3
"""Offline, post-batch Phase-1 evaluator for a V26.5.6.4 model artifact.
...
VERSION = "metnos.v26.5.6.4-offline-phase1-evaluator/1.0"
HERE = Path(__file__).resolve(strict=True).parent
REPOSITORY = HERE.parents[4]
SELF_PATH = HERE / "metnos_v26564_offline_evaluator.py"
...
def _verify_execution_context() -> None:
    ...
    if (
        ...
        raise RuntimeError(
            "V26.5.6.4 evaluator requires /usr/bin/python3 -I -B, "
...
def _load_pre_gold_validator(pins: dict[str, Any]) -> tuple[Any, Any]:
    ...
    validator = types.ModuleType("metnos_v26564_pre_gold_validator")
...
def _checkpoint_before_gold(batch: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if native:
        ...
        if (
            ...
            != "independent_v26564_infra_review_and_gate_verifier"
        ):
    ...
    if path[:2] == ("records", "[]"):
        ...
        if len(result_path) == 1:
            ...
            if key == "failure_class":
                return (str, type(None))
            if key in {"facade_latency_ms", "total_latency_ms"}:
                return (float,)
        ...
        if len(result_path) == 2 and result_path[0] == "validation":
            if result_path[1] == "attempted":
                return (bool,)
            if result_path[1] == "stage":
                return (str, type(None))
        if (
            len(result_path) == 3
            and result_path[:2] == ("validation", "codes")
            and result_path[2] == "[]"
        ):
            return (str,)
...
def _validate_inline_preflight(preflight: Any) -> dict[str, int]:
    ...
    if (
        preflight.get("version") != "metnos.v26.5.6.4-transport-preflight/1.0"
...
def _validate_batch_before_gold(
    ...
    if (
        batch.get("version") != "metnos.v26.5.6.4-compact-k1-runner/1.0"
    ...
    for ordinal, (record, control) in enumerate(zip(records, controls), 1):
        ...
        valid_result_keys = common_result_keys | {"expanded_frame", "facade_latency_ms"}
        content_invalid_result_keys = common_result_keys
        facade_invalid_result_keys = common_result_keys | {"facade_latency_ms"}
        if (
...
def evaluate_batch_bytes(batch_bytes: bytes, batch_label: str = "<memory>") -> dict[str, Any]:
    ...
    evaluator = types.ModuleType("metnos_v26564_pinned_phase1_evaluator_offline")
