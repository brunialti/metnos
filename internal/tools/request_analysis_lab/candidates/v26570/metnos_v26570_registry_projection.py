#!/usr/bin/env python3
"""V26.5.7.0 relation-typed projection of the frozen typed registry.
...
Serialisation is part of this contract: see materialise_schema. The schema is
written in insertion order, discriminant first, because the server generates
the grammar in the order the properties appear.
...
VERSION = "metnos.v26.5.7.0-registry-projection/1.0"
...
def materialise_schema(schema: dict[str, Any]) -> str:
    """Serialise the schema WITHOUT reordering its keys.

    This is not cosmetic. A server turns the schema into a grammar and emits
    an object's properties in the order the schema lists them. Sorting the
    keys puts `arguments` before `relation`, which forces the model to write
    an argument list before it has chosen the relation -- and with one branch
    per relation it must then commit blind. That is exactly what happened in
    the V26.5.6.9 live of 10 August: the model picked acl.share in 31 cases
    out of 34, against 34 out of 34 correct relations under the previous, flat
    schema. The discriminant must be serialised first, so insertion order is
    part of the contract.
    """
    return json.dumps(schema, ensure_ascii=False, allow_nan=False,
                      indent=1) + "\n"
