#!/usr/bin/env python3
"""V26.5.7.0 offline self-test: the relation-typed contract.
...
metnos_v26570_offline_selftest.py [--json <path>]
"""
...
VERSION = "metnos.v26.5.7.0-offline-selftest/1.0"
SEED = 20260810
...
SCHEMA_PATH = HERE / "metnos_v26570_relation_typed.schema.json"
PROMPT_PATH = HERE / "metnos_v26570_relation_typed.prompt.txt"
...
def main(argv: list[str] | None = None) -> int:
    ...
    projection = _load("v26570_projection", HERE / "metnos_v26570_registry_projection.py")
    expander = _load("v26570_expander", PREDECESSOR / "metnos_v26566_expander.py")
    pipeline = _load("v26570_pipeline", PREDECESSOR / "metnos_v26566_offline_pipeline.py")
    ...
    materialised_bytes = SCHEMA_PATH.read_text(encoding="utf-8")
    check(group, "the schema is serialised in generator order, not sorted",
          materialised_bytes == projection.materialise_schema(schema))
    # the discriminant must be written before what depends on it: a server
    # generates the grammar in property order, and V26.5.6.9 collapsed because
    # "arguments" preceded "relation"
    out_of_order = []
    for name, definition in schema["$defs"].items():
        for branch in definition.get("oneOf", [definition]):
            properties = list(branch.get("properties", {}))
            for discriminant in ("relation", "kind", "status", "clause_role"):
                if discriminant in properties:
                    for dependent in ("arguments", "ref", "proof", "clauses",
                                      "projection", "readings"):
                        if dependent in properties and properties.index(dependent) \
                                < properties.index(discriminant):
                            out_of_order.append(f"{name}: {dependent} before {discriminant}")
                    break
    check(group, "every discriminant is serialised before what depends on it",
          not out_of_order, "; ".join(out_of_order[:3]))
