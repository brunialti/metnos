#!/usr/bin/env python3
"""Offline author tests for the V26.5.6.7 clause-owned runner.
...
EVALUATOR_PATH = HERE / "metnos_v26567_offline_evaluator.py"
CONTRACT_DIR = HERE.parent / "v26566"
BASELINE_SELFTEST = (
        ...
        if self.post_calls in self.inner_json_invalid_at:
            ...
            if self.post_calls in self.hostile_at:
                ...
                if self.post_calls in self.semantic_break_at:
                    frame["clauses"][0]["projection"]["speech_act"] = "assertion"
            content = json.dumps(
...
def main() -> None:
    ...
    runner = load_module(RUNNER_PATH, "metnos_v26567_runner_selftest")
    tests: list[str] = []
    ...
    with tempfile.TemporaryDirectory(
        prefix=".v26567-gate-reachability-", dir=gate_test_parent,
    ) as temporary:
        root = Path(temporary)
    ...
    materialised_schema = json.loads(
        (CONTRACT_DIR / "metnos_v26566_clause_owned.schema.json").read_text("utf-8"))
    materialised_prompt = (
        CONTRACT_DIR / "metnos_v26566_clause_owned.prompt.txt").read_text("utf-8")
    ...
    evaluator = load_module(EVALUATOR_PATH, "metnos_v26567_evaluator_selftest")
    ...
    with tempfile.TemporaryDirectory(prefix="v26567-anchor-") as temporary:
        root = Path(temporary)
    ...
    with tempfile.TemporaryDirectory(prefix="v26567-preflight-") as temporary:
    ...
    with tempfile.TemporaryDirectory(prefix="v26567-live-") as temporary:
    ...
    with tempfile.TemporaryDirectory(prefix="v26567-cli-") as temporary:
    ...
    for identity in ("expander_v26566", "pipeline_v26566", "clause_owned_schema",
                     "clause_owned_prompt", "registry_projection_v26566"):
    ...
    with tempfile.TemporaryDirectory(prefix="v26567-cap-") as temporary:
        root = Path(temporary)
        small = {"unicode": "é\\n\"", "keys": ["a", "b"]}
    ...
    result = {
        "version": "metnos.v26.5.6.7-offline-author-selftest/1.0",
        "status": "PASS",
