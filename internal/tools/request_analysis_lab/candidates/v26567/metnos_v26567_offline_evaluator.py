#!/usr/bin/env python3
"""Offline, post-batch Phase-1 evaluator for a V26.5.6.7 model artifact.
...
VERSION = "metnos.v26.5.6.7-offline-phase1-evaluator/1.0"
HERE = Path(__file__).resolve(strict=True).parent
REPOSITORY = HERE.parents[4]
SELF_PATH = HERE / "metnos_v26567_offline_evaluator.py"
...
FIXED_ARTIFACT_PATHS = {
    "clause_owned_schema": (
        ...
        "metnos_v26566_clause_owned.schema.json"
    ),
    "clause_owned_prompt": (
        "internal/tools/request_analysis_lab/candidates/v26566/"
        "metnos_v26566_clause_owned.prompt.txt"
    ),
    "pipeline_v26566": (
        "internal/tools/request_analysis_lab/candidates/v26566/"
...
def _verify_execution_context() -> None:
    ...
    if (
        ...
        raise RuntimeError(
            "V26.5.6.7 evaluator requires /usr/bin/python3 -I -B, "
...
def _load_pre_gold_validator(pins: dict[str, Any]) -> tuple[Any, Any]:
    ...
    validator = types.ModuleType("metnos_v26567_pre_gold_validator")
...
def _checkpoint_before_gold(batch: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if native:
        ...
        if (
            ...
            != "user_authorized_v26567_infra_review_and_gate_verifier"
        ):
...
def _validate_inline_preflight(preflight: Any) -> dict[str, int]:
    ...
    if (
        preflight.get("version") != "metnos.v26.5.6.7-transport-preflight/1.0"
...
def _validate_batch_before_gold(
    ...
    if (
        batch.get("version") != "metnos.v26.5.6.7-clause-owned-k1-runner/1.0"
...
def evaluate_batch_bytes(batch_bytes: bytes, batch_label: str = "<memory>") -> dict[str, Any]:
    ...
    pipeline = types.ModuleType("metnos_v26567_pinned_pipeline_offline")
    ...
    evaluator = types.ModuleType("metnos_v26567_pinned_phase1_evaluator_offline")
