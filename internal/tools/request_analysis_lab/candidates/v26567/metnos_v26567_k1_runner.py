#!/usr/bin/env python3
"""Clause-owned V26.5.6.7 K1/34 runner, frozen with inference disabled.
...
VERSION = "metnos.v26.5.6.7-clause-owned-k1-runner/1.0"
HERE = Path(__file__).resolve(strict=True).parent
REPOSITORY = HERE.parents[4]
RUNNER_PATH = HERE / "metnos_v26567_k1_runner.py"
OFFLINE_EVALUATOR_PATH = HERE / "metnos_v26567_offline_evaluator.py"
SELFTEST_PATH = HERE / "metnos_v26567_offline_selftest.py"
FREEZE_PATH = HERE / "metnos_v26567_author.freeze.json"
AUTHOR_GATE_PATH = HERE / "metnos_v26567_author_pre_gate.json"
EXTERNAL_GATE_PATH = HERE / "metnos_v26567_external_gate.lock.json"
PREFLIGHT_GATE_PATH = HERE / "metnos_v26567_preflight_gate.lock.json"
...
ARTIFACT_PINS: dict[str, tuple[Path, str, int]] = {
    "registry_projection_v26566": (
        _rp("internal/tools/request_analysis_lab/candidates/v26566/metnos_v26566_registry_projection.py"),
        "7b1af2e41fba32d2129327f0423d10ad19135d17569dba54f8a01f445ef10f7d", 22319,
    ),
    "expander_v26566": (
    ...
    "clause_owned_schema": (
        _rp("internal/tools/request_analysis_lab/candidates/v26566/metnos_v26566_clause_owned.schema.json"),
        "72b2739d676d0c35c21d8a9add383a58f90b30d291c499532f680d5e902ce0b9", 11771,
    ),
    "clause_owned_prompt": (
        _rp("internal/tools/request_analysis_lab/candidates/v26566/metnos_v26566_clause_owned.prompt.txt"),
        "803721b9bff01159de0ce163aa03161c3128eccfff232b97faea7d812c7731db", 9336,
    ),
    "v26566_freeze": (
        _rp("internal/tools/request_analysis_lab/candidates/v26566/metnos_v26566_author.freeze.json"),
        "a2d88bc18eddf4e766b42a2c5cba41e1fe2e3d37a5038ae89f4eb3eb4c1f1f3e", 5020,
    ),
    "v26566_selftest_result": (
        _rp("internal/tools/request_analysis_lab/candidates/v26566/metnos_v26566_author_selftest_result.json"),
        "b41989edfb2ef37722b6affdd730841da625fb67d37031b9c24ba13cb590a9e9", 13634,
    ),
    "runtime_controls34": (
...
def _contract() -> dict[str, Any]:
    ...
    projection = _load_runtime_module(
        "registry_projection_v26566", "metnos_v26567_pinned_projection",
    )
    expander = _load_runtime_module(
        "expander_v26566", "metnos_v26567_pinned_expander",
    )
    pipeline = _load_runtime_module(
        "pipeline_v26566", "metnos_v26567_pinned_pipeline",
    )
...
def request_body(query: str, seed: int = FIXED_SEED) -> dict[str, Any]:
    ...
    return {
        ...
        "response_format": {
            "type": "json_schema",
            "json_schema": {
                "name": "phase1_typed_relational_v26566_clause_owned",
                "schema": schema,
                "strict": True,
            },
        },
...
def _transport_smoke(endpoint: str, opener: Any | None = None) -> dict[str, Any]:
    ...
    return {
        "version": "metnos.v26.5.6.7-transport-preflight/1.0",
...
def _validate_external_preflight(
    ...
    fixed = {
        ...
        "preflight_gate_authority": "user_authorized_v26567_preflight_gate_verifier",
    }
...
def _verify_preflight_gate_snapshot(
    ...
    try:
        ...
        raise RuntimeError(
            "V26.5.6.7 preflight gate is absent; author transport=false",
        ) from error
    ...
    if type(gate) is not dict or set(gate) != required:
        raise RuntimeError("V26.5.6.7 preflight gate envelope is not closed")
    expected = {
        "version": "metnos.v26.5.6.7-preflight-gate/1.0",
        "status": "authorized_transport_preflight_only",
        "authority": "user_authorized_v26567_preflight_gate_verifier",
    ...
    for key, value in expected.items():
        if not _exact_json_equal(gate.get(key), value):
            raise RuntimeError(f"V26.5.6.7 preflight gate mismatch: {key}")
...
def _verify_external_gate_snapshot(
    ...
    try:
        ...
        raise RuntimeError(
            "V26.5.6.7 external gate is absent; author inference=false",
        ) from error
    ...
    if type(gate) is not dict or set(gate) != required:
        raise RuntimeError("V26.5.6.7 external gate envelope is not closed")
    expected = {
    ...
    for key, expected_value in expected.items():
        if not _exact_json_equal(gate.get(key), expected_value):
            raise RuntimeError(f"V26.5.6.7 external gate field mismatch: {key}")
    if gate.get("authority") != "user_authorized_v26567_infra_review_and_gate_verifier":
        raise RuntimeError("V26.5.6.7 external gate authority mismatch")
...
def _verify_author_bytes() -> tuple[dict[str, Any], dict[str, str]]:
    ...
    if (
        ...
        raise RuntimeError("V26.5.6.7 requires /usr/bin/python3 -I -B and Unicode 15.0.0")
    ...
    expected_gate = {
        ...
        "lock_version": "metnos.v26.5.6.7-author-pre-gate/1.0",
        ...
        "required_external_gate_path": (
            "internal/tools/request_analysis_lab/candidates/v26567/"
            "metnos_v26567_external_gate.lock.json"
        ),
        "required_preflight_gate_path": (
            "internal/tools/request_analysis_lab/candidates/v26567/"
            "metnos_v26567_preflight_gate.lock.json"
        ),
    ...
    if (
        ...
        raise RuntimeError("V26.5.6.7 author gate/freeze unexpectedly authorizes work")
    actual_pins = {
        identity: {
    ...
    for key, value in expected_fields.items():
        if freeze.get(key) != value:
            raise RuntimeError(f"V26.5.6.7 freeze pin mismatch: {key}")
    return freeze, {
...
def _verify_author_gates_absent() -> tuple[dict[str, Any], dict[str, str]]:
    ...
    for absent_path, label in (
        ...
        try:
            ...
            raise RuntimeError(f"V26.5.6.7 {label} must be absent at author checkpoint")
