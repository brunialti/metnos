#!/usr/bin/env python3
"""V26.4.1 runner-only transport fix for the V26.4 semantic candidate.
...
VERSION = "metnos.v26.4.1-typed-relational-phase1/1.0"
RUNNER_PATH = Path(__file__)
REGISTRY_PATH = Path("/tmp/metnos_v2641_typed_registry.json")
SCHEMA_PATH = Path("/tmp/metnos_v2641_typed_phase1.schema.json")
PROMPT_PATH = Path("/tmp/metnos_v2641_typed_phase1.prompt.txt")
FIXTURE_PATH = Path("/tmp/metnos_v2641_typed_phase1_controls_frozen.json")
MUTATIONS_PATH = Path("/tmp/metnos_v2641_typed_phase1_mutations_frozen.json")
CONTAMINATION_PATH = Path("/tmp/metnos_v2641_typed_phase1_contamination_audit.json")
FREEZE_PATH = Path("/tmp/metnos_v2641_typed_phase1.freeze.json")
AUTHOR_PRE_GATE_PATH = Path("/tmp/metnos_v2641_typed_phase1_author_pre_gate.json")
EXTERNAL_GATE_PATH = Path("/tmp/metnos_v2641_typed_phase1_external_gate.lock.json")
PREFLIGHT_GATE_PATH = Path("/tmp/metnos_v2641_transport_preflight.lock.json")
MODEL_OUTPUT_PATH = Path("/tmp/metnos_v2641_typed_phase1_controls_k1.json")
PREFLIGHT_OUTPUT_PATH = Path("/tmp/metnos_v2641_transport_preflight_result.json")
INFRA_MUTATION_PROBE_PATH = Path("/tmp/metnos_v2641_infra_mutation_probe.py")
INFRA_MUTATION_PROBE_RESULT_PATH = Path("/tmp/metnos_v2641_infra_mutation_probe_result.json")
...
PARENT_FREEZE_PATH = Path("/tmp/metnos_v264_typed_phase1.freeze.json")
K1_DIAGNOSIS_MD_PATH = Path("/opt/metnos/internal/tools/request_analysis_lab/candidates/v264/metnos_v264_k1_not_evaluated_diagnosis.md")
K1_DIAGNOSIS_JSON_PATH = Path("/opt/metnos/internal/tools/request_analysis_lab/candidates/v264/metnos_v264_k1_not_evaluated_diagnosis.json")
EXPECTED_K1_DIAGNOSIS_HASHES = {
    K1_DIAGNOSIS_MD_PATH: "a16b47e024ed030736588cd3e55e0a3a5e436aa61ede692604454ee91bb51a3b",
    K1_DIAGNOSIS_JSON_PATH: "8105a843c101476012122db8c3b42ffda10b1e1706740ad56dadccd188f1120d",
}

MAX_SUCCESS_BODY_BYTES = 8 * 1024 * 1024
MAX_ERROR_BODY_BYTES = 1024 * 1024
MAX_REDACTED_EXCERPT_CHARS = 320

SEMANTIC_PARENT_HASHES = {
    REGISTRY_PATH: "448c058d805e141c871580253e815849b24a9999d98c505cab970fcd55d1e46f",
    SCHEMA_PATH: "06452e75f0a1ec7a42c9bf87dfae3da4e1528fc07b02d9276272a1e9ff0c7fec",
    PROMPT_PATH: "2e60f19500b6d6193d53f483707533ca39ec1fee6a0256b18e6a96994f53406f",
    FIXTURE_PATH: "1dfbc5d2a7a83308a0f414d1b2c0b395f096d034471930fbaa886627aa79d954",
    PARENT_FREEZE_PATH: "83d3b4441d83d8f90e5eb591f9eeaf052885150e6806cd3ceefd046d948ccde6",
}
...
def _redact_diagnostic_text(value: str, sensitive_values: tuple[str, ...] = ()) -> str:
    text = value.replace("\x00", " ")
    for secret in sorted({item for item in sensitive_values if item}, key=len, reverse=True):
        marker = f"<redacted-input:{sha_text(secret)[:12]}>"
    ...
    text = regex.sub(r"(?i)\bBearer\s+[^\s,;]+", "Bearer <redacted>", text)
    text = regex.sub(
        r'''(?i)(["']?(?:original_request|query|prompt|messages|content|cookie|set-cookie)["']?\s*[:=]\s*)
            (?:"(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'|[^,;\s}]+)''',
        r"\1<redacted>", text, flags=regex.X,
    )
    text = regex.sub(r"\b[^\s@]+@[^\s@]+\.[^\s@]+\b", "<redacted-email>", text)
    text = regex.sub(r"([?&](?:key|token|secret|password)=)[^&#\s]+", r"\1<redacted>", text, flags=regex.I)
    text = regex.sub(
        r"\b[A-Za-z0-9_+/=-]{24,}\b",
        lambda match: f"<redacted-token:{sha_text(match.group(0))[:12]}>",
        text,
    )
    return text[:MAX_REDACTED_EXCERPT_CHARS]


def _exception_chain(exc: BaseException, sensitive_values: tuple[str, ...] = ()) -> list[dict[str, Any]]:
    ...
    while current is not None and id(current) not in seen and len(chain) < 6:
        ...
        item: dict[str, Any] = {
            ...
            "message_redacted": _redact_diagnostic_text(str(current), sensitive_values),
        }
        errno_value = getattr(current, "errno", None)
        if isinstance(errno_value, int):
            item["errno"] = errno_value
        chain.append(item)
...
def _exception_diagnostic(exc: BaseException, sensitive_values: tuple[str, ...] = ()) -> dict[str, Any]:
    chain = _exception_chain(exc, sensitive_values)
    errno_value = next(
        (item["errno"] for item in chain if isinstance(item.get("errno"), int)),
        None,
    )
    return {
...
def _declared_body_length(headers: Any) -> int | None:
    try:
...
def _body_diagnostic(
    body: bytes, *, complete: bool, declared_bytes: int | None,
    include_excerpt: bool, sensitive_values: tuple[str, ...] = (),
) -> dict[str, Any]:
    ...
    if include_excerpt:
        diagnostic["excerpt_redacted"] = _redact_diagnostic_text(
            body.decode("utf-8", "replace"), sensitive_values,
        )
    return diagnostic
...
def _request_bytes_once(
    ...
    try:
        ...
        diagnostic["http_responses"] = 1
        diagnostic["request_reached_server"] = True
        diagnostic["http_status"] = response.getcode()
        if isinstance(diagnostic["http_status"], int) and 200 <= diagnostic["http_status"] < 300:
            diagnostic["server_accepted_requests"] = 1
        declared = _declared_body_length(getattr(response, "headers", None))
        try:
            body, complete = _read_bounded(response, success_limit)
        finally:
            response.close()
        diagnostic["response_body"] = _body_diagnostic(
            body, complete=complete, declared_bytes=declared,
            include_excerpt=not complete, sensitive_values=sensitive_values,
        )
        if not complete:
        ...
        if not isinstance(status, int) or not 200 <= status < 300:
            diagnostic["response_body"] = _body_diagnostic(
                body, complete=True, declared_bytes=declared, include_excerpt=True,
        ...
        try:
            try:
                ...
                diagnostic.update({
                    ...
                    "http_status": exc.code, "response_body": None,
                    **_exception_diagnostic(read_exc, sensitive_values),
                })
                return None, diagnostic
        finally:
            exc.close()
        diagnostic.update({
            ...
            "response_body": _body_diagnostic(
                ...
                declared_bytes=_declared_body_length(exc.headers),
                include_excerpt=True, sensitive_values=sensitive_values,
            ),
        ...
        diagnostic["latency_ms"] = (time.perf_counter() - started) * 1000
...
def call_once(query: str, endpoint: str, seed: int = 92) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    try:
        ...
        diagnostic.update({
            ...
            **_exception_diagnostic(exc, (query,)),
            "response_body": _body_diagnostic(
                body, complete=True, declared_bytes=len(body), include_excerpt=True,
                sensitive_values=(query,),
            ),
        })
        return {}, diagnostic
    ...
    try:
        ...
        if not isinstance(frame, dict):
            raise TypeError("model frame is not an object")
    except Exception as exc:
        diagnostic.update({
            ...
            **_exception_diagnostic(exc, (query,)),
            "model_content": {
                ...
                "sha256": sha_text(raw),
                "excerpt_redacted": _redact_diagnostic_text(raw, (query,)),
            },
        })
        return {}, diagnostic
...
def _verify_hashed_reference(lock: dict[str, Any], prefix: str, errors: list[str]) -> None:
    ...
    try:
        ...
        valid = path.is_file() and sha(path) == hash_value
...
def _validate_external_gate_payload(
    lock: dict[str, Any], *, endpoint: str | None = None,
    ...
    expected_scalars = {
        ...
        "registry_sha256": SEMANTIC_PARENT_HASHES[REGISTRY_PATH],
        "schema_sha256": SEMANTIC_PARENT_HASHES[SCHEMA_PATH],
        "prompt_sha256": SEMANTIC_PARENT_HASHES[PROMPT_PATH],
        "fixture_sha256": SEMANTIC_PARENT_HASHES[FIXTURE_PATH],
        "parent_freeze_sha256": SEMANTIC_PARENT_HASHES[PARENT_FREEZE_PATH],
        "oracle_freeze_sha256": EXPECTED_ORACLE_HASHES[ORACLE_FREEZE_PATH],
        "authorized_case_count": 34,
        "authorized_repetitions": 1,
        "authorized_output_path": str(MODEL_OUTPUT_PATH),
        ...
        "transport_preflight_result_path": str(PREFLIGHT_OUTPUT_PATH),
        "retries": 0,
    ...
    required_keys = set(expected_scalars) | {
        "authorized_endpoint",
        "independent_review_path", "independent_review_sha256",
        "oracle_review_path", "oracle_review_sha256",
        "verifier_path", "verifier_sha256",
        "transport_preflight_result_sha256",
    }
    ...
    for key, expected in expected_scalars.items():
        if lock.get(key) != expected:
            errors.append(key)
    authorized_endpoint = lock.get("authorized_endpoint")
    if not isinstance(authorized_endpoint, str) or not _valid_local_endpoint(authorized_endpoint):
        errors.append("authorized_endpoint")
    elif endpoint is not None and endpoint.rstrip("/") != authorized_endpoint.rstrip("/"):
        errors.append("endpoint_mismatch")
    for prefix in ("independent_review", "oracle_review", "verifier"):
        _verify_hashed_reference(lock, prefix, errors)
    preflight_hash = lock.get("transport_preflight_result_sha256")
    try:
        preflight_path = Path(expected_scalars["transport_preflight_result_path"])
        preflight_valid = (
            isinstance(preflight_hash, str)
            and preflight_path.is_file()
            and sha(preflight_path) == preflight_hash
        )
    except (OSError, ValueError):
        preflight_valid = False
    if not preflight_valid:
        errors.append("transport_preflight_result")
    else:
        try:
            result = json.loads(preflight_path.read_text())
            completed_at_ns = result.get("completed_at_unix_ns")
            age_ms = (
                (time.time_ns() - completed_at_ns) / 1_000_000
                if isinstance(completed_at_ns, int) else None
            )
            if (
                result.get("status") != "PASS"
                or result.get("inference_calls") != 0
                or result.get("freeze_sha256") != expected_scalars["freeze_sha256"]
                or result.get("runner_sha256") != expected_scalars["runner_sha256"]
                or not isinstance(authorized_endpoint, str)
                or result.get("endpoint", "").rstrip("/") != authorized_endpoint.rstrip("/")
                or result.get("method") != "GET"
                or result.get("request_body_bytes") != 0
                or result.get("authorizes_inference") is not False
                or age_ms is None or age_ms < 0
                or age_ms > expected_scalars["external_transport_preflight_max_age_at_gate_verification_ms"]
            ):
                errors.append("transport_preflight_result_contract")
        except Exception:
            errors.append("transport_preflight_result_json")
    if require_output_absent and MODEL_OUTPUT_PATH.exists():
        errors.append("authorized_output_already_exists")
    return errors


def verify_external_gate(endpoint: str | None = None) -> dict[str, Any]:
    if not EXTERNAL_GATE_PATH.is_file():
        raise RuntimeError("V26.4.1 external independent gate lock is missing")
    lock = json.loads(EXTERNAL_GATE_PATH.read_text())
    errors = _validate_external_gate_payload(lock, endpoint=endpoint)
    if errors:
        raise RuntimeError(f"V26.4.1 external gate rejected: {sorted(set(errors))}")
    return lock


def _validate_preflight_gate_payload(
    lock: dict[str, Any], *, endpoint: str | None = None,
    require_output_absent: bool = True,
) -> list[str]:
    errors: list[str] = []
    expected_scalars = {
        ...
        "inference_calls": 0,
        "authorized_output_path": str(PREFLIGHT_OUTPUT_PATH),
        "network_calls_before_lock": 0,
    }
    required_keys = set(expected_scalars) | {
        "authorized_endpoint", "authorized_url",
        "independent_review_path", "independent_review_sha256",
        "verifier_path", "verifier_sha256",
    }
    if set(lock) != required_keys:
    ...
    authorized_endpoint = lock.get("authorized_endpoint")
    expected_url = (
        authorized_endpoint.rstrip("/") + "/v1/models"
        if isinstance(authorized_endpoint, str) else None
    )
    if not isinstance(authorized_endpoint, str) or not _valid_local_endpoint(authorized_endpoint):
        errors.append("authorized_endpoint")
    elif endpoint is not None and endpoint.rstrip("/") != authorized_endpoint.rstrip("/"):
        errors.append("endpoint_mismatch")
    if lock.get("authorized_url") != expected_url:
        errors.append("authorized_url")
    for prefix in ("independent_review", "verifier"):
        _verify_hashed_reference(lock, prefix, errors)
    if require_output_absent and PREFLIGHT_OUTPUT_PATH.exists():
        errors.append("authorized_output_already_exists")
    return errors


def verify_preflight_gate(endpoint: str | None = None) -> dict[str, Any]:
    if not PREFLIGHT_GATE_PATH.is_file():
        raise RuntimeError("V26.4.1 transport preflight lock is missing")
    lock = json.loads(PREFLIGHT_GATE_PATH.read_text())
    errors = _validate_preflight_gate_payload(lock, endpoint=endpoint)
    if errors:
        raise RuntimeError(f"V26.4.1 transport preflight gate rejected: {sorted(set(errors))}")
    return lock


def _counters(
    diagnostic: dict[str, Any], *, validation_attempts: int = 0,
    ...
    return {
        ...
        "preflight_json_documents_decoded": int(diagnostic.get("preflight_json_documents_decoded", 0)),
...
def _transport_smoke_once(endpoint: str) -> dict[str, Any]:
    request = urllib.request.Request(
        endpoint.rstrip("/") + "/v1/models", headers={"Accept": "application/json"},
        method="GET",
    )
    body, diagnostic = _request_bytes_once(request, timeout_s=10, success_limit=1024 * 1024)
    diagnostic.update({
        "response_json_documents_decoded": 0,
        "decoded_chat_responses": 0, "decoded_frames": 0,
        "preflight_json_documents_decoded": 0,
    })
    status = "PASS"
    if body is None:
        ...
        try:
            ...
            if not isinstance(payload, (dict, list)):
                raise TypeError("model-list response is not an object or array")
            diagnostic["preflight_json_documents_decoded"] = 1
...
def run_preflight(endpoint: str) -> dict[str, Any]:
    verify_freeze()
    verify_preflight_gate(endpoint)
    result = _transport_smoke_once(endpoint)
    result.update({
        "version": VERSION,
        "freeze_sha256": sha(FREEZE_PATH),
        "runner_sha256": sha(RUNNER_PATH),
    })
    result.pop("completed_monotonic_ns", None)
    return result


def run_case(query: str, endpoint: str, *, gate_verified: bool = False) -> dict[str, Any]:
    if not gate_verified:
        verify_freeze()
        verify_external_gate(endpoint)
    segments = unicode_segments(query)
    frame, diagnostic = call_once(query, endpoint, 92)
    common = {
    ...
    if diagnostic["error"] == "model_frame_json_error":
        return {
            **common,
            ...
            "counters": _counters(diagnostic, evaluated_cases=1, invalid_cases=1),
            "validation": {"attempted": False, "valid": False, "error_codes": ["model_frame_json_error"]},
        }
    if diagnostic["error"]:
        return {
            **common,
            ...
            "counters": _counters(diagnostic),
            "validation": {"attempted": False, "valid": None, "error_codes": []},
        }
    validation = validate_frame(frame, segments)
    if not validation["valid"]:
        return {
            **common,
            "status": "evaluated_invalid",
            "failure_class": "model_frame_schema_or_semantic_invalid",
            "frame_sha256": canonical_hash(frame),
            "counters": _counters(
                diagnostic, validation_attempts=1, evaluated_cases=1, invalid_cases=1,
            ),
            "validation": {
                "attempted": True,
                "valid": False,
    ...
    return {
        **common,
        ...
        "frame": frame,
        "counters": _counters(
            diagnostic, validation_attempts=1, evaluated_cases=1, valid_cases=1,
        ),
        "validation": {"attempted": True, "valid": True, "error_codes": []},
    }
...
def evaluate_complete_records(
    records: list[dict[str, Any]], fixture: dict[str, Any],
) -> dict[str, Any]:
    if len(records) != len(fixture.get("cases", [])) or any(
        record.get("result", {}).get("status") not in {"evaluated", "evaluated_invalid"}
        for record in records
    ):
        raise RuntimeError("accuracy evaluation requires one model result for every frozen case")
    evaluation = evaluate_records(records, fixture)
    # V26.4 used a synthetic model_calls field.  Preserve its semantic evaluator
    # byte-for-byte, but never publish that ambiguous counter from V26.4.1.
    evaluation["summary"].pop("model_calls", None)
    return evaluation


def _aggregate_counters(records: list[dict[str, Any]]) -> dict[str, int]:
    keys = (
        "socket_attempts", "http_responses", "server_accepted_requests",
        "response_json_documents_decoded", "preflight_json_documents_decoded",
        "decoded_chat_responses", "decoded_frames",
        "validation_attempts", "evaluated_cases", "valid_cases", "invalid_cases",
    )
    return {
        key: sum(int(record["result"].get("counters", {}).get(key, 0)) for record in records)
        for key in keys
    }


def _latency_summary(records: list[dict[str, Any]]) -> dict[str, float | None]:
    latencies = [float(record["result"]["latency_ms"]) for record in records]
    if not latencies:
        return {"latency_call_p50_ms": None, "latency_call_p95_ms": None}
    if len(latencies) == 1:
        return {"latency_call_p50_ms": latencies[0], "latency_call_p95_ms": latencies[0]}
    return {
        "latency_call_p50_ms": statistics.median(latencies),
        "latency_call_p95_ms": statistics.quantiles(
...
def run_controls(endpoint: str) -> dict[str, Any]:
    verify_freeze()
    external_gate = verify_external_gate(endpoint)
    fixture = json.loads(FIXTURE_PATH.read_text())
    configured_controls = controls()
    if len(configured_controls) != len(fixture.get("cases", [])):
        raise RuntimeError("controls/fixture case-count mismatch")
    inline_preflight = _transport_smoke_once(endpoint)
    preflight_completed_ns = int(inline_preflight.pop("completed_monotonic_ns"))
    if inline_preflight["status"] != "PASS":
        return {
            "version": VERSION,
            "status": "NOT_EVALUATED",
            "freeze_sha256": sha(FREEZE_PATH),
            "summary": {
                "configured_cases": len(fixture["cases"]),
                "attempted_cases": 0,
                "evaluated_cases": 0,
                "inline_transport_preflight_counters": inline_preflight["counters"],
                "inference_transport_and_decode_counters": _aggregate_counters([]),
                "total_network_request_attempts": inline_preflight["counters"]["socket_attempts"],
                "retries": 0,
                **_latency_summary([]),
            },
            "abort": {
                "after_attempted_cases": 0,
                "failure_class": "inline_transport_preflight",
                "phase": inline_preflight["diagnostic"].get("phase"),
                "error": inline_preflight["diagnostic"].get("error"),
                "validation_error_codes": [],
            },
            "inline_transport_preflight": inline_preflight,
            "records": [], "evaluation_records": [],
            "native_run": True, "posthoc_credit": False, "accuracy_claimed": False,
        }
    preflight_age_ms = (time.monotonic_ns() - preflight_completed_ns) / 1_000_000
    inline_preflight["age_before_first_post_ms"] = preflight_age_ms
    max_age_ms = external_gate["inline_transport_preflight_max_age_before_first_post_ms"]
    if preflight_age_ms > max_age_ms:
        return {
            "version": VERSION,
            "status": "NOT_EVALUATED",
            "freeze_sha256": sha(FREEZE_PATH),
            "summary": {
                "configured_cases": len(fixture["cases"]),
                "attempted_cases": 0, "evaluated_cases": 0,
                "inline_transport_preflight_counters": inline_preflight["counters"],
                "inference_transport_and_decode_counters": _aggregate_counters([]),
                "total_network_request_attempts": inline_preflight["counters"]["socket_attempts"],
                "retries": 0, **_latency_summary([]),
            },
            "abort": {
                "after_attempted_cases": 0,
                "failure_class": "inline_transport_preflight_stale",
                "phase": "preflight_recency",
                "error": "preflight_age_exceeded",
                "validation_error_codes": [],
            },
            "inline_transport_preflight": inline_preflight,
            "records": [], "evaluation_records": [],
    ...
    for case in configured_controls:
        record = {
            "opaque_case_id": sha_text(case["id"]),
            "query_sha256_utf8": sha_text(case["query"]),
            "result": run_case(case["query"], endpoint, gate_verified=True),
        }
        records.append(record)
        if record["result"]["status"] == "NOT_EVALUATED":
            abort = {
                "at_opaque_case_id": record["opaque_case_id"],
                "after_attempted_cases": len(records),
                "failure_class": record["result"].get("failure_class"),
                "phase": record["result"].get("diagnostic", {}).get("phase"),
                "error": record["result"].get("diagnostic", {}).get("error"),
                "validation_error_codes": record["result"].get("validation", {}).get("error_codes", []),
            }
            break
    common_summary = {
        "configured_cases": len(fixture["cases"]),
        ...
        "evaluated_cases": sum(
            record["result"]["status"] in {"evaluated", "evaluated_invalid"}
            for record in records
        ),
        "inline_transport_preflight_counters": inline_preflight["counters"],
        "inference_transport_and_decode_counters": _aggregate_counters(records),
        "total_network_request_attempts": (
            inline_preflight["counters"]["socket_attempts"]
            + _aggregate_counters(records)["socket_attempts"]
        ),
        "retries": 0,
    ...
    if abort is not None:
        return {
            ...
            "evaluation_records": [],
            "native_run": True,
            "posthoc_credit": False,
            "accuracy_claimed": False,
        }
    evaluation = evaluate_complete_records(records, fixture)
    return {
        "version": VERSION,
        "status": "evaluated",
        "freeze_sha256": sha(FREEZE_PATH),
        ...
        "native_run": True, "posthoc_credit": False, "accuracy_claimed": True,
    }
...
def mutation_tests() -> dict[str, Any]:
    ...
    fake_author = {
        "lock_version": "metnos.v26.4.1-author-pre-gate/1.0", "status": "blocked",
    }
...
def mutation_bundle() -> dict[str, Any]:
    ...
    if core["summary"]["failed"]:
        raise RuntimeError("V26.4.1 core mutation suite failed")
    probe = json.loads(INDEPENDENT_PROBE_RESULT_PATH.read_text())
    expected = {
        "runner_sha256": "ea3fc46b50ce25a72652214b380e251f092ada87420a3e056b67eda60b24057d",
    ...
    infra = json.loads(INFRA_MUTATION_PROBE_RESULT_PATH.read_text())
    infra_expected = {
        "runner_sha256": sha(RUNNER_PATH),
        "network_calls": 0,
        "candidate_outputs_read": 0,
    ...
    infra_mismatches = {
        key: {"expected": value, "actual": infra.get(key)}
        for key, value in infra_expected.items() if infra.get(key) != value
    }
    if infra.get("failed") or infra.get("summary", {}).get("failed") != 0:
        infra_mismatches["probe_failures"] = infra.get("failed")
    if infra.get("summary", {}).get("tests", 0) < 20:
        infra_mismatches["probe_test_count"] = infra.get("summary", {}).get("tests")
    if infra_mismatches:
        raise RuntimeError(f"independent infra probe mismatch: {infra_mismatches}")
    return {
        "version": "metnos.v26.4.1-offline-mutation-bundle/1.0",
        "network_calls": 0,
        "candidate_outputs_read": 0,
        "core": core,
        "independent_probe": probe,
        "infra_mutation_probe": infra,
        "acceptance": {
            "core_failed": 0,
        ...
        "sources": {
            ...
            str(INFRA_MUTATION_PROBE_PATH): sha(INFRA_MUTATION_PROBE_PATH),
            str(INFRA_MUTATION_PROBE_RESULT_PATH): sha(INFRA_MUTATION_PROBE_RESULT_PATH),
        },
    }
...
def contamination_audit() -> dict[str, Any]:
    ...
    return {
        "audit_version": "metnos.v26.4.1-prompt-contamination-redacted/1.0",
...
def verify_semantic_parent_artifacts() -> None:
    changed = {
        str(path): {
            "expected": expected,
            "actual": sha(path) if path.is_file() else "missing",
        }
        for path, expected in SEMANTIC_PARENT_HASHES.items()
        if not path.is_file() or sha(path) != expected
    }
    if changed:
        raise RuntimeError(f"V26.4.1 semantic parent artifacts changed: {changed}")
...
def _derived_freeze_fields() -> dict[str, Any]:
    return {
        ...
        "transport_preflight_source_sha256": source_bundle_sha(
            _valid_local_endpoint, _verify_hashed_reference,
            _validate_preflight_gate_payload, verify_preflight_gate,
            _transport_smoke_once, run_preflight,
        ),
        "transport_diagnostic_source_sha256": source_bundle_sha(
            _redact_diagnostic_text, _exception_chain, _exception_diagnostic,
            _read_bounded, _declared_body_length, _body_diagnostic,
            _request_bytes_once, call_once, _counters, _aggregate_counters,
            _latency_summary, run_case, evaluate_complete_records, run_controls,
        ),
        "exclusive_output_writer_source_sha256": source_bundle_sha(
...
def freeze() -> dict[str, Any]:
    verify_semantic_parent_artifacts()
    ...
    for path, expected in EXPECTED_K1_DIAGNOSIS_HASHES.items():
        if not path.exists() or sha(path) != expected:
            raise RuntimeError(f"K1 diagnosis artifact mismatch: {path}")
    # Semantic V26.4 artifacts are immutable inputs.  This runner-only freeze
    # verifies their byte hashes and never regenerates them.
    AUTHOR_PRE_GATE_PATH.write_text(json.dumps({
        "version": VERSION,
        "lock_version": "metnos.v26.4.1-author-pre-gate/1.0",
        "status": "blocked_pending_independent_review_and_external_lock",
        "inference_allowed": False,
        "transport_preflight_allowed": False,
        "external_transport_preflight_required_before_live_gate": True,
        ...
        "optional_preflight_gate_path": str(PREFLIGHT_GATE_PATH),
        "semantic_parent_freeze_sha256": SEMANTIC_PARENT_HASHES[PARENT_FREEZE_PATH],
        "oracle_freeze_sha256": EXPECTED_ORACLE_HASHES[ORACLE_FREEZE_PATH],
    ...
    lock = {
        ...
        "standalone_transport_preflight_separately_gated": True,
        "external_transport_preflight_required_before_live_gate": True,
        "inline_transport_preflight_required_for_live": True,
        "author_pre_gate_inference_allowed": False,
        "author_pre_gate_transport_preflight_allowed": False,
        "socket_attempts_per_case_max": 1, "batch_fail_fast": True,
        "fail_fast_classes": ["inline_transport_preflight", "transport_or_response_protocol"],
        "model_invalid_continues_batch": True,
        "retries": 0,
        ...
        "infra_mutation_probe_sha256": sha(INFRA_MUTATION_PROBE_PATH),
        "infra_mutation_probe_result_sha256": sha(INFRA_MUTATION_PROBE_RESULT_PATH),
        "semantic_parent_artifacts": {
            str(path): expected for path, expected in SEMANTIC_PARENT_HASHES.items()
        },
        "failed_k1_diagnosis_artifacts": {
            str(path): expected for path, expected in EXPECTED_K1_DIAGNOSIS_HASHES.items()
        },
...
def verify_freeze() -> None:
    ...
    paths = {
        ...
        "infra_mutation_probe_sha256": INFRA_MUTATION_PROBE_PATH,
        "infra_mutation_probe_result_sha256": INFRA_MUTATION_PROBE_RESULT_PATH,
    }
    changed = {
        key: {"locked": lock.get(key), "actual": sha(path)}
        for key, path in paths.items() if lock.get(key) != sha(path)
    }
    expected_oracle_artifacts = {
        str(path): expected for path, expected in EXPECTED_ORACLE_HASHES.items()
    }
    if lock.get("oracle_artifacts") != expected_oracle_artifacts:
        changed["oracle_artifacts"] = {
            "locked": lock.get("oracle_artifacts"), "actual": expected_oracle_artifacts,
        }
    for path_text, expected in expected_oracle_artifacts.items():
        path = Path(path_text)
        actual = sha(path) if path.exists() else "missing"
        if actual != expected:
            changed[f"oracle:{path_text}"] = {"locked": expected, "actual": actual}
    expected_k1_artifacts = {
        str(path): expected for path, expected in EXPECTED_K1_DIAGNOSIS_HASHES.items()
    }
    if lock.get("failed_k1_diagnosis_artifacts") != expected_k1_artifacts:
        changed["failed_k1_diagnosis_artifacts"] = {
            "locked": lock.get("failed_k1_diagnosis_artifacts"),
            "actual": expected_k1_artifacts,
        }
    for path_text, expected in expected_k1_artifacts.items():
        path = Path(path_text)
        actual = sha(path) if path.exists() else "missing"
        if actual != expected:
            changed[f"K1-diagnosis:{path_text}"] = {"locked": expected, "actual": actual}
    if lock.get("semantic_parent_artifacts") != {
        str(path): expected for path, expected in SEMANTIC_PARENT_HASHES.items()
    }:
        changed["semantic_parent_artifacts"] = {
            "locked": lock.get("semantic_parent_artifacts"),
            "actual": {str(path): expected for path, expected in SEMANTIC_PARENT_HASHES.items()},
        }
    ...
    if changed:
        raise RuntimeError(f"V26.4.1 freeze changed: {changed}")


def write_json_exclusive_atomic(path: Path, payload: dict[str, Any]) -> None:
    """Publish one gate-authorized result without partial or overwrite states."""
...
def main() -> int:
    ...
    if args.controls:
        ...
        external = verify_external_gate(args.endpoint)
        ...
        write_json_exclusive_atomic(Path(args.output), result)
        print(json.dumps(result["summary"], sort_keys=True))
        if result["status"] != "evaluated":
            return 2
    if args.preflight:
        if not args.endpoint or not args.output:
            raise SystemExit("--preflight requires explicit --endpoint and --output")
        lock = verify_preflight_gate(args.endpoint)
        if args.output != lock["authorized_output_path"]:
            raise SystemExit("--output does not match preflight gate lock")
        result = run_preflight(args.endpoint)
        write_json_exclusive_atomic(Path(args.output), result)
        print(json.dumps({"status": result["status"], "counters": result["counters"]}, sort_keys=True))
