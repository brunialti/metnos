#!/usr/bin/env python3
"""V26.5.6.6 clause-owned projection of the frozen typed registry.

Pure, offline, deterministic. No network, no model, no filesystem writes.

This module is the single source of truth for the V26.5.6.6 request contract.
...
Design carried over from V26.5.6.5 (S1 of the V26.5.6.4 post mortem): clause
identity lives in the *document structure*, not in an inferred key. A clause
owns exactly one analysis and its own dependencies. The source span is demoted
to pure evidence: never a key, never unique-by-contract, never compared for
equality between atoms.

Delta of V26.5.6.6, blocker B3 of the independent V26.5.6.5 review: the
typed_ambiguity branch no longer repeats a whole clause array per alternative.
It carries ONE clause skeleton, and every projected clause of that skeleton
carries one reading per alternative. The frozen normal form keeps a single
global unsupported-clause list and compares the clause footprint of every
alternative, so alternatives that disagree on clause identity, spans, count or
out-of-registry membership were representable but never valid. They are now
UNREPRESENTABLE, which closes clause_ids, alternative_coverage, clause_limit
and alternative_order on any schema-valid ambiguity frame.

Delta N1: source_ordinal carries the registry-derived upper bound as well as
its lower bound, so the only emitted ordinal is bounded on both sides.
"""
...
VERSION = "metnos.v26.5.6.6-registry-projection/1.0"
...
def fingerprint_vocabulary(registry: dict[str, Any]) -> frozenset[str]:
    ...
    Everything else the model emits is counted, never copied (blocker B2 of
    the independent V26.5.6.5 review: on a schema-invalid frame the fingerprint
    copied model-controlled strings verbatim).
    """
    return frozenset(
...
def _binding_definition(
    *, dependency: bool, references: list[str], max_atoms: int,
) -> dict[str, Any]:
    ...
    # deterministic atom flattening defined by the clause array itself, and is
    # bounded on both sides by the frozen atom budget.
    branches.append({
        "type": "object", "additionalProperties": False,
        "properties": {
...
def build_schema(registry: dict[str, Any]) -> dict[str, Any]:
    ...
    # An analysis of one clause: exactly one projection, plus the auxiliary
    # producers that feed it. A second projection is not forbidden by prose,
    # it is unrepresentable. Dependencies are nested, so a dependency cannot
    # belong to another clause and no span equality is ever needed to attach
    # it. The per-clause dependency ceiling is the widest one that never
    # rejects a frame the frozen validator would accept.
    analysis_props = {
        **role_props,
    ...
    analysis_required = ["clause_role", "clause_role_proof", KEY_PROJECTION]

    projected_clause = {
        "type": "object", "additionalProperties": False,
        "properties": {**span_props, **analysis_props},
        "required": [SPAN_START, SPAN_END, *analysis_required],
    }
    reading = {
        "type": "object", "additionalProperties": False,
        "properties": dict(analysis_props),
        "required": list(analysis_required),
    }
    # One clause, one reading per alternative. The clause skeleton is shared,
    # so two alternatives cannot disagree on how many semantic clauses exist,
    # on their spans, or on which of them are out of registry.
    ambiguous_clause = {
        "type": "object", "additionalProperties": False,
        "properties": {
            **span_props,
    ...
    # One positional array carries every kind of clause, so clause identity is
    # the array index for every clause without exception.
    clause_array = {
    ...
    ambiguous_clause_array = {
        "type": "array", "minItems": 1, "maxItems": max_clauses,
        "items": {"oneOf": [
            {"$ref": "#/$defs/ambiguous_clause"},
            {"$ref": "#/$defs/unsupported_clause"},
        ]},
        # at least one clause carrying readings, so every alternative owns at
        # least one atom and the alternative count is at least two
    ...
    definitions = {
        ...
        "projection_binding": _binding_definition(
            dependency=False, references=references, max_atoms=max_atoms),
        "dependency_binding": _binding_definition(
            dependency=True, references=references, max_atoms=max_atoms),
        ...
        "ambiguous_clause_array": ambiguous_clause_array,
    }
...
each alternative is flattened on its own and its ordinals restart at one. Its \
output slot is derived and must be that source's dependency output or \
projection unknown. Emit clauses in source order.
...
typed_ambiguity status contains one clause array in which every projected \
clause carries one reading per alternative, in the same alternative order in \
every clause: reading one everywhere is the first alternative, reading two \
everywhere is the second. A clause whose analysis does not change repeats the \
same reading in every alternative, and an out-of-registry clause is shared by \
all of them, so the alternatives always describe the same semantic clauses. \
...
def build_prompt(registry: dict[str, Any]) -> str:
    ...
    lines.append(
        ...
        "projected clause of an ambiguous analysis; relation arity; role and "
        "speech agreement; output and adapter compatibility; proof "
        "compatibility; one consumed output per dependency; every "
        "source_ordinal points backward to a compatible producer inside the "
        "same flattening; atom budget; depth bound; complete distinct "
        "alternatives."
    )
    return "\n".join(lines) + "\n"
