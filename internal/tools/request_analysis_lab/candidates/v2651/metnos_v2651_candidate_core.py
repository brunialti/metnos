#!/usr/bin/env python3
"""Hash-closed offline core for the V26.5.1 compact candidate.
...
VERSION = "metnos.v26.5.1-candidate-core/0.1"
HERE = Path(__file__).resolve().parent
REPOSITORY = HERE.parents[4]
MANIFEST_PATH = HERE / "metnos_v2651_durable_manifest.json"
SELF_RELATIVE = (
    "internal/tools/request_analysis_lab/candidates/v2651/"
    "metnos_v2651_candidate_core.py"
)
ADAPTER_RELATIVE = (
...
def verified_manifest() -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    """Verify every manifest artifact before any candidate module executes."""
    ...
    own = entries.get(SELF_RELATIVE)
    if own is None or own.get("role") != "runtime_input" or own.get("import_allowed") is not True:
        raise RuntimeError("candidate core is not bound as an importable runtime input")
    return document, entries


def _artifact_bytes(entries: dict[str, dict[str, Any]], relative: str) -> bytes:
...
def _source_module(
    ...
    source = _artifact_bytes(entries, relative)
...
class CandidateCore:
    ...
    def load(cls) -> "CandidateCore":
        ...
        adapter = _source_module(entries, ADAPTER_RELATIVE, "metnos_v2651_core_adapter")
        validator = _source_module(
            entries, VALIDATOR_RELATIVE, "metnos_v2651_core_validator",
        )
        schema = json.loads(_artifact_bytes(entries, SCHEMA_RELATIVE))
        prompt = _artifact_bytes(entries, PROMPT_RELATIVE).decode("utf-8")
    ...
    def request_body(self, original_request: str, seed: int = 92) -> dict[str, Any]:
        ...
        return {
            ...
            "response_format": {
                "type": "json_schema",
                "json_schema": {
                    "name": "phase1_typed_relational_v2651_compact",
                    "schema": self.schema,
                    "strict": True,
                },
            },
...
def self_test() -> dict[str, Any]:
    ...
    fixture = json.loads(_artifact_bytes(core.entries, FIXTURE_RELATIVE))
    fixture_segments = [
        {"id": index}
