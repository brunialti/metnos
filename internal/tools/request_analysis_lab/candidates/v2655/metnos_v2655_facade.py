#!/usr/bin/env python3
"""Narrow V26.5.5 facade for an isolated, one-shot semantic validator.
...
def evaluate(original_request, frame):
    ...
    def add_encoded_size(amount):
        nonlocal encoded_budget
        if type(amount) is not int or amount < 0:
            raise RuntimeError("invalid exact JSON budget increment")
        encoded_budget += amount
        if encoded_budget > encoded_limit:
            raise ValueError("request exceeds worker protocol byte bound")

    def add_json_string_size(value, label):
        if type(value) is not str:
            raise TypeError(f"{label} is not an exact string")
        size = 2  # opening and closing JSON quotes
        for character in value:
            codepoint = ord(character)
            if 0xD800 <= codepoint <= 0xDFFF:
                raise ValueError(f"{label} contains a Unicode surrogate")
            if character in ('"', "\\"):
                size += 2
            elif codepoint < 0x20:
                ...
                size += 4
            if encoded_budget + size > encoded_limit:
                raise ValueError("request exceeds worker protocol byte bound")
        add_encoded_size(size)

    # Exact bytes outside the frame: envelope braces, two item commas, three
    # key/value colons, all three keys, and the two non-frame values.  Key
    # sorting changes order only, never this byte count.
    add_encoded_size(2 + 2 + 3)
    for envelope_key in ("frame", "original_request", "version"):
        add_json_string_size(envelope_key, "fixed envelope key")
    add_json_string_size(original_request, "original_request")
    add_json_string_size(request_version, "fixed protocol version")
    ...
    def exact_json_snapshot(value, depth=0):
        ...
        if value_type is bool:
            add_encoded_size(4 if value else 5)
            return value
        if value_type is int:
            if not -(2 ** 63) <= value <= 2 ** 63 - 1:
                raise ValueError("frame integer exceeds snapshot bounds")
            add_encoded_size(len(str(value)))
            return value
        if value_type is float:
            if not _math.isfinite(value):
                raise ValueError("frame contains a non-finite number")
            # Python's JSON encoder uses the same finite-float representation.
            add_encoded_size(len(repr(value)))
            return value
        if value_type is str:
            if len(value) > 200_000:
                raise ValueError("frame string exceeds snapshot bounds")
            add_json_string_size(value, "frame string")
            return value
        ...
        try:
            if value_type is list:
                ...
                for index, item in enumerate(value):
                    if index:
                        add_encoded_size(1)
                    result.append(exact_json_snapshot(item, depth + 1))
                return result
            add_encoded_size(2)
            result = {}
            for index, (key, item) in enumerate(value.items()):
                ...
                if index:
                    add_encoded_size(1)
                add_json_string_size(key, "frame object key")
                add_encoded_size(1)
    ...
    if len(request_bytes) != encoded_budget or encoded_budget > encoded_limit:
        raise RuntimeError("exact JSON budget disagrees with canonical encoding")
    ...
    expected_facade = (
        "internal/tools/request_analysis_lab/candidates/v2655/"
        "metnos_v2655_facade.py"
    )
    try:
