"""Build the complete query-free v0.3 preflight package."""
...
def authority() -> dict[str, Any]:
    return {
        "format": "metnos.intent-holdout-authority/0.3",
...
def glossary() -> dict[str, Any]:
    ...
    return {
        "format": "metnos.intent-holdout-neutral-glossary/0.3",
...
def rubrics() -> dict[str, Any]:
    return {
        "format": "metnos.intent-holdout-rubrics/0.3", "uncertain_is_fail": True,
...
def pilot_matrix() -> dict[str, Any]:
    ...
    for language, ca, sa, cb, sb in data:
        for cell, subtype in ((ca, sa), (cb, sb)):
            ...
            index += 1
    return {"format": "metnos.intent-holdout-pilot-matrix/0.3", "cases": 20, "query_per_context": 1, "slots": slots}
...
def mutation_catalog() -> dict[str, Any]:
    ...
    return {"format": "metnos.intent-holdout-mutations/0.3", "synthetic_only": True,
...
def freeze_payload(values: dict[str, Any]) -> dict[str, Any]:
    return {
        "format": "metnos.intent-holdout-process-freeze/0.3",
