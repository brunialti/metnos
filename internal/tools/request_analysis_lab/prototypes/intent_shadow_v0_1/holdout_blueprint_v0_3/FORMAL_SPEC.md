...
3. proiezione normativa v0.3 delle sole regole di binding e safety già approvate;
...
La projection contiene soltanto tuple strutturate `kind`, `author_gloss`, relazioni per indice e constraint di superficie. Per un false-action trap aggiunge il solo `tempting_but_insufficient_gloss`. Ogni gloss è una concatenazione meccanica degli scope strutturati revisionati, senza override manuali. Una route con scope mancante o contenente sintassi esecutiva è `authorable=false` e non può entrare nel pilot. Non contiene route, expected, reason, path, codice, pattern, esempi o dati di caso. Usa lo stesso percorso neutro per ogni BCP47; l'autore formula direttamente nella lingua assegnata.
...
Servono due audit indipendenti READY sul byte finale. Solo allora parte automaticamente il pilot. Un errore chiude v0.3 e richiede una nuova versione; non si rattoppa il campione.
