...
def proposal_value(proposal: Proposal) -> dict[str, Any]:
    return {
        "format": "metnos.intent-holdout-blueprint-proposal/0.3",
...
def parse_proposal(value: Any) -> Proposal:
    ...
    require(value["format"] == "metnos.intent-holdout-blueprint-proposal/0.3", "CLOSED_SCHEMA", "format")
...
def semantic_gold(proposal: Proposal, process_freeze_sha256: str) -> dict[str, Any]:
    return {
        "format": "metnos.intent-holdout-semantic-gold/0.3",
...
def projection_value(proposal: Proposal, glossary: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "format": "metnos.intent-holdout-author-projection/0.3",
        "proposal_sha256": digest(proposal_value(proposal)),
        "process_freeze_sha256": proposal.process_freeze_sha256,
        "language_tag": proposal.language_tag,
...
def author_bundle_value(projection: dict[str, Any], constitution_bytes: bytes) -> dict[str, Any]:
    return {
        "format": "metnos.intent-holdout-author-bundle/0.3",
...
def pool_a_bundle_value(proposal: Proposal, registry: dict[str, Any], glossary: dict[str, Any], rubrics: dict[str, Any]) -> dict[str, Any]:
    return {"format": "metnos.intent-holdout-pool-a-bundle/0.3", "proposal": proposal_value(proposal),
            "registry_sha256": digest(registry), "glossary_sha256": digest(glossary),
            "rubrics_sha256": digest(rubrics), "query_visible": False}


def pool_b_bundle_value(proposal: Proposal, query_bytes: bytes, registry: dict[str, Any],
                        glossary: dict[str, Any], rubrics: dict[str, Any]) -> dict[str, Any]:
    _validate_query_bytes(query_bytes)
    return {"format": "metnos.intent-holdout-pool-b-bundle/0.3", "proposal_sha256": digest(proposal_value(proposal)),
            "language_tag": proposal.language_tag, "query": query_bytes.decode("utf-8"),
            "registry_sha256": digest(registry), "glossary_sha256": digest(glossary),
            "rubrics_sha256": digest(rubrics), "proposal_visible": False}
...
def validate_pool_a(records: tuple[dict[str, Any], dict[str, Any]], proposal: Proposal, registry: dict[str, Any]) -> dict[str, Any]:
    ...
    for item in records:
        require(item["format"] == "metnos.intent-holdout-pool-a-judgment/0.3" and item["gold"] == expected,
                "GOLD_MISMATCH")
    return expected
...
def pool_b_judgment(identity: ReviewerIdentity, proposal: Proposal, query_bytes: bytes) -> dict[str, Any]:
    _validate_query_bytes(query_bytes)
    return {
        "format": "metnos.intent-holdout-pool-b-judgment/0.3",
        "reviewer": reviewer_value(identity),
...
def review_value(
    ...
    require(role in {"native_reviewer", "cross_language_reviewer"}, "LIFECYCLE_ROLE")
    return {
        "format": "metnos.intent-holdout-review/0.3",
        "role": role,
        "reviewer": reviewer_value(identity),
        "case_id": case_id,
...
def envelope_value(
    *, proposal_bytes: bytes, process_freeze_sha256: str, projection_bytes: bytes, author_bundle_bytes: bytes,
    ...
    return {
        "format": "metnos.intent-holdout-case-envelope/0.3",
...
def validate_case_artifacts(
    ...
    query_panel: dict[str, bytes], registry: dict[str, Any], glossary: dict[str, Any],
    matrix: dict[str, Any], rubrics: dict[str, Any], process_freeze_bytes: bytes,
) -> None:
    ...
    for role, expected_outputs in (("author", (query_bytes,)), ("pool_a", pool_a_bytes), ("pool_b", pool_b_bytes)):
        outputs = tuple(lifecycle_artifacts[(item.role, item.identity.context_id, item.case_id)][1]
                        for item in case_records if item.role == role)
    ...
    expected_inputs = {
        "pool_a": canonical_bytes(pool_a_bundle_value(proposal, registry, glossary, rubrics)),
        "pool_b": canonical_bytes(pool_b_bundle_value(proposal, query_bytes, registry, glossary, rubrics)),
