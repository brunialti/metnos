...
class MutationTests(unittest.TestCase):
    ...
    def mutation_cases(self):
        ...
        bad_a = (
            {"format": "metnos.intent-holdout-pool-a-judgment/0.3", "reviewer": {"reviewer_id": "a1", "context_id": "c1"}, "gold": changed_gold},
            {"format": "metnos.intent-holdout-pool-a-judgment/0.3", "reviewer": {"reviewer_id": "a2", "context_id": "c2"}, "gold": changed_gold},
        )
        ...
        collision = Proposal("bp-002", "it-IT", base.cell, base.subtype, base.obligations,
                             base.relations, base.surface_constraints, FREEZE)
        ...
        rubric_document = build.rubrics(); rubric_bytes = e.canonical_bytes(rubric_document)
        fake_freeze = e.canonical_bytes({
            "local_files": {"FORMAL_SPEC.md": e.digest_bytes(b"constitution")},
            "generated": {
                "glossary.json": {"payload_sha256": e.digest(self.glossary)},
                "pilot_matrix.json": {"payload_sha256": e.digest(self.matrix(base))},
                "rubrics.json": {"payload_sha256": e.digest(rubric_document)},
            },
        })
        malformed_parts = dict(envelope={}, proposal_bytes=b"not-json", projection_bytes=b"{}",
            author_bundle_bytes=b"{}", constitution_bytes=b"constitution", gold_bytes=b"{}", query_bytes=b"q",
            pool_a_bytes=(b"{}", b"{}"), pool_b_bytes=(b"{}", b"{}"), rubric_bytes=rubric_bytes,
            lifecycle_bytes=b"[]", lifecycle_artifacts={}, query_panel={}, registry=self.registry,
            glossary=self.glossary, matrix=self.matrix(base), rubrics=rubric_document,
            process_freeze_bytes=fake_freeze)
        ...
        return {
            ...
            "M11": lambda: e.validate_fingerprint_set((base, collision), scale="pilot"),
            ...
            "M16": lambda: e.validate_case_artifacts(**malformed_parts),
