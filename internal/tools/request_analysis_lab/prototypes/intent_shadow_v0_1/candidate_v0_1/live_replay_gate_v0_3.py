#!/usr/bin/env python3
"""General, oracle-free replay gate for offline evaluator 0.3.

The gate verifies the already completed 316-record measurement without making
network or model calls.  A saved/replayed extraction may differ only at
``adapter_metadata.implicit_actions_ignored``.  The field must exist in both
documents and both values must be exact JSON booleans.  Every tolerated
difference is retained in the report; every other difference is fatal.
"""
...
REPLAY_GATE_VERSION = "metnos.intent-shadow-live-replay-gate/0.3"
EVALUATOR_VERSION = "metnos.intent-shadow-live-evaluator/0.3"
FREEZE_FORMAT = "metnos.intent-shadow-live-replay-freeze/0.3"
CREATED_DATE = "2026-08-13"
ALLOWED_POINTER = "adapter_metadata.implicit_actions_ignored"
ALLOWED_REASON = "approved_exact_boolean_diagnostic_variance"
IDENTITY_FIELDS = (
    "request_ordinal",
    "sample_index",
    "panel",
...
FREEZE_PATH = HERE / "live_replay_v0_3.freeze.json"

# Closed, pre-gold dependencies.  Gold files and reports are deliberately
# absent.  Version-0.3 files are self-hashed separately by the freeze builder.
FROZEN_SOURCE_FILES = (
...
def _expected_freeze_policy() -> dict[str, Any]:
    return {
        ...
        "scope": "any_measurement_record",
        "field_required_in_saved_and_replay_when_tolerated": True,
        "exact_boolean_type_required": True,
        "both_boolean_directions_allowed": True,
        "all_other_fields_and_types_exact": True,
        "every_tolerance_reported": True,
        "reason": ALLOWED_REASON,
    }


def verify_v0_3_freeze(path: Path = FREEZE_PATH) -> dict[str, Any]:
    ...
    for key, expected in expected_constants.items():
        if not _exact_json_equal(freeze.get(key), expected):
            raise RuntimeError(f"v0.3 freeze constant mismatch:{key}")
    if type(freeze["source_files"]) is not dict or set(freeze["source_files"]) != set(FROZEN_SOURCE_FILES):
        raise RuntimeError("v0.3 freeze source membership mismatch")
    for relative in FROZEN_SOURCE_FILES:
        digest = freeze["source_files"].get(relative)
        source = ROOT / relative
        if type(digest) is not str or len(digest) != 64:
            raise RuntimeError(f"v0.3 freeze invalid digest:{relative}")
        if not source.is_file() or file_sha256(source) != digest:
            raise RuntimeError(f"v0.3 freeze source drift:{relative}")
    expected_self_files = {
        "live_replay_gate_v0_3.py": file_sha256(Path(__file__).resolve()),
        "live_evaluator_v0_3.py": file_sha256(HERE / "live_evaluator_v0_3.py"),
        "build_live_replay_v0_3_freeze.py": file_sha256(HERE / "build_live_replay_v0_3_freeze.py"),
        "test_live_evaluator_v0_3.py": file_sha256(HERE / "test_live_evaluator_v0_3.py"),
        "audit_live_evaluation_v0_3.py": file_sha256(HERE / "audit_live_evaluation_v0_3.py"),
    }
    if not _exact_json_equal(freeze["self_files"], expected_self_files):
        raise RuntimeError("v0.3 freeze self-file drift")
    lock = freeze["lock_payload_sha256"]
    if type(lock) is not str or lock != payload_sha256(freeze, "lock_payload_sha256"):
        raise RuntimeError("v0.3 freeze lock mismatch")
    return freeze
...
def compare_saved_and_replay(
    ...
    *,
    ...
    """Compare one extraction under the general approved diagnostic rule."""
    if type(record_index) is not int or record_index < 0:
        raise RuntimeError("record index is not a nonnegative exact integer")
    if type(identity) is not dict or set(identity) != set(IDENTITY_FIELDS):
        raise RuntimeError(f"record identity mismatch at {record_index}")
    ...
    if saved_value == replay_value:
        raise RuntimeError(f"unexpected canonical difference at {record_index}")
    return {
...
def validate_pre_gold(
    ...
    """Return the validated batch/manifest and an oracle-free report."""
    if batch_path.resolve() != BATCH_PATH.resolve() or seal_path.resolve() != SEAL_PATH.resolve():
        raise RuntimeError("v0.3 gate accepts only the pinned completed measurement paths")
    verify_v0_3_freeze(freeze_path)
    ...
    report = {
        ...
        "allowed_difference_policy": _expected_freeze_policy(),
        "panels": {
        ...
        "journal_sha256": file_sha256(JOURNAL_PATH),
        "v0_3_freeze_sha256": file_sha256(freeze_path),
    }
