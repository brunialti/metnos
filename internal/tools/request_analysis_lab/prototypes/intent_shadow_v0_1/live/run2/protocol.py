"""Closed, gold-free primitives for candidate-v0.3 live RUN 2.
...
CANDIDATE_DIR = LAB_ROOT / "candidate_v0_3"
BASE_CANDIDATE_DIR = LAB_ROOT / "candidate_v0_2"
LANGUAGE_PREREQUISITE_DIR = LAB_ROOT / "candidate_v0_2_1"
LEGACY_DIR = LAB_ROOT / "candidate_v0_1"

RUN_ID = "intent-shadow-v0.3-run2"
CREATED_DATE = "2026-08-13"
PROTOCOL_FORMAT = "metnos.intent-ir-live-protocol/0.3-run2"
SOURCE_RUN_ID = "intent-shadow-v0.2-run1"
...
MANIFEST_FORMAT = "metnos.intent-live-request-manifest/0.3-run2"
FREEZE_FORMAT = "metnos.intent-live-protocol-freeze/0.3-run2"
AUTHORIZATION_FORMAT = "metnos.intent-live-authorization/0.3-run2"
RUNNER_VERSION = "metnos.intent-live-runner/0.3-run2"
SEALED_BATCH_FORMAT = "metnos.intent-live-sealed-batch/0.3-run2"

CONTROL_SNAPSHOT_PATH = RUN1_DIR / "control_snapshot_run1.json"
QUERY_PANEL_PATH = RUN1_DIR / "query_panel_run1.json"
PROTOCOL_PATH = HERE / "protocol_run2.json"
MANIFEST_PATH = HERE / "request_manifest_run2.json"
FREEZE_PATH = HERE / "protocol_run2.freeze.json"
AUTHORIZATION_PATH = HERE / "authorization_run2.json"

CANDIDATE_FREEZE_PATH = CANDIDATE_DIR / "candidate_v0_3.freeze.json"
CANDIDATE_PROJECTION_PATH = BASE_CANDIDATE_DIR / "intent_ir_registry_projection_v0_2.json"
CANDIDATE_PROMPT_PATH = CANDIDATE_DIR / "intent_ir_v0_3.prompt.txt"
CANDIDATE_SCHEMA_PATH = CANDIDATE_DIR / "intent_ir_v0_3.schema.json"
...
BUILD_SOURCE_FILES = frozenset({
    ...
    str((CANDIDATE_DIR / "projection.py").relative_to(ROOT)),
    ...
    str((LANGUAGE_PREREQUISITE_DIR / "candidate_v0_2_1.freeze.json").relative_to(ROOT)),
    str((LANGUAGE_PREREQUISITE_DIR / "grandfathered_tags.py").relative_to(ROOT)),
    str((LANGUAGE_PREREQUISITE_DIR / "language_tag.py").relative_to(ROOT)),
...
IMMUTABLE_LOCAL_FILES = frozenset({
    ...
    "evaluator.py", "protocol.py", "protocol_run2.json", "request_manifest_run2.json",
    "runner.py", "self_review.md", "tests/__init__.py", "tests/test_mutations.py",
    "tests/test_run2.py", "verify.py",
})

RUN_ARTIFACT_NAMES = frozenset({
    "consumption_run2.json", "journal_run2.jsonl", "checkpoint_run2.json",
    "partial_batch_run2.json", "sealed_batch_run2.json", "seal_run2.json",
    "evaluation_run2.json",
})
...
def load_control_snapshot(path: Path = CONTROL_SNAPSHOT_PATH) -> dict[str, Any]:
    ...
    if (
        value["snapshot_format"] != SNAPSHOT_FORMAT or value["run_id"] != SOURCE_RUN_ID
...
def verify_candidate_environment() -> None:
    ...
    from internal.tools.request_analysis_lab.prototypes.intent_shadow_v0_1.candidate_v0_3.build_artifacts import check
...
def load_query_panel(path: Path = QUERY_PANEL_PATH) -> dict[str, Any]:
    ...
    if value["panel_format"] != QUERY_PANEL_FORMAT or value["run_id"] != SOURCE_RUN_ID or value["created_date"] != CREATED_DATE:
...
def load_protocol(path: Path = PROTOCOL_PATH) -> dict[str, Any]:
    ...
    if not exact_json_equal(value["authorization"], {"state": "disarmed", "authorization_file": "authorization_run2.json", "authorization_must_be_last": True, "independent_audit_required": True, "inference_executed": False}):
