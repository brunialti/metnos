#!/usr/bin/env python3
"""Single-use RUN3 style runner; only run_authorized_once can use the endpoint."""
...
class RunPaths:
    ...
    def in_directory(cls, directory: Path) -> "RunPaths":
        return cls(
            directory / "consumption_run3_style.json",
            directory / "journal_run3_style.jsonl",
            directory / "checkpoint_run3_style.json",
            directory / "partial_batch_run3_style.json",
            directory / "sealed_batch_run3_style.json",
            directory / "seal_run3_style.json",
            directory / "evaluation_run3_style.json",
        )
...
def validate_authorization(path: Path = AUTHORIZATION_PATH) -> dict[str, Any]:
    ...
    expected_keys = {
        ...
        "manifest_file_sha256", "iteration_authorization", "independent_audits",
        "root_authorization", "execution_environment", "single_use_nonce_sha256",
    }
    ...
    load_manifest()
    expected = {
        "authorization_format": AUTHORIZATION_FORMAT, "run_id": RUN_ID,
    ...
    if not exact_json_equal(value["execution_environment"], protocol["execution_environment"]):
        raise RuntimeError("authorization execution environment mismatch")
    if not exact_json_equal(value["iteration_authorization"], {
        "authorized_by": "Roberto",
        "authorization_scope": "automatic_iterative_cycle_after_style_only_change_green_suite_and_two_independent_audits",
        "this_run": 3, "rigid_run_limit": None,
    }):
        raise RuntimeError("iteration authorization mismatch")
    audits = value["independent_audits"]
    if type(audits) is not list or len(audits) != 2:
        raise RuntimeError("two independent audits required")
    ...
    for audit in audits:
        if (
            ...
            or type(audit.get("reviewer")) is not str or not audit["reviewer"]
            or type(audit.get("report_path")) is not str or not audit["report_path"]
            or type(audit.get("report_sha256")) is not str
            or audit.get("pass") is not True
        ):
            raise RuntimeError("independent audit mismatch")
        if audit["reviewer"] in reviewers or audit["report_path"] in reports:
            raise RuntimeError("independent audits must be distinct")
        reviewers.add(audit["reviewer"])
        reports.add(audit["report_path"])
    ...
    if (
        ...
        or root.get("authorized") is not True or root.get("authorized_by") != "root"
        or type(root.get("authorization_id")) is not str or not root["authorization_id"]
    ):
...
def urllib_transport(request_body: dict[str, Any], timeout_seconds: int) -> TransportResponse:
    ...
    request = urllib.request.Request(
        endpoint, data=canonical_json_bytes(request_body),
...
def _extract(_record: dict[str, Any], content: bytes) -> dict[str, Any]:
    from .arm_style import extract_response
    return extract_response(
        content, arm=_record["arm"], query=_record["query"],
...
def _execute_manifest(
    ...
    for public in manifest["records"]:
            ...
            if response.error_code is not None:
                ...
                if content is None:
                    ...
                    try:
                        ...
                        saved["extraction"] = {"status": "technical_invalid", "semantic_document": None, "technical_failure": failure}
        ...
        if stopped_reason is not None:
            break
    complete = len(records) == REQUEST_COUNT and accepted_posts == REQUEST_COUNT and stopped_reason is None
    batch = {
        "batch_format": SEALED_BATCH_FORMAT if complete else "metnos.intent-prompt-style-live-partial-batch/0.1-run3",
        "runner_version": RUNNER_VERSION, "run_id": RUN_ID,
        "state": "complete" if complete else "partial_transport_stop",
        "gold_opened": False, "authorization_sha256": authorization_sha256,
        "protocol_file_sha256": file_sha256(PROTOCOL_PATH),
        "protocol_freeze_file_sha256": file_sha256(FREEZE_PATH),
        "manifest_file_sha256": file_sha256(MANIFEST_PATH),
        "manifest_payload_sha256": manifest["manifest_payload_sha256"],
...
def preflight(
    *, authorization_state: str = "disarmed", authorization_path: Path = AUTHORIZATION_PATH,
    ...
    return {
        "status": "armed_ready" if authorization is not None else "disarmed_ready_for_two_audits",
        "run_id": RUN_ID, "request_count": REQUEST_COUNT,
        "python_hash_seed": load_protocol()["execution_environment"]["python_hash_seed"],
...
def execute_fake_full_run(manifest: dict[str, Any], *, transport: Transport, paths: RunPaths) -> dict[str, Any]:
    return _execute_manifest(
