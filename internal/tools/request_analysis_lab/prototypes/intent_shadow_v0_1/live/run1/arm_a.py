"""RUN 1 arm A: current Metnos intent extractor through a read-only adapter."""
...
def extract_response(raw_content: bytes, *, query: str, language: str) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "adapter_version": "current-control/run1", "status": "technical_invalid",
    ...
    return {
        "adapter_version": "current-control/run1", "status": "valid_representable" if semantic["kind"] != "unrepresentable" else "valid_unrepresentable",
        "raw_model_output_b64": raw_b64, "raw_model_output_sha256": raw_sha,
