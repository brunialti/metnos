#!/usr/bin/env python3
"""Single-use RUN 1 runner; only ``--execute-once`` can touch the endpoint."""
...
class RunPaths:
    ...
    def in_directory(cls, directory: Path) -> "RunPaths":
        return cls(
            directory / "consumption_run1.json",
            directory / "journal_run1.jsonl",
            directory / "checkpoint_run1.json",
            directory / "partial_batch_run1.json",
            directory / "sealed_batch_run1.json",
            directory / "seal_run1.json",
            directory / "evaluation_run1.json",
        )
...
def validate_authorization(path: Path = AUTHORIZATION_PATH) -> dict[str, Any]:
    ...
    if not exact_json_equal(value["iteration_authorization"], {"authorized_by": "Roberto", "maximum_new_runs": 3, "this_run": 1}):
...
def _execute_manifest(
    ...
    batch = {
        "batch_format": SEALED_BATCH_FORMAT if complete else "metnos.intent-live-partial-batch/0.2-run1",
        "runner_version": RUNNER_VERSION, "run_id": RUN_ID,
    ...
    _atomic_write(paths.seal, {
        "freeze_format": "metnos.intent-live-batch-seal/0.2-run1",
