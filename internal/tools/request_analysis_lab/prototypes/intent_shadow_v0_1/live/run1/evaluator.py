#!/usr/bin/env python3
"""Post-seal evaluator for RUN 1 using the symmetric v0.3 metric."""
...
EVALUATOR_VERSION = "metnos.intent-live-evaluator/0.3-run1"
...
def _validate_marker_checkpoint(
    ...
    expected_marker = {
        ...
        "protocol_freeze_sha256": file_sha256(HERE / "protocol_run1.freeze.json"),
...
def validate_complete_batch(
    ...
    if (
        ...
        or batch["protocol_file_sha256"] != file_sha256(HERE / "protocol_run1.json")
        or batch["protocol_freeze_file_sha256"] != file_sha256(HERE / "protocol_run1.freeze.json")
...
def _load_gold() -> tuple[dict[tuple[str, str], dict[str, Any]], dict[str, Any], dict[str, Any]]:
    spec = importlib.util.spec_from_file_location("run1_oracle_verifier", ORACLE_VERIFY_PATH)
