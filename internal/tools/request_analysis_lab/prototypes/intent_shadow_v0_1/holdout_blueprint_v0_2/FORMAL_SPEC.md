# Holdout blueprint-first v0.2

Stato: **PREFLIGHT ONLY — ZERO QUERY, ZERO GOLD DI CASO, ZERO RUN**  
Data: 2026-08-13

## 1. Perché esiste v0.2

Il preflight v0.1 è stato respinto da due audit indipendenti nonostante 18 test verdi. I difetti principali erano: expected non derivato esattamente, identità reviewer assenti, fingerprint dipendente da ID arbitrari, projection con leakage, closure incompleta e mutation test troppo vicini all'implementazione. v0.1 resta storico e non viene corretto retroattivamente.
...
3. proiezione normativa v0.2 delle sole regole di binding e safety già approvate;
...
- tutte e sole le edge `consumes`, con porte esplicite;
...
I giudizi contengono reviewer ID e context ID. Duplicare un record non costituisce due reviewer.

## 6. Projection e i18n

La projection contiene soltanto tuple strutturate `kind`, `author_gloss`, relazioni per indice e constraint di superficie. Per un false-action trap aggiunge il solo `tempting_but_insufficient_gloss`: descrive in forma route-free la capacità registrata che non deve essere usata come approssimazione. Ogni gloss deriva dagli scope revisionati del registro; gli unici adattamenti sono traduzioni neutrali dei record legacy privi di scope inglese e la rimozione della sintassi esecutiva. Non contiene route, expected, reason, path, codice, pattern, esempi, nomi o numeri prescritti. Usa lo stesso percorso neutro per ogni BCP47; l'autore deve formulare direttamente nella lingua assegnata.
...
- ruoli chiusi, bundle hashati, nessun accesso vietato, rete/GPU zero, nessun edit post-sigillo;
...
Servono due audit indipendenti READY sul byte finale. Solo allora parte automaticamente il pilot. Un errore chiude v0.2 e richiede una nuova versione; non si rattoppa il campione.
