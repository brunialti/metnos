"""Static closure, namespace and artifact preflight for holdout blueprint v0.4."""
...
def validate_canonical_schema() -> None:
    ...
    require(definitions["data_edge"]["required"] == ["from"]
            and set(definitions["data_edge"]["properties"]) == {"from", "output", "input"},
            "GOLD_SCHEMA", "canonical edge")
...
def validate_glossary(value: dict[str, Any]) -> None:
    ...
    for route, item in value["operations"].items():
        require(set(item) == {"authorable", "author_gloss", "source_scopes", "input_ports", "output_ports"}, "CLOSED_SCHEMA", route)
        gloss = item["author_gloss"]
        require(type(item["authorable"]) is bool and type(item["source_scopes"]) is list, "CLOSED_SCHEMA", route)
        if item["authorable"]:
            ...
            require(gloss is None, "CAPABILITY_SCOPE", "ineligible gloss")
    ...
    require(sum(item["authorable"] for item in value["operations"].values()) >= 60,
            "CAPABILITY_SCOPE", "insufficient exact author scopes")
...
def validate_mutations(value: dict[str, Any]) -> None:
    ...
    require([item["id"] for item in rows] == [f"M{i:02d}" for i in range(1, 20)], "CLOSED_SCHEMA", "mutation ids")
    require(len({item["expected_code"] for item in rows}) == 19, "CLOSED_SCHEMA", "mutation codes")
...
def validate_source_closure() -> None:
    ...
    allowed_external = {"__future__", "argparse", "ast", "collections", "copy", "dataclasses", "hashlib", "json", "os", "pathlib", "re", "sys", "tempfile", "typing", "unittest"}
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import): modules = [alias.name.split(".")[0] for alias in node.names]
                ...
                modules = ([alias.name.split(".")[0] for alias in node.names]
                           if node.level and node.module is None else [(node.module or "").split(".")[0]])
            else: continue
            for module in modules:
                require(module in local_modules or module in allowed_external, "BUILD_DRIFT", f"unbound import {module}")


def validate_namespace(actual: set[str] | None = None) -> None:
    if actual is None:
