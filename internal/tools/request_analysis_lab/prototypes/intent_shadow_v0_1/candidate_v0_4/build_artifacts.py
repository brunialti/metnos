"""Build/check the immutable candidate v0.4 prompt-only overlay."""
...
BASE_DIR = HERE.parent / "candidate_v0_3"
SCHEMA_FILE = HERE / "intent_ir_v0_4.schema.json"
PROMPT_FILE = HERE / "intent_ir_v0_4.prompt.txt"
FREEZE_FILE = HERE / "candidate_v0_4.freeze.json"
FREEZE_FORMAT = "metnos.intent-ir-prompt-overlay-freeze/0.4"
...
def expected_artifacts() -> tuple[bytes, bytes]:
    ...
    if schema != (BASE_DIR / "intent_ir_v0_3.schema.json").read_bytes():
        raise SystemExit("v0.4 schema differs from frozen v0.3")
    return schema, build_prompt(projection).encode("utf-8")


def build_freeze() -> dict[str, Any]:
    projection = validate_projection(load_projection())
    base_freeze = load_json_file(BASE_DIR / "candidate_v0_3.freeze.json")
    freeze = {
        "freeze_format": FREEZE_FORMAT,
        "status": "offline_prompt_overlay_not_runtime",
        "base_candidate": {
            "freeze_sha256": file_sha256(BASE_DIR / "candidate_v0_3.freeze.json"),
        ...
        "constraints": {
            "logical_change": ["prompt_projection_coverage_before_root_only"],
...
def check() -> list[str]:
    ...
    if type(freeze) is not dict or set(freeze) != {
        "freeze_format", "status", "base_candidate", "contract", "files",
    ...
    expected_base = {
        "freeze_sha256": file_sha256(BASE_DIR / "candidate_v0_3.freeze.json"),
        "contract": load_json_file(BASE_DIR / "candidate_v0_3.freeze.json")["contract"],
        "critical_files": _base_hashes(),
    }
