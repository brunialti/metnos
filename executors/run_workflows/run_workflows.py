#!/usr/bin/env python3
"""run_workflows — dispatcher canonico per l'oggetto `workflows`.

Il fornitore e' un ARGOMENTO (`client`), non un suffisso nel nome. Una
automazione remota non e' un promemoria dello scheduler interno (`tasks`) ne'
un processo locale (`processes`): ha un oggetto suo dal 20/9/2026.

Contratto:
    stdin:  JSON {repo, workflow, client?}
    stdout: JSON {ok, results, used} oppure {ok=false, error}
"""
...
def invoke(args):
    ...
    if backend is None:
        return {"ok": False,
                ...
                "results": [], "used": 0}
    return backend.run_workflows(args)
