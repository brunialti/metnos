#!/usr/bin/env python3
"""create_comments — dispatcher canonico per l'oggetto `comments`.

RM-0011 F2. Il fornitore e' un ARGOMENTO (`client`), non un suffisso
nel nome: e' la regola unica decisa in RM-0011. Il runtime possiede il valore
(`backend_resolver`), l'executor non lo sceglie e non lo nomina. Aggiungere
un secondo sistema di versionamento e' un modulo sotto `backends/comments/`,
non un altro executor.

Contratto:
    stdin:  JSON {repo, targets, body, client?}
    stdout: JSON {ok, results, ok_count, fail_count, failed} oppure {ok=false, error}
"""
...
def invoke(args):
    ...
    if backend is None:
        return {"ok": False,
                ...
                "results": [], "used": 0}
    return backend.create_comments(args)
