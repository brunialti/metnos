#!/usr/bin/env python3
"""create_issues — dispatcher canonico per l'oggetto `issues`.
...
un secondo sistema di versionamento e' un modulo sotto `backends/issues/`,
non un altro executor.

Contratto:
    stdin:  JSON {repo, title, client?}
    stdout: JSON {ok, results, used, available_total} oppure {ok=false, error}
"""
...
def invoke(args):
    ...
    if backend is None:
        return {"ok": False,
                ...
                "results": [], "used": 0}
    return backend.create_issues(args)
