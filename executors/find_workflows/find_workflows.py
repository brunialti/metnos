#!/usr/bin/env python3
"""find_workflows — dispatcher canonico per l'oggetto `workflows`.
...
Contratto:
    stdin:  JSON {repo, client?}
    stdout: JSON {ok, entries, used} oppure {ok=false, error}
"""
...
def invoke(args):
    ...
    if backend is None:
        return {"ok": False,
                ...
                "entries": [], "used": 0}
    return backend.find_workflows(args)
