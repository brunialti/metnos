...
ABANDONED_CROSSING_DOMAIN_V2 = b"metnos.executor-birth.abandoned-crossing/v2\0"
...
_BIRTH_CLOSED_SOURCE_REVIEW_SHA256 = "sha256:746df55d9a10e9c247671b00fff4bba4d4e5c6eec43233f5c6d46df1eba239b3"
...
class _DecodedAbandonedCrossingV2(NamedTuple):
    abandonment_id: str
    request_id: str
    release_sequence: int
    closed_build_id: str
...
class _CapturedAbandonedCrossingCandidateV2(NamedTuple):
    basename: str
    encoded: bytes
    decoded: _DecodedAbandonedCrossingV2
...
def _bind_candidate_cutover_materials_core_v1(
    ...
    transaction: _DecodedCoordinatorRecordV2,
    predecessor: _DecodedPredecessorDescriptorV1,
    captured: Mapping[str, bytes], *, review_service_source: bool = True,
    ...
    if (
        ...
        or transaction.sequence < 1 or type(review_service_source) is not bool
    ):
    ...
    if (
        ...
        or transaction.distribution_payload_hash != _raw_sha256_v1(distribution.encoded)
        or transaction.distribution_signature_hash != _raw_sha256_v1(distribution.signature)
    ):
    ...
    if (
        ...
        or (
            ...
            and (
                predecessor.administrative_bundle_hash != bundle_hash
                or predecessor.service_catalog_id != catalog.catalog_id
...
def _bind_preflight_materials_core_v1(
    ...
    *, review_service_source: bool = True,
    ...
    candidate = _bind_candidate_cutover_materials_core_v1(
        ...
        review_service_source=review_service_source,
    )
...
def _decode_abandoned_crossing_v2(encoded: bytes) -> _DecodedAbandonedCrossingV2:
    """Decode one abandonment independently of the module that wrote it."""
    value = decode_canonical_json_v1(
        encoded, MAX_COORDINATOR_CONTROL_BYTES_V2,
    )
    if (
        not isinstance(value, dict)
        or set(value) != _ABANDONED_CROSSING_KEYS_V2
        or type(value.get("schema_version")) is not int
        or value.get("schema_version") != 2
    ):
        raise _invalid("abandoned crossing schema")
    abandonment_id = _require_digest(
        value.get("abandonment_id"), "abandonment_id",
    )
    request_id = _require_digest(value.get("request_id"), "abandonment request")
    closed_build_id = _require_digest(
        value.get("closed_build_id"), "abandonment build",
    )
    head_id = _require_digest(value.get("head_id"), "abandonment head")
    deployment_descriptor_id = _require_digest(
        value.get("deployment_descriptor_id"), "abandonment descriptor",
    )
    abandoned_record_sha256 = _require_digest(
        value.get("abandoned_record_sha256"), "abandonment record",
    )
    release_sequence = value.get("release_sequence")
    reason = value.get("reason")
    if (
        type(release_sequence) is not int or release_sequence <= 0
        or not isinstance(reason, str)
        or reason not in _ABANDONED_CROSSING_REASONS_V2
    ):
        raise _invalid("abandoned crossing fields")
    unsigned = {
        key: item for key, item in value.items() if key != "abandonment_id"
    }
    if abandonment_id != _digest(
        ABANDONED_CROSSING_DOMAIN_V2, _canonical_json(unsigned),
    ):
        raise _invalid("abandonment_id")
    return _DecodedAbandonedCrossingV2(
        abandonment_id, request_id, release_sequence, closed_build_id,
        head_id, deployment_descriptor_id, abandoned_record_sha256, reason,
    )


def _abandonment_for_prefix_v2(
    abandonments: tuple[_DecodedAbandonedCrossingV2, ...], prefix: object,
) -> _DecodedAbandonedCrossingV2 | None:
    """Admit an abandonment only against the exact record it describes."""
    latest = prefix.records[-1]
    if latest.sequence != 5 or latest.state != "HEAD_REQUIRED":
        return None
    matches = tuple(
        item for item in abandonments if item.request_id == latest.request_id
    )
    if not matches:
        return None
    if len(matches) != 1:
        raise _recovery("abandonment inventory")
    abandonment = matches[0]
    if (
        abandonment.release_sequence != latest.release_sequence
        or abandonment.closed_build_id != latest.closed_build_id
        or abandonment.head_id != latest.head_id
        or abandonment.deployment_descriptor_id
        != latest.deployment_descriptor_id
        or abandonment.abandoned_record_sha256
        != _coordinator_record_hash_v2(prefix.encoded_records[-1])
    ):
        raise _recovery("abandonment binding")
    return abandonment
    ...
    try:
        ...
        allowed_coordinator = {
            *legacy_names, "successor-claims-v1", "transactions-v2",
            "legacy-disposition-v2.json", "abandoned-crossings-v2",
        }
        ...
        abandonment_names: tuple[str, ...] = ()
        abandonment_fd = -1
        if "abandoned-crossings-v2" in coordinator_names:
            abandonment_fd, abandonment_names = add_directory(
                coordinator_fd, "abandoned-crossings-v2",
                "coordinator-v1/abandoned-crossings-v2",
            )
            if any(
                _ABANDONED_CROSSING_BASENAME_RE_V2.fullmatch(name) is None
                for name in abandonment_names
            ):
                raise _recovery("abandonment inventory")
            for name in abandonment_names:
                add_file(
                    abandonment_fd, name,
                    "coordinator-v1/abandoned-crossings-v2/" + name,
                    maximum=MAX_COORDINATOR_CONTROL_BYTES_V2,
                )
        ...
        abandoned_crossings: list[_CapturedAbandonedCrossingCandidateV2] = []
        seen_abandoned_request_ids: set[str] = set()
        for name in abandonment_names:
            encoded = _read_control_file_v1(
                files["coordinator-v1/abandoned-crossings-v2/" + name],
            )
            decoded = decode_durable(_decode_abandoned_crossing_v2, encoded)
            if (
                name != decoded.request_id.removeprefix("sha256:") + ".json"
                or decoded.request_id in seen_abandoned_request_ids
            ):
                raise _recovery("abandonment binding")
            seen_abandoned_request_ids.add(decoded.request_id)
            abandoned_crossings.append(
                _CapturedAbandonedCrossingCandidateV2(name, encoded, decoded),
            )
        abandoned_crossings.sort(key=lambda item: item.decoded.request_id)
        ...
        return _CapturedFixedOwnershipStateCandidateV1(
            ...
            predecessor, tuple(abandoned_crossings),
        )
    except PreflightError as exc:
...
def _authenticate_fixed_ownership_snapshot_core_v1(
    ...
    try:
        ...
        abandoned_crossings = tuple(
            durable(_decode_abandoned_crossing_v2, item.encoded)
            for item in candidate.abandoned_crossings
        )
        if len({item.request_id for item in abandoned_crossings}) != len(
            abandoned_crossings
        ):
            raise _recovery("abandonment inventory")
        ...
        for index, claim in enumerate(claims):
            ...
            if claim.release_sequence == 1:
                ...
                # could. The abandonment is bound to that exact record, so a
                # crossing that merely stopped cannot pass for an abandoned
                # one.
                if (
                    ...
                    or (
                        previous_transaction.prefix.records[-1].sequence != 6
                        and _abandonment_for_prefix_v2(
                            abandoned_crossings, previous_transaction.prefix,
                        ) is None
                    )
                ):
        ...
        for transaction in transactions:
            ...
            # Each release keeps its OWN protected attestation and immutable
            # transaction fields. Above, that transaction is bound to its
            # signed manifest, certificate and head. These historical records
            # do not constitute a direct signature over the bundle: the full
            # signed descriptor/artifact -> bundle check belongs to the selected
            # release's material binder, never to the initial legacy bundle.
            if (
...
def _unattestable_administrative_paths_v1(
    materials: _BoundPreflightMaterialsV1,
    capture: _CapturedAdministrativeTcbV1,
) -> bool:
    """Decide whether the signed administrative paths can never be observed.

    A release names the four fixed executables of the administrative base.
    Their canonical paths are a property of this operating system, not of the
    release: no future state of the machine can make a different path appear
    at the fixed links. The measured contents must still agree with the
    prerequisite, otherwise the machine, and not the release, is what changed
    and the failure may still be transient.
    """
    executables = capture.executables
    descriptor = materials.descriptor
    prerequisite = materials.prerequisite
    ...
    measured = (
        prerequisite.python_binary_hash == executables.python_binary_hash
        and prerequisite.openssl_binary_hash == executables.openssl_binary_hash
        and prerequisite.systemctl_binary_hash
        == executables.systemctl_binary_hash
        and prerequisite.systemd_analyze_binary_hash
        == executables.systemd_analyze_binary_hash
        and prerequisite.openssl_tcb_hash == capture.openssl_tcb.openssl_tcb_hash
    )
    return observed != declared and measured


def _prove_unattestable_crossing_v1() -> tuple[str, str]:
    """Name the permanent contradiction of the selected crossing, or refuse.

    Read-only. It authenticates the same fixed history the attestation uses
    and returns a reason from the closed catalogue only when the selected
    release can never be attested on this machine. Anything else raises: an
    attestation that is merely absent, failing elsewhere or still possible is
    not an abandonment, and this function is the only thing that may say so.

    It returns the request it proved together with the reason, so the writer
    can refuse to record a proof taken about a different crossing.

    The installed sources are authenticated against their own signed inventory
    and are not required to equal today's compiled source root: the release
    being abandoned is by construction an older one, so demanding a current
    source review here would refuse exactly the case this exists for.
    """
    authenticated = _authenticate_fixed_ownership_snapshot_v1()
    ...
    if latest.sequence != 5:
        raise _invalid("unattestable crossing state")
    captured = authenticated.administrative_tcb
    if type(captured) is not _CapturedAdministrativeTcbProductV1:
        raise _invalid("unattestable crossing capture")
    _revalidate_captured_administrative_tcb_v1(
        captured.capture, _administrative_links_v1(),
        uid=0, gid=0, chain_stop=None,
    )
    if not _unattestable_administrative_paths_v1(materials, captured.capture):
        raise _invalid("unattestable crossing reason")
    from executor_birth_ownership_coordinator import (
        _ABANDONED_CROSSING_REASONS_V2,
    )

    reason = "administrative_tcb_path_unsatisfiable"
    if reason not in _ABANDONED_CROSSING_REASONS_V2:
        raise _invalid("unattestable crossing vocabulary")
    repeated_selected, repeated_materials = _load_installed_preflight_materials_v1(
        authenticated, review_sources=False,
    )
    if (
        repeated_selected != selected or repeated_materials != materials
        or not _unattestable_administrative_paths_v1(
            repeated_materials, captured.capture,
        )
    ):
        raise _invalid("unattestable crossing reread")
    return latest.request_id, reason
...
def _load_installed_preflight_materials_v1(
    ...
    materials = _bind_preflight_materials_core_v1(
        ...
        review_service_source=review_sources,
    )
