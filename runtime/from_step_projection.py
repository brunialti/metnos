...
def _is_selected_alternative(value: Any) -> bool:
    """Semantica ``requires_one_of``: falsi e zeri non selezionano un ramo."""
    if value is None:
        return False
    if isinstance(value, str):
        return bool(value.strip())
...
def _unique_names(names: list[str]) -> tuple[str, ...]:
    """Deduplica pochi nomi di schema preservandone l'ordine dichiarato."""
    out: list[str] = []
    for name in names:
        if name not in out:
            out.append(name)
...
def from_step_alternatives(consumer_schema: dict | None) -> tuple[str, ...]:
    """Argomenti che sostituiscono ``from_step``, dichiarati dal manifest.

    Un override ``from_step_alternatives`` risolve i rari schemi nei quali un
    ``requires_one_of`` contiene anche metadati di destinazione. Negli altri
    casi l'autorita' e' il gruppo che contiene ``from_step`` o ``entries``.
    ``entries`` resta il solo ripiego universale del protocollo di piping.
    """
    if not isinstance(consumer_schema, dict):
        return ()
    properties = _properties(consumer_schema)
    override = consumer_schema.get("from_step_alternatives")
    if isinstance(override, list):
        return _unique_names([
            name for name in override
            if isinstance(name, str) and name != "from_step"
            and name in properties
        ])

    alternatives: list[str] = []
    for group in consumer_schema.get("requires_one_of") or []:
        if not isinstance(group, list) \
                or not ({"from_step", "entries"} & set(group)):
            continue
        alternatives.extend(
    ...
    if alternatives:
        return _unique_names(alternatives)
...
def has_explicit_from_step_alternative(
    ...
    if not isinstance(args, dict):
        return False
    return any(
        name in args and _is_selected_alternative(args.get(name))
        for name in from_step_alternatives(consumer_schema)
    )
