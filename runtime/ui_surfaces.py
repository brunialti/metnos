...
SURFACES: tuple[UiSurfaceSpec, ...] = (
    ...
    UiSurfaceSpec(
        ...
        ("servizi raggruppati per funzione", "stato systemd e sottostato",
         ...
         "indicatore LRE grigio quando disattivato, verde solo se abilitato e pronto, giallo in transizione o con stato incerto, rosso per errore del servizio o configurazione non valida; processo nei dettagli tecnici"),
        ("services grouped by function", "systemd state and sub-state",
         ...
         "LRE indicator gray when disabled, green only when enabled and ready, amber during transition or uncertain state, red for service failure or invalid configuration; process state under technical details"),
