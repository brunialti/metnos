...
def test_check_plan_unknown_context_passes_through():
    """Cella contextual mancante -> aligned=True con motivazione esplicita."""
    plan = _plan_shim("read", "calendar", "ufoize")
    v = check_plan(plan)
    assert v.aligned is True
    assert "no contextual cell" in v.mismatch_reason
