...
Copre:
1. _render_llm_commentary chiama call_llm con tier=middle e timeout 5s.
...
# ─── 1. _render_llm_commentary chiama call_llm tier=middle ────────────────


class TestCommentaryCall(_BaseCommentaryTest):

    def test_calls_llm_with_middle_tier(self):
        ...
        with mock.patch("llm_helpers.call_llm") as m:
            m.return_value = (
                ...
                {"tier": "middle", "in_tokens": 800, "out_tokens": 60,
                 "latency_ms": 1200},
            )
            data = {
        ...
        # Verifica che call_llm sia stato invocato con tier=middle.
        _, kwargs = m.call_args
        self.assertEqual(kwargs.get("tier"), "middle")
...
class TestIntegration(_BaseCommentaryTest):
    ...
    def test_render_with_llm_produces_three_sections(self):
        ...
        self.assertIn("**Pipeline OGGI**", out)
        # Sezione 2 (perf savings).
        self.assertIn("## Stima risparmio", out)
...
class TestPromptFrontmatter(unittest.TestCase):
    ...
    def test_it_prompt_exists_with_frontmatter(self):
        ...
        for key in ("role: promoter_commentary", "tier: middle",
