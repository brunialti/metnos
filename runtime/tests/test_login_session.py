"""Test di login_session (ADR 0082, 4/5/2026).
...
sys.path.insert(0, str(_RUNTIME.parent / "executors" / "login_session"))
...
class TestLoginSession(unittest.TestCase):
    ...
    def setUp(self):
        # Override credentials.CRED_DIR + login_session.COOKIES_DIR
        self.tmp = tempfile.mkdtemp()
        import credentials
        import login_session
        self._old_cred = credentials.CRED_DIR
        self._old_cook = login_session.COOKIES_DIR
        credentials.CRED_DIR = Path(self.tmp) / "credentials"
        login_session.COOKIES_DIR = Path(self.tmp) / "cookies"
    ...
    def tearDown(self):
        ...
        login_session.COOKIES_DIR = self._old_cook
    ...
    def test_login_failure_401(self):
        import login_session, credentials
    ...
    def test_cached_cookie_reuse(self):
        import login_session
        domain = self._store_test_creds()
        out1 = login_session.invoke({"domain": domain})
        ...
        out2 = login_session.invoke({"domain": domain})
    ...
    def test_force_relogin(self):
        ...
        out1 = login_session.invoke({"domain": domain})
        n_posts = len(_state["post_log"])
        out2 = login_session.invoke({"domain": domain, "force": True})
        self.assertTrue(out2["ok"], out2)
