...
Run: `python3 -m pytest runtime/tests/test_late_result_a0.py -xvs`.
"""
...
class UserNoticesTests(unittest.TestCase):
    ...
    def test_append_drain_roundtrip(self):
        self.un.append("http", "host", "avviso 1")
        self.un.append("http", "host", "avviso 2")
        self.un.append("telegram", "host", "altro destinatario")
        out = self.un.drain("http", "host")
        self.assertEqual(out, ["avviso 1", "avviso 2"])
        self.assertEqual(self.un.drain("http", "host"), [])  # svuotata
        self.assertEqual(self.un.drain("telegram", "host"),
                         ["altro destinatario"])

    def test_write_prepends_notice(self):
        import agent_runtime
        self.un.append("http", "host", "📬 op completata in ritardo")
