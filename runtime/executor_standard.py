...
_MIN_SANDBOXES = frozenset({"job-object", "appcontainer"})
...
def _validate_authority(findings: list[StandardFinding], manifest: dict) -> None:
    ...
    if not isinstance(capabilities, list) or not capabilities:
        ...
        for index, capability in enumerate(capabilities):
            ...
            if isinstance(capability, dict):
                ...
                if "outbound_args" in capability:
                    outbound_args = capability.get("outbound_args")
                    if (not isinstance(outbound_args, list)
                            or not outbound_args
                            or len(outbound_args) != len(set(outbound_args))
                            or any(not isinstance(arg_name, str)
                                   or not arg_name.strip()
                                   for arg_name in outbound_args)):
                        _add(
                            findings, "outbound_args_shape",
                            f"capabilities[{index}].outbound_args must be a "
                            "non-empty list of unique argument names",
                        )
                    else:
                        if name not in _OUTBOUND_CAPABILITY_NAMES:
                            _add(
                                findings, "outbound_args_family",
                                f"capabilities[{index}].outbound_args requires "
                                "an external transport capability",
                            )
                        properties = args_schema.get("properties") or {}
                        for arg_name in outbound_args:
                            if (not isinstance(properties, dict)
                                    or arg_name not in properties):
                                _add(
                                    findings, "outbound_args_arg",
                                    f"capabilities[{index}].outbound_args "
                                    f"references undeclared argument {arg_name!r}",
                                )
                if name == "fs:read":
                    properties = args_schema.get("properties") or {}
                ...
                path_annotations = ("path_args", "parent_path_args")
                annotated = any(key in capability for key in path_annotations)
                if annotated and not (isinstance(name, str)
                                      and name.startswith("fs:")):
                    _add(
                        findings, "filesystem_scope_annotation_family",
                        f"capabilities[{index}] path annotations require fs:*",
                    )
                properties = args_schema.get("properties") or {}
                for annotation in path_annotations:
                    if annotation not in capability:
                        continue
                    fields = capability.get(annotation)
                    if (not isinstance(fields, list) or not fields
                            or any(not isinstance(field, str) or not field
                                   for field in fields)):
                        _add(
                            findings, "filesystem_scope_annotation_shape",
                            f"capabilities[{index}].{annotation} must be a "
                            "non-empty list of argument names",
                        )
                        continue
                    for field in fields:
                        ...
                        if not isinstance(spec, dict):
                            _add(
                                findings, "filesystem_scope_annotation_arg",
                                f"capabilities[{index}].{annotation} references "
                                f"undeclared argument {field!r}",
                            )
                        elif declared_type not in ("string", "array"):
                            _add(
                                findings, "filesystem_scope_annotation_type",
                                f"capabilities[{index}].{annotation} argument "
                                f"{field!r} must be string or array",
                            )
    ...
    if placement is not None:
        if not isinstance(placement, dict):
            ...
            minimum = placement.get("min_sandbox")
            if minimum is not None and minimum not in _MIN_SANDBOXES:
                _add(
                    findings, "placement_min_sandbox",
                    f"placement.min_sandbox must be one of "
                    f"{sorted(_MIN_SANDBOXES)}",
                )
            if placement.get("device_ok") is True and minimum is None:
                _add(
                    findings, "placement_min_sandbox",
                    "device_ok executors must declare placement.min_sandbox",
                )
    ...
    if "system:undo" in capability_names:
        if not isinstance(placement, dict) or placement.get("scope") != "server":
            _add(
                findings, "undo_placement",
                "system:undo requires placement.scope = 'server'",
            )
        if manifest.get("revertible"):
            _add(
                findings, "undo_revertibility",
                "a system:undo broker cannot itself be revertible",
            )
