...
~/.config/mykleos/mail.env (legacy register.it). Variabili attese:
  metnos: METNOS_MAIL_HOST_IMAP, METNOS_MAIL_PORT_IMAP, METNOS_SYSTEM_USER,
          METNOS_SYSTEM_PASS (oppure METNOS_ROBERTO_USER/_PASS).
  mykleos: MYKLEOS_MAIL_HOST, MYKLEOS_MAIL_PORT, MYKLEOS_MAIL_USER,
           MYKLEOS_MAIL_PASS.
...
def _account_creds(account: str) -> dict:
    ...
    if account == "mykleos":
        ...
        return {
            "imap_host": env.get("MYKLEOS_MAIL_HOST", "imap.register.it"),
            "imap_port": int(env.get("MYKLEOS_MAIL_PORT", "993")),
            "smtp_host": env.get("MYKLEOS_SMTP_HOST", "authsmtp.securemail.pro"),
            "smtp_port": int(env.get("MYKLEOS_SMTP_PORT", "465")),
            "user": env.get("MYKLEOS_MAIL_USER", ""),
            "password": env.get("MYKLEOS_MAIL_PASS", ""),
            "verify_tls": False,  # register.it cert per *.securemail.pro
        }
...
def resolve_account(name: str, *, min_score: float = 0.45) -> str | None:
    ...
    Esempi col setup attuale (metnos_system, metnos_roberto, mykleos,
    knowcastle, tiscali):
      "metnos"  → "metnos_system" (J=0.5 vs system, 0.5 vs roberto;
                  tiebreak: SequenceMatcher 0.71 vs 0.66 → system).
      "knowcas" → "knowcastle" (substring; J=0 → tiebreak SeqMatcher).
...
def list_known_accounts() -> list[str]:
    ...
    for nm in ("metnos_system", "metnos_roberto", "mykleos"):
        try:
            c = _account_creds(nm)
