        ...
        with tempfile.TemporaryDirectory(prefix="metnos-reconcile-release-") as raw:
            for index, (origin, name) in enumerate(pending):
                ...
                try:
                    ...
                    # Inventory already exposes an exact empty store
                    # placeholder before a contract's first publication.
                    # It is still a new core contract, not a corrupt current
                    # generation and not an already installed executor.
                    new_core = current is None and origin is core
