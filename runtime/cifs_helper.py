...
(es. `cifs_192.168.1.20`); questo permette di riusare la stessa
...
def temp_credentials_file(domain: str) -> Iterator[Tuple[Optional[str], Optional[str]]]:
    ...
    Uso tipico:
        ...
        with temp_credentials_file("cifs_192.168.1.20") as (cred_path, err):
            if err:
...
def store_cifs_credentials(
    ...
    *,
    ...
    `cifs_<server>` (es. `cifs_192.168.1.20`). I campi `server` e `share`
...
def domain_for_server(server: str) -> str:
    ...
    Esempi:
        domain_for_server("192.168.1.20") -> "cifs_192.168.1.20"
