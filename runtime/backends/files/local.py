...
- `delete_dirs(args)`: rimuove directory vuote; `force=true` abilita la
  rimozione ricorsiva (vettoriale).
...
def _safe_format(template: str, entry: dict):
    ...
    if isinstance(template, str) and "${" in template:
        import re as _re
        # A planner may describe a grouped report row as
        # `${entry.entries.*.field}` even when the carrier is the canonical
        # flat list.  During the per-entry render this means the current
        # record's field; duplicate rendered paths are aggregated below.
        template = _re.sub(
            r"\$\{\s*entry\.entries\.\*\.(\w+)\s*\}", r"{\1}", template)
        # Equivalent compact dialect occasionally emitted by planners.  The
        # surrounding write loop already renders once per current record and
        # coalesces equal paths, so the omitted ``entry.`` is unambiguous.
        template = _re.sub(
            r"\$\{\s*entries\.\*\.(\w+)\s*\}", r"{\1}", template)
...
def _collect_write_specs(args: dict):
    ...
    if isinstance(entries, list):
        ...
        grouped_content_template = bool(content_template)
        ...
        def _with_set_fields(entry):
            if not (isinstance(set_fields, dict) and isinstance(entry, dict)):
                return True, entry
            rendered = {}
            for key, value in set_fields.items():
                if (isinstance(value, str)
                        and re.search(r"\$\{\s*entry\.", value)):
                    ok, value = _safe_format(value, entry)
                    if not ok:
                        return False, value
                rendered[key] = value
            return True, {**entry, **rendered}
        ...
        if not path_template and scalar_path and str(scalar_path).strip():
            ...
            for entry in entries:
                ok, entry = _with_set_fields(entry)
                if not ok:
                    return None, entry
        ...
        for entry in entries:
            ...
            specs.append({"path": p, "content": c,
                          "encoding": enc_default, "mode": mode_default})
        if grouped_content_template:
            # An explicit record content template plus equal rendered paths
            # means several records contribute to one target (for example one
            # report per domain).  Coalesce before any I/O so fail_if_exists
            # remains collision-safe against pre-existing files without
            # treating members of this invocation as external collisions.
            grouped: dict[str, dict] = {}
            for spec in specs:
                path_key = str(spec["path"])
                if path_key not in grouped:
                    grouped[path_key] = dict(spec)
                    continue
                previous = str(grouped[path_key].get("content") or "")
                current = str(spec.get("content") or "")
                grouped[path_key]["content"] = "\n".join(
...
def _write_one(path, content, encoding, mode):
    ...
    prev_blob_path = None
    if pre_existed and mode in {"overwrite", "append"}:
        try:
            prev_blob_path = backup_file_for_undo(abs_path)
        except OSError as exc:
            return False, {
                "path": abs_path,
                "ok": False,
                "error_code": "ERR_UNDO_BACKUP_FAILED",
                "error": _msg("ERR_OP_FAILED", reason=str(exc)),
                "detail": "pre-write undo backup failed; file left unchanged",
            }, []
    ...
    try:
        if encoding == "binary":
            ...
            entry = {"path": abs_path, "created": (not pre_existed),
                     ...
                     "mode": mode}
            if prev_blob_path:
                entry["prev_blob_path"] = prev_blob_path
...
def move(args: dict) -> dict:
    ...
    for i, entry in enumerate(entries):
        ...
        prev_blob_path = None
        if overwrite and dst_path.exists() and dst_path != src_path:
            if dst_path.is_symlink() or not dst_path.is_file():
                failed.append({
                    ...
                    "error": _msg(
                        "ERR_OP_FAILED",
                        reason="overwrite of a non-regular destination is not reversible",
                    ),
                })
                continue
            try:
        ...
        if ok:
            row = {"src": str(src_path), "dst": str(dst_path)}
            if prev_blob_path:
                # After swap_src_dst restores the source, this path is where
                # restore_blob_backup must put the clobbered destination.
...
def delete_files(args: dict) -> dict:
    ...
    out = vector_result(results, failed, entry_key="results")
    if results:
        # Select only the local reverse branch within the manifest ceiling.
        # Provider-backed deletion records their own JIT reverse metadata.
        out["_undo"] = {
...
def _resolve_spreadsheet_output_path(raw: str | Path) -> Path:
    """Resolve explicit spreadsheet paths like the other file backends.

    A bare filename keeps the convenient ``spreadsheets/`` default.  A
    relative path with directories is already an explicit workspace location
    and must use the same canonical resolver as directories and ordinary
    files.  Using ``PATH_USER_DATA`` directly is observably different on
    remote Windows shims, where the file workspace is rooted at
    ``~/.local/share/metnos``.
    """
    ...
    if path.is_absolute():
        return path
    if path.parent == Path("."):
        return _spreadsheet_out_dir() / path
    return _normalize_input_path(str(path))
...
def _resolve_xlsx_path(args: dict, *, must_exist: bool):
    ...
    if not raw:
        return None, {"ok": False, "error_code": "ERR_ARG_MISSING",
                      ...
                      "error_class": "invalid_args"}
    p = _resolve_spreadsheet_output_path(raw)
...
def _rows_to_values(rows_source, columns) -> list:
    """Normalizza righe dirette e materializza l'header dichiarato, se manca.

    Vale sia per ``entries=list[list]`` provenienti da read_spreadsheet, sia per
    ``values=list[list]`` prodotti direttamente dal planner.  I due carrier
    hanno la stessa semantica tabellare: se ``columns`` è presente, la larghezza
    combacia e riga 0 non è già quell'header, lo anteponiamo.  In caso di
    larghezza incompatibile preserviamo le righe as-is (dati > cosmesi).

    §2.8: evita sia la perdita silenziosa delle row-entries (turn
    3da933e5-family), sia fogli popolati ma senza intestazione quando il planner
    separa ``columns`` e ``values`` (turn 68629b56974c4701).
    """
    rows = _normalize_rows(rows_source)
...
def _drop_declared_header(rows: list, columns) -> list:
    ...
    if not cols or not rows or len(rows[0]) != len(cols):
        return rows
    head0 = [str(c).strip().casefold() for c in rows[0]]
    declared = [c.strip().casefold() for c in cols]
    return rows[1:] if head0 == declared else rows
...
def create_spreadsheet(args: dict) -> dict:
    ...
    if explicit_path:
        path = _resolve_spreadsheet_output_path(explicit_path)
...
def write_spreadsheet(args: dict) -> dict:
    ...
    raw = (raw or "").strip()
    if raw:
        path = _resolve_spreadsheet_output_path(raw)
    else:
    ...
    try:
        if _is_csv(path):
            ...
            if file_mode == "a" and path.stat().st_size > 0:
                written_rows = _drop_declared_header(rows, args.get("columns"))
            with path.open(file_mode, newline="", encoding="utf-8") as fh:
                csv.writer(fh).writerows(written_rows)
        else:
            import openpyxl
            if existed:
                ...
                if mode == "overwrite":
                    ...
                    first_row_has_data = any(
                        cell.value is not None
                        for cell in next(ws.iter_rows(min_row=1, max_row=1), ()))
                    if first_row_has_data:
                        written_rows = _drop_declared_header(
                            rows, args.get("columns"))
            else:
...
# --------------------------------------------------------------------------
# LIST DIRS  (enumerazione di un livello, §2.2 `list` = contenitore)
# --------------------------------------------------------------------------
# Trasferito qui dall'executor il 20/9 (RM-0011 F0). Era l'unico verbo su
# cartelle rimasto dentro il proprio executor: find/create/delete_dirs stavano
# gia' in questo modulo. Finche' la logica locale viveva la' dentro, list_dirs
# non poteva avere un provider senza diventare un'eccezione.
...
def _mtime_iso(value: float) -> str | None:
    ...
    try:
        return datetime.datetime.fromtimestamp(
            value, datetime.timezone.utc).isoformat()
...
def list_dirs(args: dict) -> dict:
    """Elenca il contenuto di una cartella locale: file, cartelle, link.

    Non filtra (`filter_entries` e' il posto giusto per farlo) e arricchisce
    ogni voce con tipo, categoria, dimensione e data.
    """
    if not isinstance(args, dict):
        return {
            "ok": False,
    ...
    base, alias_note = _resolve_path_with_alias(path)
    ...
    failed = [_walk_failure(error.path, error.reason)
