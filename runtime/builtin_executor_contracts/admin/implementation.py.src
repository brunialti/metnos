...
from safety.canonicalize import (
    apply_declared_option_defaults,
    declared_numeric_option_values,
    ArgvValidationError,
    ValidatedArgv,
...
# The runtime passes the user's original request to this builtin; a value
# with the same name coming from the planner is always discarded.
ACCEPTS_REQUEST_TEXT = True
AUTHORISED_CALLERS = (
...
# planner): DEVI/NON DEVI/OK/ERRORE pattern per the prompt style rules §6.
MANIFEST_VIRTUAL = {
    "name": "admin",
    "version": "1.1.0",
    "description": (
...
# A whitelisted command runs without a card only when the user's request names
# every literal operand the approved pattern does not already cover.  The rule
# is structural and language-neutral, never a list of names: an operand the
# request does not contain gets the ordinary approval card, so a substituted
# target is shown to the user before it runs.
_REQUEST_NOT_CHECKED = object()
_STANDALONE_FLAG_RE = re.compile(r"-[A-Za-z0-9]|--[A-Za-z0-9][A-Za-z0-9-]*")
_NUMBER_WORD_RE = re.compile(r"\d+(?:[.,]\d+)?")


def _request_words(request_text: str) -> frozenset[str]:
    """Words of the request exactly as written: nothing is ever removed.

    Any character can belong to a target (``1::``, ``/tmp/note)``), so a word
    carrying extra punctuation simply does not match and the command gets
    the ordinary card.
    """
    return frozenset(request_text.split())


def _approved_action(pattern: object) -> str | None:
    """The action a whitelist pattern fixes literally, or None for ``*``."""
    parts = str(pattern).split(":", 2)
...
def _operands_in_request(validated: ValidatedArgv, request_text: object, *,
    ...
    matched whitelist pattern in the first positional slot.  An option value
    the central grammar declares numeric is covered when the request states
    it, or when it equals the declared default and the request states no
    number at all.  Everything else, compact flags and ``--key=value`` values
    included, must appear verbatim in the request.  Without a request text
    there is no silent execution.
    """
    if not isinstance(request_text, str) or not request_text.strip():
        return False
    words = _request_words(request_text)
    argv = validated.command_argv
    declared = declared_numeric_option_values(validated.signature.binary, argv)
    request_states_a_number = any(_NUMBER_WORD_RE.fullmatch(word) for word in words)
    ...
    for index in range(1, len(argv)):
        token = argv[index]
        if _STANDALONE_FLAG_RE.fullmatch(token):
            continue
        if index in declared:
            default = declared[index]
            if token in words or (
                default is not None and not request_states_a_number
                and float(token) == default
            ):
                continue
            return False
...
def _evaluate_validated_argv(
    ...
    *,
    ...
    When ``request_text`` is given, a whitelist hit runs silently only if the
    request names every operand the approved pattern does not cover; otherwise
    it yields the ordinary approval card.
    """
    ...
    try:
        ...
        silent_allowed = whitelisted_row is not None and (
            request_text is _REQUEST_NOT_CHECKED
        ...
        if whitelisted_row is not None and not _is_admin_actor(actor):
            audit["guest_policy"] = "card_required_no_execution"
        elif whitelisted_row is not None:
            audit["silent_withheld"] = "operands_not_in_request"
...
def _format_credentials_required(domain: str, ctx: dict) -> str:
    ...
    Dialog-manager pattern (project rules §10.6, project_dialog_manager_authorization_ux):
...
def _invoke_impl(*, intent: str, command_proposed: str,
    ...
    try:
        validated = validate_argv(argv)
        # Declared option defaults join the command before any card or
        # consent, so what the user sees and approves is exactly what runs.
        normalized = apply_declared_option_defaults(
            validated.signature.binary, validated.command_argv)
            ...
            payload = {
                ...
                "on_complete": {
                    ...
                    "resume_args": {
                        ...
                        "credentials_domain": target_domain,
                        # Server-side pending state keeps the original
                        # request, so the resumed call is judged on it too.
