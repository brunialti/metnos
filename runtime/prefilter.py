...
# ACTION_MAPPING e' la fonte condivisa dei sinonimi semantici usata da synt e
# intent extraction. La mappa locale conserva soltanto le forme flesse e i
# casi contestuali; ogni alias monolitico della SoT entra automaticamente nel
# prefilter, evitando che la comprensione dipenda da tre liste divergenti.
try:
    from vocab import ACTION_MAPPING as _ACTION_MAPPING
    for _canonical, _mapping in _ACTION_MAPPING.items():
        for _lang in ("it", "en"):
            for _alias in _mapping.get(_lang, []):
                if isinstance(_alias, str) and re.fullmatch(r"[a-z0-9]+", _alias):
                    _VERB_TO_CANONICAL.setdefault(_alias, _canonical)
...
def is_direct_message_query(query: str | None) -> bool:
    """Recognise an explicit recipient followed by a message body.

    ``scrivi a <recipient>: <body>`` / ``write to <recipient>: <body>`` is
    messaging, not filesystem writing.  Requiring both the recipient/body
    separator and a non-path target keeps ordinary ``write to /path`` queries
    in the files domain.  This deterministic boundary is also reused by the
    smoke gate and the alternative prefilter strategies.
    """
    return bool(query and _DIRECT_MESSAGE_RE.search(query))
...
def rank(query, catalog, k=10, min_score=1):
    ...
    canonical_verb = detect_canonical_verb(qtokens, query)
...
def rank_with_intent(query, catalog, intent, *, k=3):
    ...
    # Syntactic high-confidence boundary: an explicit human recipient plus a
    # colon-delimited body is a message even when an LLM classifies the
    # polysemous surface verb "scrivi/write" as filesystem writing.
    if is_direct_message_query(query):
        verb, obj = "send", "messages"
    if not verb:
